...
SET rt.SegmentTypeId =
    (
        SELECT CrsId FROM dbo.CatRateSegment WITH(NOLOCK) WHERE CrsShortName = 'NAG'
    ),
...
SET SegmentTypeId =
    (
        SELECT CrsId FROM dbo.CatRateSegment WITH(NOLOCK) WHERE CrsShortName = 'MEG'
    ),
	TokenUpdated = 'SYS-BPEDROZA',
	DateUpdated = GETDATE()
WHERE RateId = @IdRate
      AND TownshipSourceId IN ( 84, 73,74,78,81, 86, 87, 79, 80 )
      AND TownshipDestinyId IN ( 84, 73,74,78,81, 86, 87, 79, 80 )
      AND RowStatus = 1;
...
SET rt.SegmentTypeId =
    (
        SELECT CrsId FROM dbo.CatRateSegment WITH(NOLOCK) WHERE CrsShortName = 'ESP'
    ),
	rt.TokenUpdated = 'SYS-BPEDROZA',
	rt.DateUpdated = GETDATE()
...
--SEGMENTO CAPITAL
IF NOT EXISTS(
		SELECT CrsId FROM dbo.CatRateSegment WHERE CrsShortName = 'CAG'
)
BEGIN
...
UPDATE rt
	SET	RT.SegmentTypeId =   (
        SELECT CrsId FROM dbo.CatRateSegment WITH(NOLOCK) WHERE CrsShortName = 'CAG'
    ),
	rt.TokenUpdated = 'SYS-BPEDROZA',
	rt.DateUpdated = GETDATE()
	FROM dbo.RateTownshipCoverage rt
	...
	WHERE rt.RateId = @IdRate 
	AND rt.RowStatus = 1 
	AND TWD.IdTownship IN ( 84, 73,74,78,81, 86, 87, 79, 80 )
	AND PRO.IdProvince <> 7
	AND rt.SegmentTypeId not IN (
					SELECT CrsId FROM CatRateSegment WITH(NOLOCK) WHERE CrsShortName IN ('MEG', 'LOC')
					);
...
-- CONFIGURAR COBERTURA DEPARTAMENTAL
	...
	AND rt.SegmentTypeId NOT IN (
						SELECT CrsId FROM CatRateSegment WITH(NOLOCK) WHERE CrsShortName IN ('LOC','MEG', 'CAG')
						);
...
SET rt.SegmentTypeId =
    (
        SELECT CrsId FROM dbo.CatRateSegment WITH(NOLOCK) WHERE CrsShortName = 'REG'
    ),
	rt.TokenUpdated = 'SYS-BPEDROZA',
	rt.DateUpdated = GETDATE()
FROM dbo.RateTownshipCoverage rt
...
WHERE rt.RateId = @IdRate
      ...
      AND PRD.IdProvince IN ( 7, 6, 16, 3, 18 )
      	AND rt.SegmentTypeId not IN (
					SELECT CrsId FROM CatRateSegment WITH(NOLOCK) WHERE CrsShortName IN ('MEG', 'LOC', 'FOR','CAG')
					);
	--3		Chimaltenango
	--6		Escuintla
	--7		Guatemala
...
--SEGMENTO FORANEO PETEN
-- Foraneo Peten
UPDATE rt
SET rt.SegmentTypeId =
    (
        SELECT CrsId FROM dbo.CatRateSegment WITH(NOLOCK) WHERE CrsShortName = 'FPG'
    ),
	rt.TokenUpdated = 'SYS-BPEDROZA',
	rt.DateUpdated = GETDATE()
FROM dbo.RateTownshipCoverage rt
