...
DECLARE @ParentModuleId INT = (SELECT TOP 1 CM.ModIdModule FROM [DeliveryBackOffice].[dbo].[CatModule] CM WITH(NOLOCK) WHERE CM.ModName = 'Mi perfil' COLLATE Latin1_General_CI_AI AND CM.ModPath = '/individual')
...
VALUES
	('Terminos y condiciones', @ParentModuleId, '/individual/terminos-condiciones', 'Nuevo módulo individual de terminos y condiciones', 6, 'bi-file-text-fill', 1, 1, 'SYS-ARUIZ', GETDATE(), NULL, NULL, 0)
...
SELECT
	@RolID, @SystemId, IM.ModuleId, 1, 'SYS-ARUIZ', GETDATE(), NULL, NULL, 2, NULL
FROM
	@InsertedModule IM