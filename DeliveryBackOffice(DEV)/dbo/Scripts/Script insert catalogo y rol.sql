...
GO
INSERT [dbo].[CatModule] ( [ModName], [ModIdModuleParent], [ModPath], [ModDescription], [ModOrder], [ModMetadata], [ModVisible], [ModRowStatus], [ModTokenCreated], [ModDateCreated], [ModTokenUpdated], [ModDateUpdated], [ModGroup]) VALUES (N'Centro de Canje', NULL, N'/express/centro-canje', N'Centro de canje express center ', 45, N'fa bi bi-shop-window fa-1x', 1, 1, N'evasquez', CAST(N'2024-01-06T00:39:59.723' AS DateTime), NULL, NULL, 1)
GO


--- rol aquí tiene que ir en este campo: [RmsIdModule] que se insertan  en la tabla CatModulo

DECLARE @IDMODULE INT =(select * from dbo.catmodule WHERE ModDescription='Centro de Canje' and ModPath='/individual/centro-canje');
DECLARE @IDMODULEEXC INT =(select modidmodule from  dbo.catmodule WHERE ModDescription='Centro de canje express center ' and ModPath='/express/centro-canje');
