BEGIN TRY
    ...
    --CORREGIR ORDER PLAN AMIGO PARA DEJARLO COMO PRODUCCION
    DECLARE @IdCatAMIGO INT = (
    ...
    DECLARE @IdMaxPosition INT = (
                                     SELECT MAX(MPTP.Position) + 1
                                     FROM [MarketplaceTagsByProduct] MPTP WITH (NOLOCK)
                                     WHERE MPTP.MarketplaceProductTagsId = @IdCatTODOS
                                 );

    UPDATE MPTP
    SET MPTP.Position = @IdMaxPosition,
        MPTP.TokenUpdated = 'SYS-BPEDROZA',
        MPTP.DateUpdated = GETDATE()
    FROM [MarketplaceTagsByProduct] MPTP
    WHERE MPTP.CatSubscriptionId = @IdCatAMIGO
