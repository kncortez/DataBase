CREATE TABLE [dbo].[DeliveryLink] (
    ...
    [IsUserWithoutLogin]            BIT             DEFAULT ((0)) NULL,
    [NirPhone]                      NVARCHAR (5)    NULL,
    [Packages]                      VARCHAR (MAX)   NULL,
    PRIMARY KEY CLUSTERED ([IdDeliveryLink] ASC),
    CONSTRAINT [FK_DeliveryLink_AccountId] FOREIGN KEY ([AccountId]) REFERENCES [dbo].[Account] ([AccIdAccount]),
    CONSTRAINT [FK_DeliveryLink_CatPaymentTypeId] FOREIGN KEY ([CatPaymentTypeId]) REFERENCES [dbo].[CatPaymentType] ([PayTypeId]),
    CONSTRAINT [FK_DeliveryLink_CatTypeServiceId] FOREIGN KEY ([CatTypeServiceId]) REFERENCES [dbo].[CatTypeService] ([CtsId]),
    CONSTRAINT [FK_DeliveryLink_DeliveryFacCODId] FOREIGN KEY ([DeliveryFacCODId]) REFERENCES [dbo].[DeliveryFavCOD] ([IdDeliveryFavCOD]),
    CONSTRAINT [FK_DeliveryLink_DeliveryLinkStatusId] FOREIGN KEY ([DeliveryLinkStatusId]) REFERENCES [dbo].[DeliveryLinkStatus] ([IdDeliveryLinkStatus]),
    CONSTRAINT [FK_DeliveryLink_GuideSerie] FOREIGN KEY ([GuideSerie], [GuideNumber]) REFERENCES [dbo].[DeliveryOrder] ([Guide_Serie], [Guide_Number]),
    CONSTRAINT [FK_DeliveryLink_ReceiverCatCityPlaceId] FOREIGN KEY ([ReceiverCatCityPlaceId]) REFERENCES [dbo].[CatCityPlace] ([IdCityPlace]),
