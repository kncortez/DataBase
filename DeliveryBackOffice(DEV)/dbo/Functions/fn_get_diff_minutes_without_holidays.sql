...
BEGIN
	...
	IF CAST(@StartDate AS DATE) IN (SELECT nlc.NoLaborDate FROM DeliveryBackOffice.dbo.NoLaborCalendar nlc WITH (NOLOCK) WHERE nlc.RowStatus = 1) 
    BEGIN
        SET @NewStartDate
	...
	IF CAST(@EndDate AS DATE) IN (SELECT nlc.NoLaborDate FROM DeliveryBackOffice.dbo.NoLaborCalendar nlc WITH (NOLOCK) WHERE nlc.RowStatus = 1) 
    BEGIN
        SET @NewEndDate
	...
	WHERE CAST(CTE.date_list AS DATE) IN (SELECT nlc.NoLaborDate FROM DeliveryBackOffice.dbo.NoLaborCalendar nlc WITH(NOLOCK) WHERE nlc.RowStatus = 1)
