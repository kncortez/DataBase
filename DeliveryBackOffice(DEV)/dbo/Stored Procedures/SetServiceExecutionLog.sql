...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO ScheduleServiceHistory
			VALUES (@name, @description, CAST(@status AS INT), @token, GETDATE(), NULL, NULL, @time,(SELECT sc.IdServicesConfig FROM ServicesConfig sc WITH (NOLOCK) WHERE sc.ServiceProcess = @ProcessName), @Type)
