...
-- =============================================
Create PROCEDURE [dbo].[sphdSetVisitPointClientParser]
        ...
        BEGIN TRY
            ...
            BEGIN
				...
				IF ((@CustomerID > 0)  AND (@IdVisitPoint = -1)
				   AND NOT EXISTS(SELECT vpc.IdVisitPointClient FROM dbo.VisitPointClientParser vpc 
									WHERE vpc.DescriptionOfClient =  @DescriptionOfClient 
									AND (vpc.CustomerID = @CustomerID)))
                BEGIN
					 DECLARE @CodeOfReference AS INT = -1;
					 SET @CodeOfReference = (SELECT TOP (1) vpc3.CodeOfReference + 1 FROM dbo.VisitPointClientParser vpc3 ORDER BY vpc3.CodeOfReference DESC)
					
                    INSERT INTO dbo.VisitPointClientParser
                    (
                        CodeOfReference,
                        DescriptionOfClient,
                        StatusClient,
            ...
            BEGIN
                ...
                IF ((@IdVisitPoint > 0) AND EXISTS (SELECT vpc.IdVisitPointClient FROM dbo.VisitPointClientParser vpc WHERE vpc.CodeOfReference = @IdVisitPoint)) 
                BEGIN
                    UPDATE DeliveryBackOffice.dbo.VisitPointClientParser
