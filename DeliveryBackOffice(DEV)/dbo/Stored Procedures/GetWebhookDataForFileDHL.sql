...
BEGIN
	...
	/***************************************************************************
	************* EVALUAR QUE TIPO DE ACCI�N SE VA A REALIZAR ******************
	****************************************************************************/
	...
	BEGIN
		...
		IF (@HOSTNAME <> '') 
		BEGIN
			--Definir el nombre del archivo y creaci�n del lote
			...
			SELECT '202' [status], 'Acci�n Creacion de lote' [message];
	...
	BEGIN
		...
		--PRINT '-- ASIGNACI�N DE GUIAS A LOTE GENERADO --'
		-- Guias no asignadas 
	...
	BEGIN
		...
		--PRINT '-- GENERACI�N DE ARCHIVO QUE SE ENVIARA A FORZA --'
	...
	ELSE IF (@T_TYPE = 4)
	BEGIN
		--PRINT '-- ACTUALIZACION DEL ESTATUS DEL LOTE ESPECIFICADO (CREACI�N DEL ARCHIVO Y LOTE) --'
	...
	ELSE 
	BEGIN
		Select '404' [status], 'Acci�n No definida' [message];
	END
