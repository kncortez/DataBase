...
BEGIN
    ...
    BEGIN
        ...
        SELECT ModIdModule AS Id,
                ModName AS [Description]
    ...
    BEGIN
		...
		DECLARE @NewAutoSalesMainRates INT = (SELECT TOP 1 RH.RheId FROM [DeliveryBackOffice].[dbo].[RateHeader] RH WITH(NOLOCK) WHERE RH.RheName = 'Tarifario de servicio estandar autoventas' COLLATE Latin1_General_CI_AI);
          ...
          SELECT  ISNULL(cd.[AbcId], 0) AS Id, 
					...
					ISNULL(cd.[Height], 30) AS Height, 
					ISNULL(cd.[Width], 30) AS Width, 
					ISNULL(cd.[Length], 30) AS Length, 
					ISNULL(cd.MassWeight, 1) AS Weight,
    ...
    BEGIN
        ...
        SELECT  DescriptionOfClient AS [Name], 
				...
				ISNULL(TWS.IdTownship, '') AS IdTownship, 
    ...
    BEGIN
        SELECT  CONVERT(NVARCHAR, IdDeliveryOption) AS Id,
				ISNULL(Name, 'N/A') AS [Name], 
				ISNULL(Description, 'N/A') AS [Description]
    ...
    BEGIN
       ...
       SELECT DISTINCT
			    ...
			    SELECT DISTINCT ISNULL(TWS.HeaderCode, '') AS HeaderCode,
						...
						ISNULL([CODAccountBankID], '') AS IdBank,
						ISNULL(dbk.[Name], '') AS BankDescription,
						ISNULL(dbk.[Acronym], '') AS Acronym,
						ISNULL([CODAccountName], '') AS NameAccount,
						ISNULL(cba.[BankAccountType], '') AS TypeAccount,
    ...
    BEGIN
        ...
        PRINT 'PRUEBA 15';
        SELECT ISNULL(c.IdIncidenceType, '') AS Id,
    ...
    BEGIN
       SELECT CONVERT(NVARCHAR, IdCatIncidenceTransfer) AS Id,
    ...
    BEGIN
        SELECT cta.TarId AS ArticleTypeId,
			   cta.TarName AS ArticleType
