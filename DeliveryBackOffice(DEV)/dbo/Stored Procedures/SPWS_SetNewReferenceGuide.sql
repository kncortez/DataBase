...
BEGIN
    BEGIN TRANSACTION
            BEGIN TRY
                ...
                SELECT GuideSerie, GuideNumber, TicketNumber, StatusOrder = 0, IdCustomer = 0
                INTO #Guides
                FROM @TblNewReferenceGuides;

                UPDATE G
                SET G.StatusOrder = ISNULL(O.StatusOrderId,0),
                    G.IdCustomer = ISNULL(O.IdCustomer,0)
                FROM #Guides G
                CROSS APPLY (
                ...
                BEGIN
                    SELECT 0 AS [StatusCode], 'Las guías que intentas modificar no están en un estado que permita esta acción. Solo puedes modificar guías que estén en los estados "Generado" o "Solicitado". Revisa el estado de las guías antes de continuar.' AS [Message]
                END
                ELSE
                BEGIN
