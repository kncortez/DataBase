...
SELECT [PerFirstName] AS 'NOMBRES'
  ...
  JOIN [DeliveryBackOffice].[dbo].[RegisterUser] ru WITH(NOLOCK) on ru.UsrIdPerson = p.PerIdPerson
  JOIN [DeliveryBackOffice].[dbo].[RolByUserByAccount] rbuba WITH(NOLOCK) on rbuba.RuaIdUser = ru.UsrIdUser
  JOIN [DeliveryBackOffice].[dbo].[Account] a WITH(NOLOCK) ON a.AccIdAccount = rbuba.RuaIdAccount
  LEFT JOIN [DeliveryBackOffice].[dbo].[DeliveryOrder] do WITH(NOLOCK) on do.IdCustomer = a.IdCustomer and do.StatusOrderId NOT IN (7) -- anuladas
