...
BEGIN
    ...
    CREATE NONCLUSTERED INDEX tempGuides
    ON #listGuides (
                       Guide_Serie
                     , Guide_Number
                   );
    ...
    CREATE NONCLUSTERED INDEX IX_LGNE_NGUIDES
    ...
    CREATE NONCLUSTERED INDEX IX_LGI_GUIDES
    ON #listGuidesIncluded (
                               Guide_Serie
                             , Guide_Number
                           );
    -----------------------------------------------------------------------------------------------------------------
    ...
    CREATE NONCLUSTERED INDEX IX_LGE_GUIDES
    ON #listGuidesExcluded (
                               Guide_Serie
                             , Guide_Number
                           );
    -----------------------------------------------------------------------------------------------------------------
    ...
    CREATE NONCLUSTERED INDEX IX_PPT_GNS
            ...
            (
                ...
                FROM #PendingPaymentTemp                            ppt
                    INNER JOIN DeliveryBackOffice.dbo.DeliveryOrder do WITH (NOLOCK)
                        ON ppt.GuideSerie = do.Guide_Serie
                           AND ppt.GuideNumber = do.Guide_Number
                WHERE Guide_Serie = ppt.GuideSerie
                      AND Guide_Number = ppt.GuideNumber
            );
    ...
    INSERT INTO @CODAnticipatedTable
    (
        GuideSerie
      , GuideNumber
      , IdCustomer
         ...
         , IIF((do.Collect_OnDelivery - ISNULL(RCO.CODExempt, @CODExemptDefault)) > 0
             , IIF(OP.Deposit_Number IS NULL
                 , IIF(ISNULL(VPC.ExcludeCommissionCOD, ISNULL(C.ExcludeCommissionCOD, 0)) = 1
                     , 0
                     , (CONVERT(DECIMAL(18, 2), ((do.Collect_OnDelivery) * ISNULL(RCO.CODRate, @CODRateDefault) / 100))))
                 , 0)
             , 0) AS 'ComisionCOD'
         , CASE
               WHEN (
                        ACC.AnticipatedCODComission IS NOT NULL
                        AND ACC.AnticipatedCODComission > 0.00
                    )
                    AND
                    (
                        ACC.InitialRange <= do.Collect_OnDelivery
                        AND do.Collect_OnDelivery <= ACC.FinalRange
                    ) THEN
                   ACC.AnticipatedCODComission
               ELSE
                   CASE
                       WHEN CPmin1.Value >= do.Collect_OnDelivery
                            AND do.Collect_OnDelivery <= CPmax1.Value THEN
                           CAST(CPv1.Value AS DECIMAL)
                       WHEN CPmin2.Value >= do.Collect_OnDelivery
                            AND do.Collect_OnDelivery <= CPmax2.Value THEN
              ...
              (
                  SELECT ' { "Guide": "' + pg.Guide + '", ' +
                      ...
                      '"SenderName": "' + pg.SenderName + '", ' + '"SenderAddress": "' + pg.SenderAddress + '", '
                         ...
                         + '", ' + '"Indications": "' + pg.Indications + '", ' + '"CurrencySymbol": "'
                         + CurrencyPriceSymbol + '.",' + '"AmountToCollect": '
                         + CAST(CAST(pg.AmountToCollect AS DECIMAL(18, 2)) AS VARCHAR) + ', ' + '"ServicePrice": '
                         + CAST(CAST(ISNULL(pg.AmountToPay, 0) AS DECIMAL(18, 2)) AS VARCHAR) + ', ' + '"CODAmount": '
                         + CAST(CAST(ISNULL(pg.CODAmount, 0) AS DECIMAL(18, 2)) AS VARCHAR) + ', '
                         + '"PriceShippment": ' + CAST(CAST(ISNULL(pg.Price, 0) AS DECIMAL(18, 2)) AS VARCHAR) + ', '
                         + '"TypePayment": "' + pg.TypePayment + '"' + ', ' + '"IsCollect": '
                         + CAST(ISNULL(pg.IsCollect, 0) AS VARCHAR) + ', ' + '"Pieces": '
                         + CAST(ISNULL(pg.Pieces, 0) AS VARCHAR) + ', ' + '"ServiceType": "' + pg.ServiceType + '", '
                         + '"IdCustomer": ' + CAST(ISNULL(c.IdCustomer, 0) AS VARCHAR) + ', ' + '"IdPortafolio": '
                         + CAST(ISNULL(c.IdPortafolio, 0) AS VARCHAR) + ', ' + '"COD": '
                         + CAST(ISNULL(c.COD, 0) AS VARCHAR) + ', ' + '"ComisionCOD": '
                         + CASE
                               WHEN pg.ServiceType != 'COD' --Si la guía no es de tipo COD no deberia cobrar comision
                      THEN
                                   '0'
                               WHEN ISNULL(c.ComisionCOD, 0) < ISNULL(@MinCODCommissionAmount, 0) THEN
                                   ISNULL(CAST(@MinCODCommissionAmount AS VARCHAR), '0')
                               ELSE
                                   ISNULL(CAST(c.ComisionCOD AS VARCHAR), '0')
                           END + ', ' + '"ComisionCODAnticipated": '
                         + CAST(ISNULL(c.ComisionCODAnticipated, 0) AS VARCHAR) + ', ' + '"GuideDetail": [ '
                  FROM #PendingPaymentTempId         pg
                      LEFT JOIN @CODAnticipatedTable c
                          ON pg.GuideSerie = c.GuideSerie
                             AND pg.GuideNumber = c.GuideNumber
                  WHERE Id = @Index
              );

			  SELECT * FROM     #PendingPaymentTempId         pg
                      LEFT JOIN @CODAnticipatedTable c
                          ON pg.GuideSerie = c.GuideSerie
                             AND pg.GuideNumber = c.GuideNumber
                  WHERE Id = @Index
                         ...
                         (
                             SELECT ' { "Guide": "' + CONCAT(lge.Guide_Serie, CAST(lge.Guide_Number AS VARCHAR))
                                 ...
                                 '"StatusOrderId": ' + CAST(ISNULL(lge.StatusOrderId, 0) AS VARCHAR) + ', '
                                    + '"Description": "' + 'Guía en estado : ' + lge.Description
                                    + ' , no permite realizar el proceso.' + '" }, '
