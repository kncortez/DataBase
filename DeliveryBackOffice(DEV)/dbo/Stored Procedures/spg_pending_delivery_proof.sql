...
BEGIN
    ...
    SET NOCOUNT ON;
    SELECT DO.Guide_Serie + CAST(DO.Guide_Number AS VARCHAR) AS Guide,
           DATT.ID_Courier,
           CONVERT(DATE, DATT.Date_Created) AS Fecha_Despacho,
           DATT.ID_Proof,
    ...
    WHERE (
              DATT.Delivered = 1
              OR DATT.ID_Incident IS NOT NULL
          )
          AND
          (
              Accepted IS NULL
    ...
    GROUP BY DO.Guide_Serie,
             DO.Guide_Number,
             DATT.ID_Courier,
             CONVERT(DATE, DATT.Date_Created),
             DATT.ID_Proof,
