...
BEGIN
    ...
    SELECT @StationId = cs.IdStation
     ...
     FROM DeliveryBackOffice.dbo.InternalUser iu
          JOIN DeliveryBackOffice.dbo.RegisterUser ru ON iu.RegisterUserID = ru.UsrIdUser
                                                     AND ru.UsrRowStatus = 1
          JOIN DeliveryBackOffice.dbo.RolByUserBySystem rus ON ru.UsrIdUser = rus.RusIdUser
                                                           AND rus.RusRowStatus = 1
          JOIN DeliveryBackOffice.dbo.CatStation cs ON rus.StationId = cs.IdStation
                                                   AND cs.RowStatus = 1
          LEFT JOIN DeliveryBackOffice.dbo.VisitPointClient VPC ON VPC.CodeOfReference = cs.CodeOfReference 
    ...
    SELECT @VpCodeOfReference VpCodeOfReference,
           @StationId StationId,
