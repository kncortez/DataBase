...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            IF EXISTS (SELECT 1 FROM @RouteAssignment)
            BEGIN
                COMMIT TRANSACTION;
                ...
                UPDATE URS
                SET URS.TotalGuidesSettled = TotalPieces.TotalGuides,
                ...
                FROM [DeliveryBackOffice].[dbo].[UnifiedRouteSettlement] URS WITH (NOLOCK)
                    INNER JOIN
                    (
                        SELECT URS.IdUnifiedRouteSettlement,
                               COUNT(DISTINCT URSD.GuideNumber) 'TotalGuides',
                               SUM(URSD.PiecesSettled) 'TotalSettled',
                               SUM(URSD.PiecesMissing + PiecesMissingExtra) 'TotalMissing'
                        FROM @RouteAssignment RA
                            ...
                            OUTER APPLY
                        (
                            SELECT COUNT(DISTINCT URSDP.PieceNumber) PiecesMissingExtra
                            FROM [DeliveryBackOffice].[dbo].[UnifiedRouteSettlementDetailPiece] URSDP WITH (NOLOCK)
                            WHERE URSDP.ActCode IS NOT NULL
                                  AND URSDP.UnifiedRouteSettlementDetailId = URSD.IdUnifiedRouteSettlementDetail
                                  AND URSDP.RowStatus = 1
                        ) URSDPmissing
                        GROUP BY URS.IdUnifiedRouteSettlement
                    ) TotalPieces
                        ON URS.IdUnifiedRouteSettlement = TotalPieces.IdUnifiedRouteSettlement;

                SELECT @TotalSettlementPieces = SUM(urs.TotalPiecesSettled)
                ...
                SELECT @TotalServicesPending = COUNT(DISTINCT sm.IdServiceManagement)
                FROM @RouteAssignment ra
                ...
                WHERE sm.RowStatus = 1
                      AND
                      (
                          stsm.IdSubTypeServiceManagment IS NULL
                          OR stsm.[Name] = 'Recolección'
                      )
                ...
                SELECT ISNULL(
                                 SUM(   IIF(
                                            stsm.[Name] IN ( 'Entrega', 'Devolución' )
                                            AND so.OrderDescription IN ( 'Entregado', 'Entregado En Express Center',
                                                                         'COD liquidado', 'COD pagado', 'Devuelto',
                                                                         'Devuelto en Express Center',
                                                                         'Traslado a Express Center'
                                                                       ),
                       ...
                       ISNULL(
                                 SUM(   IIF(
                                            ...
                                            AND so.OrderDescription NOT IN ( 'Generado', 'Solicitado', 'Anulado',
                       ...
                       ISNULL(
                                 ISNULL(
                                           SUM(   IIF(
                                                      (
                                                          (
                                                              URS.UserSettlement IS NULL
                                                              AND
                                                              (
                                                                  stsm.IdSubTypeServiceManagment IS NULL
                                                                  OR stsm.[Name] = 'Recolección'
                                                              )
                                                              AND so.OrderDescription NOT IN ( 'Generado',
                                                                                               'Solicitado', 'Anulado',
                                                                                               'Programado para recolección'
                                                                                             )
                                                          )
                                                          ...
                                                          (
                                                              URS.UserSettlement IS NULL
                                                              AND stsm.[Name] IN ( 'Entrega', 'Devolución' )
                                                              AND so.OrderDescription NOT IN ( 'Entregado',
                                           ...
                                           0
                                       ) - MAX(ISNULL(URS.TotalPiecesMissing, 0)),
                                 0
                             ) TotalPhysicalPieces,
                       ISNULL(
                                 SUM(   IIF(
                                            ...
                                            AND so.OrderDescription IN ( 'Generado', 'Solicitado',
                                                                         'Programado para recolección'
                                                                       ),
                                            do.Pieces_Dry + do.Pieces_Cold,
                                            0)
                       ...
                       + ISNULL(
                                   SUM(   IIF(
                                              URS.UserSettlement IS NULL
                                              AND stsm.[Name] IN ( 'Entrega', 'Devolución' )
                                              AND so.OrderDescription NOT IN ( 'Paquete Extraviado', 'Entregado',
                                                                               'Entregado En Express Center',
                                                                               'COD liquidado', 'COD pagado',
                                                                               'Devuelto',
                                                                               'Devuelto en Express Center',
                                   ...
                                   0
                               ) + @TotalServicesPending TotalPendingSettlement,
                       ISNULL(@TotalSettlementPieces, 0) TotalSettlementPieces,
                       ISNULL(
                                 SUM(   IIF(
                                            URS.UserSettlement IS NULL
                                            AND stsm.[Name] IN ( 'Entrega', 'Devolución' )
                                            AND so.OrderDescription IN ( 'Entregado', 'COD liquidado', 'COD pagado',
                                                                         'Devuelto'
                                                                       ),
                                            IIF(do.IsCollect = 1, do.PriceShippment, 0)
                ...
                WHERE (
                          css.[Name] <> 'Cancelado'
                          OR css.IdServiceStatus IS NULL
                      );

                -- Table 2 - Información Courier
                SELECT @SenderReceiverId CourierId,
