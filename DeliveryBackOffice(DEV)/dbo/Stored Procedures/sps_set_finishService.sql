...
BEGIN
    ...
    SELECT @ForzaPointsGenerationType = Value
    FROM #ConfigParamsTemp
    WHERE Name = 'ForzaPointsGenerationType';

    SELECT @ForzaPointsGenerationValue = Value
    FROM #ConfigParamsTemp
    ...
    (
        ...
        WHERE OrderDescription = 'COD Anticipado'
              AND RowStatus = 1
    );
    ...
    BEGIN
        ...
        BEGIN CATCH
            ...
            VALUES
            (CAST(ERROR_MESSAGE() AS VARCHAR(300)), ERROR_NUMBER(), CAST(ERROR_PROCEDURE() AS VARCHAR(3000)),
             ERROR_LINE(), NULL, NULL, @TokenP, GETDATE());
        END CATCH;
