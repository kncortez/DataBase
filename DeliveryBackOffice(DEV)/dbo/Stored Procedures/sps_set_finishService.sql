...
BEGIN
        ...
        BEGIN
            ...
            BEGIN --VER GUIAS VALIDAS
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        BEGIN
                            ...
                            DECLARE @statusOrderId INT = CASE UPPER(@ServiceType)
                                                             WHEN 'PICKUP' THEN
                                                                 21
                                                             WHEN 'DELIVERY' THEN
                                                                 22
                            ...
                            DECLARE @observations NVARCHAR(200)
                                = CASE UPPER(@ServiceType)
                                      WHEN 'PICKUP' THEN
                                          'Recibido de ' + @Name
                                      WHEN 'DELIVERY' THEN
                                          CONCAT('Entregado a ', @Name, ' ', ISNULL(@CUI, ''))
                                      WHEN 'RETURN' THEN
                                          'Devuelto a ' + @Name
                                  END;
                            ...
                            BEGIN
                                ...
                                INSERT INTO #GuidesToProcessTEMP
                                (
                                    GuideSerieTEMP,
                                    GuideNumberTEMP
                                )
                                SELECT dlo.Guide_Serie,
                                       dlo.Guide_Number
                                FROM #listGuidesEnabled lge
                                    INNER JOIN DeliveryOrder dlo WITH (NOLOCK)
                                        ON lge.Guide_Serie = dlo.Guide_Serie
                                ...
                                BEGIN
                                    ...
                                    EXEC [dbo].[SetServiceRecolectCODAnticipated] @GuideSerie = @GuideSerieT,
                                    ...
                                    WHERE GuideSerieTEMP = @GuideSerieT
                                          AND GuideNumberTEMP = @GuideNumberT;
                                END;
                            ...
                            SET @NewStatusOrderId = CASE
                                                        WHEN UPPER(@ServiceType) = 'PICKUP' THEN
                                                            21
                                                        WHEN UPPER(@ServiceType) = 'DELIVERY' THEN
                                                            22
                                                        WHEN UPPER(@ServiceType) = 'RETURN' THEN
                                                            23
                                                        ELSE
                                                            NULL -- opcionalmente manejar casos inesperados
                                                    END;
                ...
                BEGIN
                    ...
                    SELECT '2' AS 'ResponseCode',
                           'La guía no existe en el sistema.' AS 'Description',
            ...
            BEGIN --VER GUIAS INVALIDAS

                SELECT '3' AS 'ResponseCode',
                       'Todas las guías ingresadas son invalidas.' AS 'Description',
                       ISNULL(lge.StatusOrderDescription, '') AS 'StatusOrderDescription',
                       CONCAT(lge.Guide_Serie, CAST(lge.Guide_Number AS VARCHAR)) AS 'Guide',
                       CAST(ISNULL(lge.StatusOrderId, 0) AS VARCHAR) AS 'StatusOrderId'
                FROM #listGuidesDisabled lge;
    ...
    BEGIN CATCH
        ...
        VALUES
        (CAST(ERROR_MESSAGE() AS VARCHAR(300)), ERROR_NUMBER(), CAST(ERROR_PROCEDURE() AS VARCHAR(3000)), ERROR_LINE(),
         NULL, NULL, @TokenP, GETDATE());
...
--CODIGOS:
-- 1 -> Exitoso.
-- 2 -> La guía no existe en el sistema.
-- 3 -> Todas las guías ingresadas son invalidas.	
-- 4 -> Existen guías validas ingresadas pero tambien hay guías invalidas.
-- 5 -> La suma de las guias no coincide con el monto de pago.	
-- 6 -> Hubo un error en el procedimiento almacenado.
-- 7 -> No devuelve informacion en el procedimiento almacenado.
-- 8 -> Hubo un error en el procedimiento almacenado.