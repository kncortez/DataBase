...
BEGIN
 ...
 -- Datos cliente Cabecera de factura 
   ...
   --Datos detalle de factura
	...
	DECLARE		@dti_description varchar(MAX)=(Select [Description] From [dbo].[CatArticleSAP] WITH (NOLOCK) Where Name='Next Day Delivery Foraneo' COLLATE Latin1_General_CI_AI) 
	...
	DECLARE		@SAPCode nvarchar(50)=(Select SAPCode From [dbo].[CatArticleSAP] WITH (NOLOCK) Where Name='Next Day Delivery Foraneo' COLLATE Latin1_General_CI_AI)
		...
		BEGIN
		  ...
		  SELECT 
			...
			WHERE AccountId =   @IdAccount AND 
				 M.RowStatus = 1 AND 
				 M.CatMembershipId = @IdSalePackage
