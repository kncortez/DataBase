...
BEGIN
    ...
    BEGIN TRY
        ...
        SET @XmlVisitPointClient
            = CAST('<LstVisitPointClient><PointClient>'
                   + REPLACE(@LstVisitPointClient, ',', '</PointClient><PointClient>')
                   + '</PointClient></LstVisitPointClient>' AS XML);
        ...
        INSERT INTO @TempVisitPointClient
        (
            IdVisitPointClient,
            GuideSerie,
            GuideNumber
        ...
        WHERE id.dti_fk_header IS NULL
              ...
              (
                  ...
                  (
                      (
                          @Option = 3
                          AND ISNULL(ISNULL(vst.ExcludePriceShippingCOD, cus.ExcludePriceShippingCOD), 0) = 0
                          AND ISNULL(do.IsLastMileReturn, 0) = 0
                      ) -- COD Contado (NULL se trata como Contado)
                      OR
                      (
                          @Option = 4
                          AND ISNULL(ISNULL(vst.ExcludePriceShippingCOD, cus.ExcludePriceShippingCOD), 0) = 1
                          AND ISNULL(do.IsLastMileReturn, 0) = 0
                      ) -- COD Crédito
                      OR
                      (
                          @Option = 5
                          AND ISNULL(do.IsLastMileReturn, 0) = 1
                      ) -- Intentos de entrega
                  )
              )
              AND EXISTS
        ...
        IF EXISTS (SELECT TOP 1 1 FROM @InvoiceByVisitPointDetails)
        BEGIN
