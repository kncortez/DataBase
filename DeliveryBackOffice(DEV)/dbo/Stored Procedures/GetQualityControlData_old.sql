...
CREATE PROCEDURE [dbo].[GetQualityControlData_old]
    @Token VARCHAR(200) = ''
  , @IdCourier BIGINT
  , @DateRoute DATE
AS
BEGIN
    ...
    ON #TmpAlertList (SchedulePickupId);
                                                      ...
                                                      CONVERT(
                                                               ...
                                                               , ISNULL(
                                                                         ...
                                                                         , DATEADD(
                                                                                      HOUR
                                                                                    , 19
                                                                                    , CAST(CAST(spk.StartDate AS DATE) AS DATETIME)
                                                                                  )
                                                                       )
                                                               , 8
                                                             )
                                                 ...
                                                 SELECT STUFF(
                                                        (
                                                            SELECT TOP 1
                                                                   ...
                                                                   + (CONVERT(VARCHAR, doa.DateCreated, 24)) + ' - '
                                                                   + (CONVERT(VARCHAR, doa.DateCreated, 103)) + '"}'
                                                            FROM DeliveryOrderAlert doa WITH (NOLOCK)
                                                            WHERE doa.ServiceManagementId = sma.IdServiceManagement
                                                                  AND doa.RowStatus = 1
                                ...
                                (
                                    ...
                                    "ServiceType":"'
                                                          ...
                                                          + CONVERT(
                                                                       VARCHAR
                                                                     , ISNULL((CASE
                                                                                   WHEN ISNULL([DOR].[IsLastMileReturn], 0) = 0 THEN
                                                         ...
                                                         ISNULL(SDFG.Latitude, 0) != 0 AND ISNULL(SDFG.Longitude, 0) != 0
                                                          ...
                                                          + IIF((
                                                                    doadel.GuideNumber IS NOT NULL
                                                                    AND DOR.IsLastMileReturn = 0
                                                                )
                                                                OR
                                                                (
                                                                    doaret.GuideNumber IS NOT NULL
                                                                    AND DOR.IsLastMileReturn = 1
                                                                )
                                                              , ',"HighPriority":'
                                                                + CONVERT(
                                           ...
                                           SELECT STUFF(
                                                  (
                                                      ...
                                                      WHERE sm.ServiceStatusId IN ( 1, 4, 7, 8 )
                                                            ...
                                                            AND CONVERT(VARCHAR, sm.DateCreated, 103) = CONVERT(
                                                                                                                   VARCHAR
                                                                                                                 , GETDATE()
                                                                                                                 , 103
                                                                                                               )
