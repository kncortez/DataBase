...
-- Description:	< Validar y obtener puntos para poder realizar pago de guías de carrito de compras de express center >
-- =============================================
CREATE PROCEDURE [dbo].[spHW_ValidateForzaPointsFromExpressCenter] 
	@AccountId INT = NULL,
...
BEGIN
	...
	BEGIN
		...
		IF (@CustomerAccountId IS NULL AND @CustomerId IS NULL)
			BEGIN 
		...
		IF ((SELECT COUNT(Guide_Serie) FROM @GuidesList) > 0)
			BEGIN
				...
				IF (@CustomerAccountId IS NOT NULL AND (SELECT TOP 1 AccountId FROM @CustomerList) != @CustomerAccountId)
					BEGIN
		...
		IF (@CustomerAccountId IS NOT NULL) 
			BEGIN
				SET @AvailableForzaPoints = (	SELECT	SUM([M].[AvailablePoints])
												...
												WHERE	[M].[AccountId] = @CustomerAccountId
													AND [M].[RowStatus] = 1
	...
	ELSE
    BEGIN
		SELECT 0 [spResult], 'El proceso no puede continuar debido a que el usuario no tiene permisos de consultar la información.' [spMessage];
		RETURN;
    END
