...
BEGIN
    BEGIN TRY
        DECLARE @NoPiece INT,
                ...
                @Valid INT,
                @ValidCountry INT,
                @GuideSerie NVARCHAR(2) = N'FD',
                @InContainerGuides NVARCHAR(MAX) = N'',
        ...
        BEGIN
            ...
            CREATE NONCLUSTERED INDEX tempContainerGuide
            ON #TempContainerGuides (
                                        GuideSerie,
                                        GuideNumber
                                    );
            ...
            INSERT INTO @ListContainer
            VALUES
            (@Container);
            INSERT INTO @ListReferences
            VALUES
            (@Reference);

            INSERT INTO #TempContainerGuides
            EXEC GetGuidesByContainerByReference @ListContainer,
                                                 @ListReferences,
                                                 @IdCountry,
                                                 @IdPickup;;

            SET @InContainerGuides =
            (
                SELECT STRING_AGG(CONCAT(GuideSerie, GuideNumber, '-', NoPiece), ',')
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    (
                        SELECT 1
                        FROM ShippingContainer WITH (NOLOCK)
                        WHERE ReferenceContainer = @Container
                    )
                    BEGIN
                        SELECT 2 AS [StatusCode],
                    ...
                    BEGIN
                        ...
                        BEGIN
                            ...
                            (
                                SELECT 1
                                FROM FinishPickUpContainerDetail WITH (NOLOCK)
