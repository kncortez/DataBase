...
CREATE PROCEDURE [dbo].[GenerateDynamicJSON]
(
	...
	SELECT '['+ STUFF(
	   (		
		SELECT distinct 		
			...
			'"Collected":'       + CONVERT(VARCHAR,CASE WHEN Serv.IsCollect = 1 THEN 'true' else 'false' end) + ','+
			'"GuideNumber":'     + CONVERT(VARCHAR,Serv.Guide_Number) + ','+
			'"PaymentDocument":"'     + CASE WHEN paidguide.DocumentType = 0 THEN 'Dep. ' else 'Aut. ' END + COALESCE(paidguide.Deposit_Number,'') + '",'+
			'"CollectOnDelivery":' + CONVERT(VARCHAR,COALESCE(Settlement_Collect_OnDelivery,'0')) + ','+
			'"CollectAmount":' + CONVERT(VARCHAR,COALESCE(Serv.PriceShippment,'0')) + ','+
			...
			'"SenderName":"' + UPPER(COALESCE(serv.Sender_FirstName,'')) + ' '+ UPPER(COALESCE(serv.Sender_LastName,'')) + '",'+			
			'"PayManifest_Number":' + CONVERT(VARCHAR,COALESCE(PHead.Manifest_Number,'')) + ',' +
			
			'"BankId":' + CONVERT(VARCHAR,COALESCE(DCB.DCBA_Bank_Id,'-1')) + ',' +
			'"BankName":"' + CONVERT(VARCHAR,COALESCE(DLB.Acronym,'NO DISPONIBLE')) + '",' +
			'"BankAccountType":"' + CONVERT(VARCHAR,COALESCE(DCB.DCBA_BankAccountType,'NO DISPONIBLE')) + '",' +
			'"BankAccountId":"' + CONVERT(VARCHAR,COALESCE(DCB.DCBA_Num_account,'NO DISPONIBLE')) + '",' +
...
SELECT  distinct
			...
			'"Name":"' + Ctm.Name  + '",'+			
			'"Email":"' + COALESCE(cpr.NotificationEmail,'No existe correo asociado')  + '",'+
			'"PointOfService":"' + CONVERT(varchar,COALESCE('',''))  + '",'+ --QUITADO vpclient.ContactName
			'"FeeRate":0' + ','+
		...
		GROUP BY Ctm.IdCustomer,Ctm.Name,cpr.NotificationEmail
		--,vpclient.ContactName  QUITADO
