...
============================================
=== CHANGELOG ================================
2026-03-06	|	Épica: FDAPI-5556	|	Autor: Erick Guerra	|
2026-05-27  |   Épica: FDAPI-5556   |   Autor: Juan Ramírez |  Corrección para obtener información de todas las guías solicitadas
=========================================== */

CREATE OR ALTER PROCEDURE [dbo].[SPHW_GetManyShippingGuideDataInZPLFormat]  
(  
 ...
 BEGIN TRY  
  ...
  FROM [DeliveryBackOffice].[dbo].[DeliveryOrder] DO WITH(NOLOCK)  
   ...
   LEFT JOIN [DeliveryBackOffice].[dbo].[RatebyCustomer] RC WITH(NOLOCK)  
          ON DO.IdCustomer = RC.RbcIdCustomer  
         AND RC.RbcRowStatus = 1  
         AND RC.RbcCodeOfReference IS NOT NULL
   ...
   INNER JOIN (  
                    SELECT 
                         dop.GuideSerie,  
                         dop.GuideNumber,  
                         MAX(dop.PieceWeight)         AS PieceWeight,
                         MAX(dop.PiecePhysicalWeight) AS PiecePhysicalWeight, 
  ...
  WHERE DO.Sender_ID = ISNULL(RC.RbcCodeOfReference, DO.Sender_ID)
