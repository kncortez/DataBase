...
BEGIN
    ...
    SELECT ISNULL(res.UstStatus, 'N/A') UstStatus,
		   ...
		   (CASE WHEN ISNULL(usr.ChangePassword, 0) = 1 AND ISNULL(usr.UsrPasswordExpiration, CAST(GETDATE() AS DATE)) <= CAST(GETDATE() AS DATE) THEN 1 ELSE 0 END) PasswordExpired
    INTO #User
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							SELECT
							ROW_NUMBER() OVER(ORDER BY CMO.ModIdModule ASC),
								(
						...
						DECLARE @ValTAC INT = (SELECT [dbo].[FnValidateTermsAndConditions](@Username,0,1))
						...
						SET @RolEXP = (SELECT TOP 1 cr.RolName FROM RegisterUser ru
								...
								SELECT STUFF(
												(
													SELECT ',{"Name":"' + DescriptionOfClient + '",' + '"ContactName":"'
														   + ISNULL(ContactName, '') + '",' + '"Phone":"'
														   + ISNULL(VPC.Phone, '') + '",' + '"Email":"' + ISNULL(Email, '')
														   + '",' + '"IdTownship":"'
