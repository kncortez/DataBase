...
-- Description:	< Login de Comercios Afiliados usuarios internos >
-- =============================================

CREATE PROCEDURE [dbo].[GetAffiliateLogIn]	
	@UserCode BIGINT = 0,
	@Mail VARCHAR(50), 
	@UserName VARCHAR(200), 
	@Password VARCHAR(200), 

	@SystemName NVARCHAR(50) = 'Hermes Comercios Afiliados'
AS
    BEGIN
  ...
  --      DECLARE @IdSystem INT = (
		...
		--	WHERE 
		--		CS.SysNameSystem = 'Hermes Comercios Afiliados' COLLATE Latin1_General_CI_AI 
		--		AND 
		--		CS.SysRowStatus = 1
		--); 
        ...
        (
            SELECT 400 AS IdResult, 
                   'Usuario o contraseña inválida' AS Message, 
                   'Invalid' AS Id
            UNION
        ...
        (
            ...
            --                                    AND 
                                        ...
                                        (
                                            ...
                                            WHERE us.UsrEmail=@Mail order by cmo.ModOrder FOR XML PATH(''), TYPE
                                        ...
                                        (
                                            SELECT STUFF(
                                        (
                                            SELECT ',{"IdUser":"' + CONVERT(VARCHAR, us.UsrIdUser) + '",' + '"UserName":"' + us.UsrNickName + '",' + '"TacName":"Comercio Afiliado",' + '"RolName":"' + ro.RolName + '"' + '}'
                                            FROM DeliveryBackOffice.dbo.RegisterUser us WITH(NOLOCK)
												...
												WHERE us.UsrEmail = @Mail FOR XML PATH(''), TYPE
                                        ).value('.', 'varchar(max)'), 1, 1, '')
                                        );
							  ...
							  BEGIN
                               ...
                               SET @JsonProfileEXP = (SELECT STUFF((
		   							SELECT DISTINCT
											',{"Name":"' + ISNULL( AFF.AffiliateName ,'') + '",' +
											'"ContactName":"' + isnull( RTRIM(LTRIM(CONCAT(AFF.AffiliateContactName,' ', ''))) , '')  + '",' +	  	  
											'"ContactPhone":"' + isnull(AFF.AffiliateContactPhone, '')  + '",' +
											'"Contactemail":"' + isnull(AFF.AffiliateContactEmail, '')  + '"}' 
					...
					INNER JOIN RegisterUserByAffiliate RUA WITH(NOLOCK)
					 ON RUA.RegisterUserId = ru.UsrIdUser
					INNER JOIN DeliveryBackOffice.dbo.Affiliate AFF WITH(NOLOCK)
					ON AFF.IdAffiliate = RUA.AffiliateId
			         WHERE ru.UsrEmail = @Mail
...
END
            ...
            END;
            ELSE -- usuario o contraseña inválido
            BEGIN
