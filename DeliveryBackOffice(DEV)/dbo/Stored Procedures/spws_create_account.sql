...
BEGIN
	...
	DECLARE @IdCustomer as INT        --IdCustomer que se inserta en la tabla dbo.Customer
		...
		IF NOT EXISTS(SELECT 1 FROM [DeliveryBackOffice].[dbo].[Customer] WHERE RegexEmail LIKE '%' + @Email + '%' AND IdCustomerType = 1 AND RowSatus = 1)
		BEGIN
		...
		BEGIN
			set @jsonResult =(
						SELECT STUFF(( 
						SELECT '{"IdResult":' + convert(varchar,IdResult)    +',' 
						+ '"Message":"' + Message + '"}' from #errormessage where Id ='Corp'
