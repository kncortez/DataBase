...
BEGIN
    ...
    DECLARE @UpdatedAddress AS TABLE (IdUpdated BIGINT);
    ...
    BEGIN
        ...
        (
            ...
            WHERE @ProvinceTownship LIKE '%' + Twn.TownshipName + '%' COLLATE Latin1_General_CI_AI
        );
    ...
    BEGIN TRY
        ...
        if
        (
            select count(RuaIdAccount) from #Access
        ) > 0 -- el usuario tiene acceso  a la cuenta indicada
        begin

            select uad.UadIdAddress
            into #Address
            from dbo.UserAddress uad
            where uad.UadIdAddress = @IdAddress
                  and uad.UadIdAccount = @IdAccount
            ...
            SELECT @Department = ProvinceName,
            ...
            begin
                ...
                begin
                    ...
                    (
                        SELECT STUFF(
                                        (
                                            SELECT '{"IdResult":' + convert(varchar, IdResult) + ',' + '"IdAddress":'
                                                   + convert(varchar, @IdAddress) + ',' + '"Message":"' + Message
                                                   + '"}'
            ...
            begin
                SET @CodeOfReference =
                (
                    SELECT MAX(CodeOfReference) + 1 FROM VisitPointClient
                )
                ...
                (
                    SELECT STUFF(
                                    (
                                        SELECT '{"IdResult":' + convert(varchar, IdResult) + ',' + '"IdAddress":'
                                               + convert(varchar, @IdAddress) + ',' + '"CodeOfReference":'
                                               + convert(varchar, @CodeOfReference) + ',' + '"Province":"'
                                               + convert(varchar, @Department) + '",' + '"Township":"'
                                               + convert(varchar, @TownshipName) + '",' + '"HeaderCode":"'
                                               + convert(varchar, @HeaderCode) + '",' + '"CityPlace":"'
                                               + convert(varchar, @CityName) + '",' + '"IdProvince":"'
                                               + convert(varchar, @IdDepartment) + '",' + '"Message":"' + Message
                                               + '"}'
                                        from @ResponseMessages
                                        where Id = 'Insert'
                                        FOR XML PATH(''), TYPE
            ...
            begin
                ...
                begin
                    ...
                    SET [NirPhone] = @NirPhone,
                        ...
                        [Zone] = @Zone
                    OUTPUT inserted.IdConfirmedAddress
                    INTO @UpdatedAddress
                    (
                        IdUpdated
                    )
                    ...
                    (
                        SELECT TOP 1
                            1
                        FROM [DeliveryBackOffice].[dbo].[ConfirmedAddressByCustomer] CABC WITH (NOLOCK)
                            INNER JOIN @UpdatedAddress UA
                                ON CABC.ConfirmedAddressId = UA.IdUpdated
                        WHERE CABC.CustomerId = @IdCustomer
                              AND CABC.RowStatus = 1
                    )
                       )
                    BEGIN
                ...
                begin
                    ...
                    (   @NirPhone,
                        ...
                        (
                            select top 1
                                IdStatus
                            from dbo.CatStateConfirmedAddress
                            where NameState = 'CONFIRMADO'
                        )
                    );
    ...
    IF OBJECT_ID('tempdb.dbo.#Address', 'U') IS NOT NULL
        DROP TABLE #Address;
    IF OBJECT_ID('tempdb.dbo.#Access', 'U') IS NOT NULL
        DROP TABLE #Access;
