	...
	BEGIN TRY
		...
		begin
			...
			where uad.UadIdAddress =  @IdAddress and uad.UadIdAccount = @IdAccount
				...
				Select @HeaderCode=HeaderCode, @TownshipName=TownshipName from dbo.Township with(nolock);
				SET @CityName=(SELECT CityPlace FROM DBO.CatCityPlace with(nolock) WHERE IdCityPlace=@IdCityPlace)

				--Select count(Predeterminated) from VisitPointClient where CustomerID = 7757 AND Predeterminated = 1
			    
				if((@Predeterminated = 1) AND ((Select count(Predeterminated) from VisitPointClient where CustomerID = @IdCustomer AND Predeterminated = 1) >=1))
				  begin
					Declare @IdVP Int;
					SET @IdVP = (SELECT IdVisitPointClient FROM VisitPointClient WHERE CustomerID = @IdCustomer AND Predeterminated = 1)
					UPDATE VisitPointClient
						SET Predeterminated = 0
						WHERE IdVisitPointClient = @IdVP
				  end
				...
				begin 
					...
					UPDATE VP
					   SET VP.IdTownship = @IdTownship
						  ...
						  ,VP.IdPopulated = @IdPopulated
						  ,VP.Predeterminated = @Predeterminated
						  ,VP.VisibleInGuide = @VisibleInGuide
						  ,VP.PickupsProgram = @PickupsProgram
					FROM [dbo].[UserAddress] UADD LEFT JOIN [dbo].[VisitPointClient] VP with(nolock)
						ON UADD.CodeOfReference=VP.CodeOfReference
					 WHERE [UadIdAddress] =  @IdAddress
