	...
	BEGIN 
		...
		BEGIN
			SELECT  
				...
				ISNULL(CONVERT(varchar,TBL.DateCreated,103),'') AS	[RealDeliveryDate],
			...
			WHERE vpclient.VisitPointId = @IdVisitPoint
				...
				AND serv.DateCreated >= @DateIni AND serv.DateCreated < DATEADD(DAY, 1, @DateFin)
