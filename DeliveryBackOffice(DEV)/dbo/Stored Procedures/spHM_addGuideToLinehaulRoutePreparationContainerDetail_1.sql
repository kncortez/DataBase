...
BEGIN
	...
	DECLARE @ValidateContainer AS INT				-- 

	
	

	SELECT @CONTAINER_HUB=COALESCE([LRPC].[HubDestinyId], 0) 
	, @ValidateContainer = ctc.AllowStopOver
						  FROM	[dbo].[LinehaulRoutePreparationContainer] LRPC
							INNER JOIN dbo.Container ct WITH(NOLOCK) ON ct.IdContainer = LRPC.ContainerId
							INNER JOIN dbo.CatTypeContainer ctc WITH(NOLOCK) ON ctc.IdCatTypeContainer = ct.CatTypeContainerId
...
BEGIN
	...
	SET @EXISTING_LRPCD = (SELECT COUNT([LRPD].[IdLinehaulRoutePreparationContainerDetail])
						FROM	[dbo].[LinehaulRoutePreparationContainerDetail] LRPD WITH(NOLOCK)
