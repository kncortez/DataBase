...
-- =============================================
CREATE PROCEDURE [dbo].[spg_report_DeliveryBySender_bnhl]
...
and Att.ID_Courier = 6--31

--select * from DeliveryBackOffice.dbo.DeliveryAttempt
--where Guide_Number = 71323 and Id_Courier = 6

select * from #ListAttempt
order by 1 desc
...
,CONVERT(DECIMAL(10,2),CAST(sum(Accepted) AS DECIMAL)/CAST(count(1) AS DECIMAL)*100) CompliancePercentage
