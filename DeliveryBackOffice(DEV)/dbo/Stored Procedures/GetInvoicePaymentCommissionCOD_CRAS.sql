...
-- =============================================
CREATE PROCEDURE [dbo].[GetInvoicePaymentCommissionCOD_CRAS]
AS
BEGIN
    BEGIN TRY
        ...
        CREATE CLUSTERED INDEX ix_GuidesCommission
        ON #GuidesCommission (
                                 [GuideSerie]
                               , [GuideNumber]
                             );
        ...
        CREATE CLUSTERED INDEX ix_InvoicePaymentCommissionCOD
        ON #InvoicePaymentCommissionCOD ([IdCustomer]);
        ...
        BEGIN
            ...
            SELECT ISNULL(cbv.NameBillingVolume, @NameVolumeBillingDefault)                              NameBillingVolume
                 ...
                 , gc.Amount                                                                             Amount
                 , CASE
                       WHEN cu.TaxIdentificationNumber IS NULL
                            OR cu.TaxIdentificationNumber = '' THEN
                           bp.[Name]
                       ELSE
                           cu.[Name]
                   END                                                                                   [Name]
            ...
            WHERE (
                      (cbt.DescriptionBillingTime = 'Diario')
                      OR
                      (
                          cbt.DescriptionBillingTime = 'Fecha de corte'
                          AND
                          (
                              DATEPART(dd, ISNULL(vpcon.BillingCut_offDate, cu.BillingCut_offDate)) = DATEPART(
                                                                                                                  dd
                                                                                                                , GETDATE()
                                                                                                              )
                              OR DATEPART(dd, EOMONTH(GETDATE())) = DATEPART(dd, GETDATE())
                                 AND DATEPART(dd, EOMONTH(GETDATE())) < DATEPART(
                                                                                    dd
                                                                                  , ISNULL(
                                                                                              vpcon.BillingCut_offDate
                                                                                            , cu.BillingCut_offDate
                                                                                          )
                                                                                )
                          )
                      )
...
,33070
            ...
            SELECT NameBillingVolume  NameBillingVolume
                 ...
                 , CASE
                   ...
                   END                [Name]
                 , CASE
                       WHEN ipcCOD.[Address] IS NULL
                            OR ipcCOD.[Address] = '' THEN
                           'CIUDAD'
                       ELSE
                           ipcCOD.[Address]
                   END                [Address]
            ...
            DECLARE @Retries INT
                = ISNULL((
                             SELECT ConfEP.ConfigParameterValue
            ...
            SELECT inv_pk_id Invoice
            FROM invoiceHeader WITH (NOLOCK)
            WHERE CatInvoiceTypeId = @IdCatInvoiceType
                  AND
                  (
                      inv_descriptionFEL IS NULL
