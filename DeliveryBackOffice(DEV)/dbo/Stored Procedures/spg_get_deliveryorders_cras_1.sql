...
-- =============================================
CREATE PROCEDURE [dbo].[spg_get_deliveryorders_cras]
...
BEGIN
    ...
    BEGIN
        ...
        IF (@GuideNumber = 0)
        BEGIN
            SELECT CAST(serv.Sender_ID AS VARCHAR) + ' - ' + UPPER(ISNULL(serv.Sender_FirstName, '')) + ' '
                   + UPPER(ISNULL(serv.Sender_LastName, ''))                   [NameOfSender]
                 , ISNULL(
                             UPPER(ISNULL(serv.Receiver_FirstName, '')) + ' '
                             + UPPER(ISNULL(serv.Receiver_LastName, ''))
                           , ''
                         )                                                     [NameOfReceiver]
