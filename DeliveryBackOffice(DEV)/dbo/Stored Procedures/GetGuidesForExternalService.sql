...
-- =============================================
CREATE PROCEDURE [dbo].[GetGuidesForExternalService]
    @Date DATETIME = NULL,
    @Hub NVARCHAR(5) = 'GUA',
...
BEGIN
	...
	-- Devuelve la fecha del día con las horas 00:00:00
    ...
    BEGIN TRY
        ...
        SET RowStatus = 0,
            ...
            DateUpdated = GETDATE()
        FROM [DeliveryBackOffice].[dbo].[ExternalPlatformServiceLog] EPSL WITH(NOLOCK)
        ...
        FROM DeliveryBackOffice.dbo.DeliveryOrder DOR WITH (NOLOCK)
            JOIN (
				SELECT 
					...
					DOD.Guide_Number,
					DOD.StatusOrderId,
					MAX(DOD.DateCreatedInSystem) 'DateCreatedInSystem'
				FROM
            ...
            LEFT JOIN [DeliveryBackOffice].[dbo].[ExternalPlatformServiceLog] EPSL WITH(NOLOCK)
                ON DOR.Guide_Serie = EPSL.GuideSerie
			...
			LEFT JOIN
				...
				ON
					...
					SZCZero.RowStatus = 1
        WHERE 
			  DOR.StatusOrderId = @Status -- Permanece en estado correcto
              AND 
			  EPSL.IdExternalPlatformServiceLog IS NULL -- Que no este siendo procesada
              AND
			  DOR.Guide_Number NOT IN -- No haya sido exitosamente trasladada a la plataforma externa
                (
                    SELECT 
						EXPSRG.GuideNumber
                    FROM 
						DeliveryBackOffice.dbo.ExtPlatServiceRelationshipWithGuide EXPSRG WITH(NOLOCK)
                    WHERE 
						EXPSRG.RowStatus = 1
						AND 
						CAST(EXPSRG.DateCreated AS DATE) = CAST(@Date AS DATE)
                )
