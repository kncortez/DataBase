...
BEGIN
   ...
   SELECT  DISTINCT
           ...
           SUM(IIF([DO].IsLastMileReturn  = 1 , 1, 0)) Returned_Guides,
           SUM(IIF([DO].IsLastMileReturn  = 1 , (DO.Pieces_Dry + DO.Pieces_Cold), 0)) Returned_Pieces,
           ...
           SUM(IIF(DODtransfer.TransferExists = 1 AND DSD.RowStatus = 1, 1, 0)) Transfer_Guides_Checkpoint,
           SUM(IIF(DODtransfer.TransferExists = 1 AND DSD.RowStatus = 1, (DO.Pieces_Dry + DO.Pieces_Cold), 0)) Transfer_Pieces_Checkpoint,
    ...
    (
        ...
        WHERE  [DOD].[Guide_Serie] = [DSD].[Guide_Serie]
			  ...
			  AND [DOD].StatusOrderId NOT IN (@STATUS_TRANSFERED_EX_ID,@STATUS_RETURNED_ID)
              AND [DOD].[RowStatus] = 1
	...
	WHERE [DOBS].[Date_Dispatched] BETWEEN @StartDateTime AND   @EndDateTime 
	   GROUP BY [DOBS].[ID],
             [DOBS].[Date_Dispatched],
             [SRE].[CUI],
             [SRE].[First_Name],
