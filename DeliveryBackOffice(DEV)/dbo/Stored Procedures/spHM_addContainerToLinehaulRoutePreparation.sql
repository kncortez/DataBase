...
BEGIN
    ...
    SET @EXISTING_LRP =
    (
        SELECT COUNT([LRP].[IdLinehaulRoutePreparation]) AS CONT
    ...
    BEGIN
        SELECT 0 [spResult],
               'Manifiesto de despacho NO existe' [spMessage];
        RETURN;
    END;

    SET @EXISTING_C =
    (
        SELECT COUNT([C].[IdContainer]) AS CONT
    ...
    BEGIN
        SELECT 0 [spResult],
               'Contenedor ingresado NO existe' [spMessage];
        RETURN;
    END;

    SET @GENERATED_STATUS_ID =
    (
    ...
    SET @EXISTING_LPC =
    (
        SELECT COUNT([LPC].[IdLinehaulRoutePreparationContainer]) AS CONT
    ...
    BEGIN
        ...
        SET @EXISTING_LPC = 0;
        SELECT 0 [spResult],
               'Contenedor se encuentra activo en otro proceso.' AS [spMessage];
        RETURN;
    END;

    SET @CONTAINER_STOPOVER =
    (
        SELECT COUNT([LPC].[IdLinehaulRoutePreparationContainer]) AS CONT
        FROM [dbo].[LinehaulRoutePreparationContainer] LPC
    ...
    BEGIN
        ...
        SET @CONTAINER_STOPOVER_HUB =
        (
            SELECT [LPC].[HubDestinyId]
            FROM [dbo].[LinehaulRoutePreparationContainer] LPC
            WHERE [LPC].[CatLinehaulStatusId] = @STOPOVER_STATUS_ID
                  AND [LPC].[ContainerId] = @ContainerId
                  AND [LPC].[LinehaulRoutePreparationId] != @LinehaulRoutePreparationId
        ...
        SET @IS_LINEHAUL_COVERAGE =
        (
            SELECT COUNT([LC].[IdLinehaulCoverage]) AS CONT
        ...
        BEGIN
            SELECT 0 [spResult],
                   'Contenedor escaneado se encuentra en ESCALA, el destino NO está cubierto por la ruta actual' [spMessage];
            RETURN;
        END;

        SELECT 1 [spResult],
               'Contenedor se encuentra en ESCALA' [spMessage];
        RETURN;
    END;
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            VALUES
            (   @LinehaulRoutePreparationId, @ContainerId, @GENERATED_STATUS_ID, 0, -- GuideQuantity
                0,                                                                  -- DryPieceQuantity
                0,                                                                  -- ColdPieceQuantity
                1,                                                                  -- RowStatus
                @TknUser, SYSDATETIME());
