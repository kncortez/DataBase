        ...
        BEGIN
            ...
            (
                ...
                WHERE ddd.Guide_Serie = ord.Guide_Serie
                      ...
                      AND CONVERT(DATE, ddd.DateCreatedInSystem) = CONVERT(DATE, getdate()-2)
            ...
            WHERE CONVERT(DATE, ds.Date_Dispatched) = CONVERT(DATE, getdate()-2)
            ...
            FROM dbo.DeliveryOrder                            ord WITH (NOLOCK)
                ...
                INNER JOIN dbo.DeliveryOrderDetail            ddd WITH (NOLOCK)
                    ON ddd.Guide_Serie = ord.Guide_Serie
                       ...
                       AND ddd.StatusOrderId = 45
                       AND CONVERT(DATE, ddd.DateCreatedInSystem) = CONVERT(DATE, getdate()-2)
                       AND ddd.SystemOrigin in( 2,5) --desktop					
                LEFT JOIN DenariusUser_Dev.dbo.LGN_LogByToken tk WITH (NOLOCK)
                    ON tk.SSN_IdToken = CONVERT(VARCHAR(50), ddd.UserCreated) --ddd.UserCreated
            ...
            (
                ...
                LEFT JOIN dbo.CatIncidenceClasification cic WITH (NOLOCK)
                    ON cic.IdCatIncidenceClasification = cti.IncidenceClasificationId
            WHERE CONVERT(DATE, ddd.DateCreatedInSystem) = CONVERT(DATE, getdate()-2)
                  AND ddd.SystemOrigin in( 2,5) -- desktop
                  AND HUbs.IdHubLogistic IN
                      (
                          SELECT IdHubLogistics FROM @TblHubLogistic
                ...
                SELECT IIF(ord.StatusOrderId = 4, 1, 0)                                                 Pending
                     ...
                     , (CASE
                            WHEN IncidenceTbl.SSN_IdUser IS NULL THEN
                                'Vendedor Rutero'
							WHEN IncidenceTbl.SSN_IdUser IS NULL AND IncidenceTbl.SSN_Username IS NULL THEN 'Expres Center'
                ...
                SELECT --TOP 1000
                  ...
                  , (CASE
                         WHEN tk.SSN_IdUser IS NULL THEN
                             'Vendedor Rutero'
						 WHEN tk.SSN_IdUser IS NULL AND tk.SSN_Username IS NULL THEN 
						     'Expres Center' 
                         ELSE
                             'Usuario Desktop'
                     END
            ...
            (
                SELECT --TOP 1000
                  ...
                  , (CASE
                         WHEN IncidenceTbl.SSN_IdUser IS NULL THEN
                             'Vendedor Rutero'
					     WHEN IncidenceTbl.SSN_IdUser IS NULL AND IncidenceTbl.SSN_Username IS NULL THEN 
						     'Expres Center'
                         ELSE
                             'Usuario Desktop'
                     END
