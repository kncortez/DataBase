...
BEGIN
	...
	BEGIN TRY			
		...
		DECLARE @TempGuidesToSettled TABLE (
			GuideSerie NVARCHAR(2),
			GuideNumber INT,
...
--LISTANDO ENTREGAS
			...
			WHERE 
				...
				AND(
					...
					DO.StatusOrderId IN (@ORDERSTATUS_DELIVERED,@ORDERSTATUS_RETURNED,@STATUSFAILED_DO, @STATUSINCIDENCE)
					OR TransferGuide.IsTraslate = 1
				)
				AND RA.DateOfRoute =@Date
			--LISTANDO RECOLECCIONES

			INSERT INTO @TempGuidesToSettled
				(GuideSerie, GuideNumber, GuideFlow)
			SELECT
				GTS.GuideSerie
				,GTS.GuideNumber
				,(CASE 
			...
			INNER JOIN DeliveryOrder do WITH (NOLOCK)
				ON GTS.GuideSerie = DO.Guide_Serie
			...
			UPDATE
				GTS
			SET
				GTS.IsDelivered = (CASE WHEN TGTS.GuideFlow IS NOT NULL AND TGTS.GuideFlow = 'IsBazar' THEN 0 ELSE GTS.IsDelivered END)
				,GTS.IsReturn = (CASE WHEN TGTS.GuideFlow IS NOT NULL AND TGTS.GuideFlow = 'IsReturn' THEN 1 ELSE 0 END)
				,GTS.IsArrival = (CASE WHEN TGTS.GuideFlow IS NOT NULL AND TGTS.GuideFlow = 'IsArrival' THEN 1 ELSE 0 END)
				,GTS.IsTransfer = (CASE WHEN TGTS.GuideFlow IS NOT NULL AND TGTS.GuideFlow = 'IsBazar' THEN 0 ELSE GTS.IsTransfer END)
			FROM
				@GuidesToSettled GTS
				LEFT JOIN
					@TempGuidesToSettled TGTS
					ON
						GTS.GuideSerie = TGTS.GuideSerie
						AND
						GTS.GuideNumber = TGTS.GuideNumber
			...
			SELECT
				...
				0,
				NULL,
				MAX(IIF( ISNULL(GTS.IsArrival,0) = 1, 1, 0)),
				MAX(IIF( ISNULL(GTS.IsReturn,0) = 1, 1, 0)),
				MAX(IIF( ISNULL(GTS.IsDelivered,0) = 1, 1, 0 )),
			...
			FROM @GuidesToSettled GTS
				...
				INNER JOIN DBO.UnifiedRouteSettlementDetail URSD ON
					...
					AND URSD.GuideSerie = GTS.GuideSerie
