...
BEGIN
    ...
    SET @idType =
    (
        SELECT IdCatInvoiceType FROM CatInvoiceType WHERE Name = 'Otros'
    )
    ...
    SELECT @CustomerType = ISNULL(Cu.IdCustomerType, 0)
    ...
    BEGIN TRY
        ...
        (
                  ...
                  AND res.UstStatus = 'ACTIVE'
                       ...
                       (
                           ...
                           FROM [dbo].RegisterUser usr WITH (NOLOCK)
                               ...
                               INNER JOIN [dbo].Account ac WITH (NOLOCK)
                                   ON ac.AccIdAccount = rua.RuaIdAccount
                           WHERE rus.RusIdSystem = 1
                                 AND ac.AccRowStatus = 1
                                 AND usr.UsrEmail = RTP.InvoiceEmail
                       ) = 'ACTIVE'
                       AND RTP.ProductGiftShippingEmail IS NULL THEN
                           1
                       ELSE
            ...
            BEGIN
                ...
                VALUES
                (1, @OrderNumber, 2, @inv_amount, GETDATE(), 1, 1, @Token, GETDATE(), NULL, NULL);
        ...
        SELECT TOP 1
            ...
            @inv_IVA
                = SUM(ISNULL((CS.SubscriptionCost - ISNULL(CS.SubscriptionFixedValue, 0)), CM.MembershipCost)
                      - (ISNULL((CS.SubscriptionCost - ISNULL(CS.SubscriptionFixedValue, 0)), CM.MembershipCost) / @IVA)
                     )
        ...
        SELECT @dti_fk_header,
               ...
               CS.SubscriptionName,
               (CS.SubscriptionCost - ISNULL(CS.SubscriptionFixedValue, 0))
               - ((CS.SubscriptionCost - ISNULL(CS.SubscriptionFixedValue, 0)) / @IVA),
               (CS.SubscriptionCost - ISNULL(CS.SubscriptionFixedValue, 0)),
               @dti_dateRegister,
               @dti_tokenRegister,
               @SAPCode,
        ...
        (
            ...
            where RegisterUserId = @IdAccount
                  AND IdCountry = @IdCountry
                  AND RowStatus = 1
            ORDER BY DateCreated DESC
        );
