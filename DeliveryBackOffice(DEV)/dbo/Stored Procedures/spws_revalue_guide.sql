...
------ Carga inicial de datos ----------------------------------------------------------------------------
	 ...
	 IF(@UseMembership = 1)
		BEGIN
		   ...
		   Autor: Jerson Ochoa 30-12-2022 */
			...
			SELECT	TOP 1	@MembershipSubscriptionLogId = IdMembershipSubscriptionLog
			...
			IF @MembershipSubscriptionLogId IS NOT NULL
				BEGIN
					IF @SubscriptionId IS NULL
						BEGIN
							SELECT	@ServiceValue = IIF(@ServiceAppliedCount <= ms.MembershipMaxServiceFixedValue, ms.MembershipFixedValue, -1)
							FROM	Membership ms
							...
							ELSE
								BEGIN
									...
									SELECT		TOP 1 @DiscountValue = DiscountValue
									...
									WHERE		mdr.MembershipId = @MembershipId
										AND		((@ServiceAppliedCount BETWEEN mdr.DiscountLowServiceRange AND mdr.DiscountTopServiceRange)
					...
					ELSE
						BEGIN
							...
							SELECT	TOP 1 @ServiceValueSubscription = IIF(@ServiceAppliedCount <= sc.SubscriptionMaxServiceFixedValue,sc.SubscriptionFixedValue,-1)
							...
							ELSE
								BEGIN
									...
									WHERE		sdr.SubscriptionId = @SubscriptionId
										AND		((@ServiceAppliedCount BETWEEN sdr.DiscountLowServiceRange AND sdr.DiscountTopServiceRange)
			...
			ELSE
				BEGIN
					...
					SELECT	TOP 1 @MembershipId = ms.IdMembership
					...
					IF @MembershipId IS NOT NULL AND @PriceShippment IS NOT NULL AND @PriceShippment > 0
						BEGIN
							...
							ELSE
								BEGIN
									...
									SELECT		TOP 1 @SubscriptionId = sc.IdSubscription
													...
													IF @DiscountValue IS NOT NULL
														BEGIN
															IF @Type = 'Porcentaje'
																BEGIN
																	SET @DiscountMembership = @PriceShippment * (@DiscountValue/100)
																	SET @DecriptionDiscount = CONCAT('Por rango de servicios suscripción, porcentaje ',@DiscountValue)
																	SET @ServiceAppliedType = 2
																END
															ELSE IF @Type = 'Monto'
																BEGIN
																	SET @DiscountMembership = @DiscountValue
																	SET @DecriptionDiscount = CONCAT('Por rango de servicios suscripción, monto ',@DiscountValue)
																	SET @ServiceAppliedType = 2
																END
															ELSE IF @Type = 'Servicio'
																BEGIN
																	SET @DiscountMembership = @PriceShippment
																	SET @DecriptionDiscount = CONCAT('Por rango de servicios suscripción, servicio ',@PriceShippment)
																	SET @ServiceAppliedType = 2
																END
									...
									IF @SubscriptionId IS NULL OR @ServiceAppliedType IS NULL
										BEGIN
											...
											IF @DiscountValue IS NOT NULL
												BEGIN
													...
													IF @Type = 'Porcentaje'
														BEGIN
															SET @DiscountMembership = @PriceShippment * (@DiscountValue/100)
															SET @DecriptionDiscount = CONCAT('Por rango de servicios membresía, porcentaje ',@DiscountValue)
															SET @ServiceAppliedType = 1
														END
													ELSE IF @Type = 'Monto'
														BEGIN
															SET @DiscountMembership = @DiscountValue
															SET @DecriptionDiscount = CONCAT('Por rango de servicios membresía, monto ',@DiscountValue)
															SET @ServiceAppliedType = 1
														END
													ELSE IF @Type = 'Servicio'
														BEGIN
															SET @DiscountMembership = @PriceShippment
															SET @DecriptionDiscount = CONCAT('Por rango de servicios membresía, servicio ',@PriceShippment)
															SET @ServiceAppliedType = 1
														END
								...
								BEGIN
									...
									IF @SetUpdate = 'true'
										BEGIN
											...
											SET @DecriptionDiscount = CONCAT('Se aplicó descuento de membresía y suscripción (',@DecriptionDiscount,')')
											--Insertar log
	...
	/* Termina membresías y suscripciones */
	  ...
	  begin
		...
		select 
			@NewPrice = isnull(tr.Price,0) + isnull(tr.FragilRate,0) + isnull(tr.CollectedRate,0) + isnull(tr.InsuranceRate,0) + isnull(tr.OverWeightRate,0) + isnull(tr.CreditCardRate,0)
...
print 'existe'
    ...
    FROM @TblCost det
        LEFT JOIN dbo.BreakdownOfPayment bk
            ON  bk.IdCost = @IdCost AND bk.Description = det.Description COLLATE Latin1_General_CI_AI
		 ...
		 set @jsonResult =( SELECT STUFF(( 
					...
					'"ServiceShortName":"' + isnull(tr.Service,'') + '",' +
						...
						iif((ABS(isnull(tr.Discount,0)))>0, 
							...
							'"Price":"' + CONVERT(varchar, CAST((dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' ,(tr.Discount),'false')  ) AS decimal(18, 2)) ) + '",'  +
							'"Currency":"' + COALESCE(tr.Currency,'') + '"' +
							'}' 
						,' '  ) +
