...
CREATE PROCEDURE [dbo].[SetFinishPickUpBatch_bnhl2]
...
BEGIN
    ...
    END CATCH;
        ...
        --  [Message]
        --)
        EXEC [dbo].[spws_get_validate_guides_pickup_bnhl2]
             @InGuides = @InGuides,
             @IdPickup = @IdPickup,
             @Token = @Token;

        PRINT 'sale entonces del picukp'
        SELECT * FROM #Temp 
