		...
		BEGIN
			...
			BEGIN
				...
				SELECT
					...
					,ABC.Code 'ParcelCode'
					,w.Item 'ParcelWeight'
					,CAST(0 AS INT) 'ParcelOverweight'
				...
				-- Revisar sobrepesos de paquetes
				UPDATE
					POPT
				SET
					POPT.ParcelOverweight = (
						CASE
							WHEN POPT.ParcelWeight > ABC.MassWeight THEN (POPT.ParcelWeight - ABC.MassWeight)
							ELSE 0
						END
					)
				FROM
					#ParcelOverweightPerType POPT
					INNER JOIN
						[DeliveryBackOffice].[dbo].[ArticleByCustomer] ABC WITH(NOLOCK)
						ON
							POPT.ParcelCode = ABC.Code COLLATE Latin1_General_CI_AI
							AND
							ABC.AbcRowStatus = 1

				SET @OverWeight = ISNULL((SELECT SUM(POPT.ParcelOverweight) FROM #ParcelOverweightPerType POPT),0);
