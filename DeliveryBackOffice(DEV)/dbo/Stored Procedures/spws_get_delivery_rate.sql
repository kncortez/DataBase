...
BEGIN
	...
	--DECLARE @UseMembership AS bit=0
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SELECT
					@RateId = ISNULL(ISNULL(SC.AlternativeRateHeaderId, CS.AlternativeRateHeaderId), @IdRate)
				FROM
					[DeliveryBackOffice].[dbo].[Subscription] SC WITH(NOLOCK)
					INNER JOIN
						[DeliveryBackOffice].[dbo].[CatSubscription] CS WITH(NOLOCK)
						ON
							SC.CatSubscriptionId = CS.IdCatSubscription
				WHERE
					SC.IdSubscription = @CustomerHasActiveSubscription
			...
			BEGIN
				...
				-- Si falla en encontrar tarifa "valida", defecto la tarifa actual
				SELECT
					@RateId = ISNULL(ISNULL(SC.RateHeaderId, CS.RateHeaderId), @IdRate)
				FROM
					[DeliveryBackOffice].[dbo].[Subscription] SC WITH(NOLOCK)
					INNER JOIN
						[DeliveryBackOffice].[dbo].[CatSubscription] CS WITH(NOLOCK)
...
------------------------------------- verificar tarifas de piezas irregulares ---------------------------------------------------
		...
		IF(@IdRate IN (
			@NewMainRates
			,@NewAlternativeRates
			,@NewAutoSalesMainRates
			,@TarifaPlanBasico
			,@TarifaPlanBasicoPlus
			,@TarifaPlanGold
			,@TarifaPlanCorporativo
			,@TarifaPlanBasicoAlt
			,@TarifaPlanBasicoPlusAlt
			,@TarifaPlanGoldAlt
			,@TarifaPlanCorporativoAlt
		))
		BEGIN
	...
	IF @CalculateMembership = 'true'
		BEGIN
			...
			DECLARE @i INT = 0
			DECLARE @total INT = ISNULL(( SELECT MAX(Id) FROM @TempRate), 0);
			...
			IF @MembershipId IS NOT NULL
				BEGIN
					...
					SELECT TOP 1 @DiscountValue2 = DiscountValue
										...
										IF (@ServiceValue = 0)
											BEGIN
												SELECT	@Discount = (@PriceShippment + tr.CreditCardRate),
														@NewPriceShippment = (tr.FragilRate + tr.CollectedRate + tr.InsuranceRate + tr.OverWeightRate)
												FROM	@TempRate tr
												WHERE	Id = @i;
												...
												IF @ServiceValueSubscription >= 0
													...
													ELSE 
														BEGIN
															...
															SELECT	@NewPriceShippment = (@ServiceValueSubscription + tr.FragilRate + tr.CollectedRate + tr.InsuranceRate + tr.CreditCardRate + tr.OverWeightRate)
															FROM	@TempRate tr
															WHERE	Id = @i;
														END
												ELSE
													BEGIN
														IF @DiscountValue IS NOT NULL
															BEGIN
																...
																SELECT	@NewPriceShippment = (@PriceShippment - @Discount) + (tr.FragilRate + tr.CollectedRate + tr.InsuranceRate + tr.CreditCardRate + tr.OverWeightRate)
																FROM	@TempRate tr
																WHERE	Id = @i;
	...
	If @FormatResponse ='Json'
		...
		IF(@IdRate IN (
		 ...
		 set @jsonResult =( SELECT STUFF(( 
					select 
						...
						'"UseMembership":"' + Convert(varchar(1),@CalculateMembership) + '",' +
		...
		begin
			...
			select 
				...
				,tr.Segment
				,tr.Service
				, IIF(@PriceWithCreditCard = 1, (tr.BaseRate -  tr.Discount  + tr.IrregularPieceRate + tr.CreditCardRate), (tr.BaseRate -  tr.Discount  + tr.IrregularPieceRate)) as Price--  + tr.FragilRate + tr.CollectedRate + tr.InsuranceRate  +tr.CreditCardRate + tr.OverWeightRate  + tr.IrregularPieceRate ) as Price
