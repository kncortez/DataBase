...
-- =============================================
CREATE PROCEDURE [dbo].[SetCorporateUser]
    @Code BIGINT
  , @UserName VARCHAR(50)
  , @Email NVARCHAR(200)
...
BEGIN
	...
	UNION
	SELECT  500 AS IdResult
		,'El correo electrónico ya existe en el sistema'  AS Message
		,'ExistMail' as Id 
	UNION
	SELECT  500 AS IdResult
	...
	WHERE cu.DefaultPerCountry = 1
		AND cuCOD.RowStatus = 1
		AND cu.Currency_Status = 1
	...
	WHERE RowStatus = 1
		AND IdUser = @Code OR Username = @UserName;

	SELECT 
		@MailMatch = CASE WHEN ru.UsrEmail = @Email THEN 1 ELSE 0 END
	FROM DeliveryBackOffice.dbo.RegisterUser ru WITH (NOLOCK)
	WHERE ru.UsrRowStatus = 1
		AND ru.UsrEmail = @Email;
	...
	BEGIN
		SET @jsonResult =(
				SELECT STUFF(( 
				SELECT '{"IdResult":' + convert(varchar,IdResult)    +',' 
				+ '"Message":"' + Message  + '"}' from #errormessage_SCU where Id ='ExistMail'
