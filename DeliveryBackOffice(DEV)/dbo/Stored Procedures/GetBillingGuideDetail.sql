...
BEGIN
	...
	SET @CostId = (SELECT TOP 1 Co.IdCost FROM [DeliveryBackOffice].[dbo].[Cost] Co WHERE Co.ProductNumber = @ProductNumber ORDER BY IdCost DESC);
	...
	SELECT	TOP 1 @AppliedCoupon = ISNULL(BOP.PromoCouponId,0)
	...
	SELECT		@AppliedMembership = 1, 
				...
				@IsFixedValueDiscount = (CASE
											 WHEN [MSL].[LogServiceNumber] <= [M].[MembershipMaxServiceFixedValue] 
											 THEN 1 
											 ELSE 0
										 END),
				@Amount =				(CASE
											WHEN [MSL].[LogServiceNumber] <= [M].[MembershipMaxServiceFixedValue]
											THEN [MSL].[LogGuideNewValue]
											ELSE [MSL].[LogGuideOriginalValue]
										END),
				@ValueType =			(CASE 
											WHEN [MSL].[LogServiceNumber] <= [M].[MembershipMaxServiceFixedValue]
											THEN 'Porcentaje'
											ELSE (
													SELECT		TOP 1 ISNULL(CVT.ValueTypeName, 'Porcentaje')
													...
													WHERE		MDR.MembershipId = [M].[IdMembership]
														AND		MSL.LogServiceNumber BETWEEN MDR.DiscountLowServiceRange AND ISNULL(MDR.DiscountTopServiceRange, 1000000000)
												)
										END),
				@PromoValue =			(CASE 
											WHEN [MSL].[LogServiceNumber] <= [M].[MembershipMaxServiceFixedValue]
											THEN 0
											ELSE (
													SELECT	TOP 1 ISNULL(MDR.DiscountValue, 0)
													FROM	[DeliveryBackOffice].[dbo].[MembershipDiscountRange] MDR WITH(NOLOCK)
													WHERE	MDR.MembershipId = [M].[IdMembership]
														AND	MSL.LogServiceNumber BETWEEN MDR.DiscountLowServiceRange AND ISNULL(MDR.DiscountTopServiceRange, 1000000000)
												)
	...
	SELECT		@AppliedSubscription = 1,
				@SalesPackageType = 2,
				@IsFixedValueDiscount = (CASE
											WHEN [MSL].[LogServiceNumber] <= [S].[SubscriptionMaxServiceFixedValue]
											THEN 1
											ELSE 0
										END),
				@Amount =				(CASE
				...
				@ValueType =			(CASE 
											...
											ELSE (
													...
													WHERE		SDR.SubscriptionId = [S].[IdSubscription]
														AND		MSL.LogServiceNumber BETWEEN SDR.DiscountLowServiceRange AND ISNULL(SDR.DiscountTopServiceRange, 1000000000)
												)
										END),
				@PromoValue =			(CASE 
											WHEN [MSL].[LogServiceNumber] <= [S].[SubscriptionMaxServiceFixedValue]
											THEN 0
											ELSE (
													SELECT	TOP 1 ISNULL(SDR.DiscountValue, 0)
													FROM	[DeliveryBackOffice].[dbo].[SubscriptionDiscountRange] SDR WITH(NOLOCK)
													WHERE	SDR.SubscriptionId = [S].[IdSubscription]
														AND	MSL.LogServiceNumber BETWEEN SDR.DiscountLowServiceRange AND ISNULL(SDR.DiscountTopServiceRange, 1000000000)
												)
	...
	ELSE
		BEGIN
			SET @Amount = (	SELECT TOP 1 DO.PriceShippment 
