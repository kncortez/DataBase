...
-- Description:	<Asignar tarifa por cliente>
-- =============================================
CREATE PROCEDURE [dbo].[spgAssignRatePerCustomer]
	@IdCustomer AS INT = 0
	,@IdRate AS INT = 0
...
BEGIN
 SELECT 'FALSE' [blnResult],
		...
		'' AS [ErrorProcedure],
		'' AS [ErrorLine],
		'Parámetro ' + '"IdCustomer"' + ' Obligatorio' AS [Message]
RETURN 0
END
...
BEGIN TRANSACTION
	...
	SET @IdRatebyCustomerAssigment  = (SELECT TOP (1) rbc.RbcId
										...
										AND rbc.RbcCodeOfReference IS NULL -- No sea visit point
	...
	BEGIN 
		...
		EXECUTE [dbo].[sphdSetRateLog] 
		  ...
		  ,@TokenCreated = @Token
		  ,@ModuleCreated = 'Socios de negocio'
