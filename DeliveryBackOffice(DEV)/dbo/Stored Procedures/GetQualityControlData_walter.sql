...
-- =============================================
create PROCEDURE [dbo].[GetQualityControlData_walter]
    ...
    BEGIN TRY
		...
		***********************************************************************************************************************************/
        ...
        WHERE --dsd.datecreated BETWEEN @CurrentDateAsDatetime AND @CurrentDateAsDatetimeFinishDay
            --CAST(dsd.datecreated AS DATE) = @DateToday
            dsd.DateCreated >= @DateToday
            AND dsd.DateCreated < DATEADD(DAY, 1, @DateToday)
            AND dsd.RowStatus = 1
        ...
        (
            ...
            FROM dbo.DeliveryOrderDetail ordd WITH (NOLOCK)
                ...
                LEFT JOIN [DeliveryBackOffice].[dbo].[DeliveryOrderBySettlement] ds WITH (NOLOCK)
                    ON dsd.ID_DeliveryOrderBySettlement = ds.ID
            WHERE
                --CAST(ordd.datecreated AS DATE) = @DateToday
                ordd.DateCreated >= @DateToday
                AND ordd.DateCreated < DATEADD(DAY, 1, @DateToday)
                ...
                (
                    dsd.ID IS NULL
                    OR CAST(dsd.DateCreated AS DATE) < @DateToday
                )
                AND ISNULL(ord.SenderCountryId, 'GT') = @IdCountry
