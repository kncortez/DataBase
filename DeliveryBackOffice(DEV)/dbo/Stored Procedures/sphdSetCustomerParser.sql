...
-- =============================================
 Create PROCEDURE [dbo].[sphdSetCustomerParser]
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN 
			print 'insert record'
			IF NOT EXISTS ( SELECT cli.IdCustomer FROM CustomerParser cli where cli.Name = @NameCustomer) 
			BEGIN
			INSERT INTO [DeliveryBackOffice].[dbo].[CustomerParser]
				   ([Name]
				   ,[Description]
				   ,[Domain]
				   ,[RegexSubject]
		...
		BEGIN
				...
				UPDATE [DeliveryBackOffice].[dbo].[CustomerParser]
