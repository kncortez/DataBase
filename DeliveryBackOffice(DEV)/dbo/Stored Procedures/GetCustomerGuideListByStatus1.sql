    ...
    -- Description:	Este procedimiento almacenado, GetCustomerGuideListByStatus, se utiliza para obtener una lista de guías para un cliente en especifico
    --				que estan en estado 'Generado' o 'Solicitado'.
    -- =============================================
    CREATE PROCEDURE [dbo].[GetCustomerGuideListByStatus1] 
    @IdCustomer INT,
    ...
    IF @ListStatus <> '0' AND NOT EXISTS(
            SELECT
                TOP 1 1
            FROM
                [dbo].[DeliveryOrder] AS do WITH(NOLOCK)
            OUTER APPLY
                [dbo].[DelimitedSplit8K](@ListStatus,',') AS ds
            WHERE
