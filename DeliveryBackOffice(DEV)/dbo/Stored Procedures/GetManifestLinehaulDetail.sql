...
BEGIN
    ...
    DECLARE @SubtypeBoxContainer BIGINT =
            (
                ...
                FROM [DeliveryBackOffice].[dbo].[CatContainerSubtype] CCS WITH (NOLOCK)
                WHERE [CCS].ContainerSubtypeName = 'CAJA' COLLATE Latin1_General_CI_AI
            );
    DECLARE @SubtypeFloorContainer BIGINT =
            (
                SELECT TOP (1)
                       [CCS].IdCatContainerSubtype
                FROM [DeliveryBackOffice].[dbo].[CatContainerSubtype] CCS WITH (NOLOCK)
                WHERE [CCS].ContainerSubtypeName = 'PISO' COLLATE Latin1_General_CI_AI
            );

    -- Insert statements for procedure here
    -- Table 0 Información del manifiesto
    SELECT lrp.IdLinehaulRoutePreparation IdLinehaulRoutePreparation,
           lrp.ContainerQuantity ContainerTotal,
           ISNULL(
           (
               SELECT SUM(lrpc.GuideQuantity)
           ...
           ISNULL(
           (
               SELECT SUM(lrpc.DryPieceQuantity + lrpc.[ColdPieceQuantity])
               FROM LinehaulRoutePreparationContainer lrpc WITH (NOLOCK)
                   INNER JOIN Container c WITH (NOLOCK)
                       ON lrpc.ContainerId = c.IdContainer
                          AND c.RowStatus = 1
    ...
    SELECT [TotalGuidesLinehaul].[Guide],
    ...
    FROM
    (
        SELECT [DispatchedLienahul].[Guide],
        ...
        (
            SELECT TOP 100 PERCENT
                   ...
                   CONCAT(
                             (lrpcd.DryPieceQuantity + lrpcd.ColdPieceQuantity),
                             '/',
                             (lrpcd.GuideDryPieceTotal + lrpcd.GuideColdPieceTotal)
                         ) Pieces,
                   ...
                   IIF((lrpcd.DryPieceQuantity + lrpcd.ColdPieceQuantity) = (lrpcd.GuideDryPieceTotal
                   ...
                   ad.ActId Act,
                   CASE
                       WHEN ad.ActId IS NOT NULL THEN
                       (
                           SELECT STUFF(
                                  (
                                      SELECT ', ' + CAST(adp.PieceNumber AS VARCHAR)
        ...
        UNION
        SELECT [ExtraGuidesLinehaul].[Guide],
            ...
            FROM
                ...
                INNER JOIN [dbo].[LinehaulRouteSettlementContainer] LRSC WITH (NOLOCK)
                    ON [LRSC].[LinehaulRouteSettlementId] = [LRS].[IdLinehaulRouteSettlement]
                       AND [LRSC].[ContainerId] <> [LRPC].[ContainerId]
                ...
                INNER JOIN [dbo].[LinehaulRouteSettlementContainerDetail] LRSCD WITH (NOLOCK)
                    ON [LRSCD].[LinehaulRouteSettlementContainerId] = [LRSC].[IdLinehaulRouteSettlementContainer]
                       AND [LRSCD].[GuideNumber] <> [LRSCD].[GuideNumber]
                       AND [LRSCD].[RowStatus] = 1
                INNER JOIN [dbo].[Container] Ctn WITH (NOLOCK)
                    ON [Ctn].[IdContainer] = [LRSC].[ContainerId]
                INNER JOIN [dbo].[HubLogistics] HL WITH (NOLOCK)
                    ON [HL].[IdHubLogistic] = [LRSC].[HubId]
                INNER JOIN [dbo].[DeliveryOrder] DO WITH (NOLOCK)
                    ON [DO].[Guide_Serie] = [LRSCD].[GuideSerie]
                       AND [DO].[Guide_Number] = [LRSCD].[GuideNumber]
                INNER JOIN [dbo].[StatusOrder] SO WITH (NOLOCK)
                    ON [SO].[StatusOrderId] = [DO].[StatusOrderId]
                LEFT JOIN [dbo].[ActDetail] AD WITH (NOLOCK)
                    ON [AD].[GuideSerie] = [LRSCD].[GuideSerie]
                       AND [AD].[GuideNumber] = [LRSCD].[GuideNumber]
                       AND [AD].[RowStatus] = 1
            WHERE [LRP].[IdLinehaulRoutePreparation] = @IdLinehaulRoutePreparation
				AND IIF(hl.IdCountry IS NULL, 'GT', hl.IdCountry)= @IdCountry
            ORDER BY HL.HubName
    ...
    GROUP BY [TotalGuidesLinehaul].[Guide],
             [TotalGuidesLinehaul].[Pieces],
             [TotalGuidesLinehaul].[ContainerDescription],
             [TotalGuidesLinehaul].[HubName],
             [TotalGuidesLinehaul].[HubAbbreviation],
