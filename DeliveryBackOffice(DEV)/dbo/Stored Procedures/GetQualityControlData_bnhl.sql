...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @Pending_Counter INT = 0,
                ...
                --@ConfirmationIncidents_Counter INT = 0,
                --@UConfirmationIncidents_Counter INT = 0;
		...
		***********************************************************************************************************************************/
        ...
        --set @CurrentDateAsDatetime = '2024-07-02 00:00:00.000'
        ...
        select @Pending_Counter = sum(   case
                                             when gdd.statusorderid = 4 then
                                                 1
                                             else
                                                 0
                                         end
                                     ),
               @Delivered_Counter = sum(   case
                                               when gdd.statusorderid in ( 5, 24, 25 ) then
                                                   1
                                               else
                                                   0
                                           end
        ...
        where dsd.datecreated
              BETWEEN @CurrentDateAsDatetime AND @CurrentDateAsDatetimeFinishDay
              and gdd.statusorderid in ( 4, 5, 24, 25 )
              and dsd.rowstatus = 1;
