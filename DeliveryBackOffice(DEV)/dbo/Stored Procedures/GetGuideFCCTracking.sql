...
BEGIN
	...
	DECLARE @GoodResponseMessage NVARCHAR(MAX) = CONCAT('Estimado cliente, la guía FD ', Cast(@GuideNumber AS varchar), ' <STATUS> desde el <DATE>, gracias por usar nuestros servicios.');
	...
	BEGIN TRY
		...
		INSERT INTO
			@ResponseTable
			(GuideNumber, GuideStatus, GuideStatusDescription, GuideStatusDate ,GuideStatusMesseage
			)
		SELECT 
			TOP 1  
				DOD.Guide_Number
		...
		BEGIN
			...
			SELECT
				TOP 1
					1 [blnResult]
					,REPLACE(REPLACE(@GoodResponseMessage,'<STATUS>',RT.GuideStatusMesseage),'<DATE>',REPLACE(CONVERT(NVARCHAR, RT.GuideStatusDate, 103),'/',' de ')) [messageResult]
					--,@GoodResponseMessage [messageResult]
