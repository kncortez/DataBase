...
BEGIN
    ...
    DECLARE @GoodResponseMessage NVARCHAR(MAX)
        = CONCAT(
                    'Estimado cliente, la guía FD ',
                    CAST(@GuideNumber AS VARCHAR),
	...
	END
        ...
        BEGIN
            ...
            SELECT @Datetext = RT.GuideStatusDate
            ...
            SET @Datetext2 = @diafecha + N' ' + @diaf + N' de ' + @mes + N' de ' + @anio;
            SELECT TOP 1
                   1 [blnResult],
            ...
            SELECT A.Tag,
                   CASE
                       WHEN A.Tag = '<GuideSerie>' THEN
                           'FD'
                       WHEN A.Tag = '<GuideNumber>' THEN
                           CAST(B.GuideNumber AS VARCHAR)
                       WHEN A.Tag = '<GuideStatus>' THEN
                           B.GuideStatusDescription
                       WHEN A.Tag = '<StatusDate>' THEN
                           @Datetext2
                       WHEN A.Tag = '<TownshipDelivery>' THEN
                           B.TownShipName
                       WHEN A.Tag = '<TownshipPickup>' THEN
                           B.TownShipName
                       WHEN A.Tag = '<Incidence>' THEN
                           B.Incidence
                       WHEN A.Tag = '<NamePersonReceives>' THEN
                           B.ReceiverName
                       WHEN A.Tag = '<NamePersonSending>' THEN
                           B.NamePersonSending
                       WHEN A.Tag = '<ProvidenceDelivery>' THEN
                           B.ProvidenceDelivery
                       WHEN A.Tag = '<ProvidencePickup>' THEN
                           B.ProvidencePickup
                       WHEN A.Tag = '<NameHubOrigin>' THEN
                           B.HUB
                       WHEN A.Tag = '<NameExc>' THEN
                           B.EXC
                       WHEN A.Tag = '<Client>' THEN
                           B.Client
                       ELSE
                           ''
                   END TagValue
        ...
        ELSE
        BEGIN
            SELECT 0 [blnResult], -- Indica que no existe un último estado publico posible de retornar
