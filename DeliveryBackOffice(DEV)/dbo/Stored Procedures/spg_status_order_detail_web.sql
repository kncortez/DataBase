...
BEGIN
    ...
    (
        SELECT 0 [EventID],
               ...
               '' NextSteps,
			   '' [UserIncident],
			   --IIF(COI.ValidGeolocationEvidence IS NULL AND do.StatusOrderId=50,
      --                IIF(IsConfirmed = 1 AND IsDenied = 0 AND do.StatusOrderId=50, 1, 0),
      --                          IIF(COI.ValidGeolocationEvidence = 1 AND do.StatusOrderId=50, 1, 0))
			                          ''   AS 'ValidGeolocationEvidence',
			   --IIF(COI.ValidPhotographicEvidence IS NULL AND do.StatusOrderId=50,
				  --     IIF(IsConfirmed = 1 AND IsDenied = 0 AND do.StatusOrderId=50, 1, 0), 
				  --                 IIF(COI.ValidPhotographicEvidence = 1 AND do.StatusOrderId=50, 1, 0))
