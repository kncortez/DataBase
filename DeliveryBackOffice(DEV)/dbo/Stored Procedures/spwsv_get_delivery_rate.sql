...
-- =============================================
CREATE PROCEDURE [dbo].[spwsv_get_delivery_rate]
...
------- determinar el Tarifario y tipo de tarifario que se va a aplicar -------------------------------------------------------
	...
	DECLARE @IdServiceType INT
	SET @IdServiceType = (SELECT TOP 1 CTS.CtsId FROM [DeliveryBackOffice].[dbo].[CatTypeService] CTS WITH(NOLOCK) WHERE CTS.CtsShortName = 'NDD' COLLATE Latin1_General_CI_AI);
    ...
    BEGIN
	   ...
	   IF (EXISTS (SELECT TOP 1 1 FROM [DeliveryBackOffice].[dbo].[VisitPointClient] VPC WITH(NOLOCK) WHERE VPC.CodeOfReference = @CodeOfReferenceDestiny AND VPC.StatusClient = 1 AND VPC.DescriptionOfClient LIKE 'FD%EXC%' COLLATE Latin1_General_CI_AI))
		BEGIN
			  ...
			  FROM [DeliveryBackOffice].[dbo].[AlternativeRatebyCustomer] ARC 
			  WHERE ARC.CustomerId = @IdCustomer AND ARC.RowStatus = 1  AND @IdRate = @NewMainRates
...
------------------------- Determinar si el servico es TDA ------------------------------------------------------------------
	...
	BEGIN
		...
		SET @IdServiceType = (SELECT TOP 1 CTS.CtsId FROM [DeliveryBackOffice].[dbo].[CatTypeService] CTS WITH(NOLOCK) WHERE CTS.CtsShortName = 'TDA' COLLATE Latin1_General_CI_AI);
	...
	BEGIN
		...
		SET @IdServiceType = (SELECT TOP 1 CTS.CtsId FROM [DeliveryBackOffice].[dbo].[CatTypeService] CTS WITH(NOLOCK) WHERE CTS.CtsShortName = 'SDD' COLLATE Latin1_General_CI_AI);
...
------------------------------------- verificar tarifas de piezas irregulares ---------------------------------------------------
		...
		IF(@IdRate IN (@NewMainRates, @NewAlternativeRates))
		BEGIN
		...
		declare @jsonResult as nvarchar(max)
		 ...
		 set @jsonResult =( SELECT STUFF(( 
					...
					'"ServiceShortName":"' + isnull(tr.Service,'') + '",' +
						...
						'"Currency":"' + @Currency + '"' +
						'}' ,' '  ) +
						iif((tr.CreditCardRate)>0, ',{"Description":"' + 'Otros Cargos' + '",' + 
