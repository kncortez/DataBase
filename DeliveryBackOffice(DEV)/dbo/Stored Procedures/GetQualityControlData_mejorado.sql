...
BEGIN
            ...
            SELECT --TOP 1000
              ...
              , --IIF(ord.StatusOrderId = 4
                  ...
                  --    , IIF(cfi.IsConfirmed = 1, 'Incidencias Procesadas', 'Incidencias Pendientes de Procesar'))) 
					  ...
					  [Description]
              , SUM(1)/*SUM(IIF(ord.StatusOrderId = 4, 1, 0))  */                                                            [Pending]
              , /*SUM(IIF(ord.StatusOrderId = 4, 0, IIF(IncidenceTbl.DeliveryAttemptId IS NULL, 1, 0)))   */0           [Delivered]
              , /*SUM(IIF(ISNULL(cfi.IsConfirmed, 0) = 1, 1, 0)) */0                                                    [ConfirmationIncidents]   --ConfirmedIncidents
              , /*SUM(IIF(ISNULL(cfi.IsConfirmed, 1) = 1, 0, 1)) */0                                                    [UnConfirmationIncidents] --UnconfirmedIncidents
    ...
    --            OUTER APPLY
            ...
            WHERE CONVERT(DATE, ds.Date_Dispatched) = CONVERT(DATE, GETDATE())
				  ...
				  AND ord.StatusOrderId IN (4,5,25,24) --ENTREGADO, COD PAGADO, COD LIQUIDADO --EN RUTA
