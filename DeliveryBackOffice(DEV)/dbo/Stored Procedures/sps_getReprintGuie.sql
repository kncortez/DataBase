/* =================================================
   SP:        sps_getReprintGuie
   Propósito: Reporte que carga la información para la reimpresión de guías
   Autor:     ---
   Historia:  ---
   Fecha:     ---
...
BEGIN
    ...
    DECLARE @FranchiseVisitPointTypeId INT =
            (
                ...
                WHERE [KOVPC].[KindOfVPName] = 'Concesionario'
                      AND [KOVPC].[IdCountry] = @CountryThatConsults
            );
    ...
    DECLARE @j INT = 0,
            ...
            @identityCost INT;
    DECLARE @StatusPackage INT =
            (
                SELECT IdCatSalesPackageStatus
                FROM CatSalesPackageStatus
    ...
    BEGIN
        SELECT @arpieces
            = @arpieces + '{"length":' + CONVERT(VARCHAR, ISNULL(PieceLength, 0)) + ',' + +'"width":'
              ...
              + '"ParcelCode":"' + COALESCE(ParcelCode, '') + '",' + ++'"fragil":' + CASE
                                                                                         WHEN fragile = 1 THEN
                                                                                             'true'
                                                                                         ELSE
                                                                                             'false'
    ...
    WHERE ct.GuideSerie = @Serie_Number
        AND ct.GuideNumber = @Guide_Number;
    ...
    (
        ...
        WHERE do.Guide_Serie = @Serie_Number
    ...
    DECLARE @PaymentAllowsCollect INT =
            (
                SELECT TOP 1
                       COUNT(   CASE
                                    WHEN s.CatTypeSubscriptionId = 2 THEN
                                        1
                                    WHEN s.CatTypeSubscriptionId = 1 THEN
                                        0
                                    WHEN s.CatTypeSubscriptionId IS NULL THEN
                                        0
                                    ELSE
                                        0
                                END
                ...
                WHERE LogGuideSerie = @Serie_Number
               ...
               + STUFF(
                          (
                              SELECT DISTINCT TOP 1 --Price
                                     ...
                                     + '",' + '"ContentDescription":"'
                                     + CONVERT(VARCHAR, ISNULL(dev.Package_Description, '')) + '",'
                                     + '"IdCountry":"'
                                  ...
                                  '"name":"'
                                     + IIF(
                                           ...
                                           AND [vp].[IdKindOfVPClient] = @EXCKindOfVPC,
                                           REPLACE(
                                                      dbo.fnt_String_Escape(ISNULL([vp].[DescriptionOfClient], ''), 'json'),
                                                      '"',
                                                      ''
                                                  ) + ' - ',
                                           '')
                                  ...
                                  '"contact":"'
                                     ...
                                     + '"address1":"'
                                     + REPLACE(
                                                  dbo.fnt_String_Escape(COALESCE(dev.Receiver_Address, ''), 'json'),
                                                  '"',
                                                  '\"'
                                              ) + '",' + '"address2":"'
                                     + REPLACE(
                                     ...
                                     + dbo.fnt_String_Escape(COALESCE(dev.TypeService, 'EXP'), 'json') + '",'
                                  ...
                                  '"COD":  {' + '"CashOnDelivery": ' + (CASE
                                     ...
                                     + COALESCE(   (CASE
                                                        WHEN ISNULL([dev].[IsLastMileReturn], 0) = 1 THEN
                                                            'D'
                                                        WHEN MMBSHP.IdMembership IS NOT NULL THEN
                                                            'F'
                                                        WHEN ctm.BusinessSegmentID = @IDCatBusinessB2B THEN
                                                            'B'
                                                        ELSE
                                                            'E'
                                                    END
                                     ...
                                     + COALESCE(
                                                   CONCAT('https://forzadelivery.com/rastreo/', Guide_Serie, Guide_Number),
                                                   ''
                                               ) + '",' + '"UseMembership": '
                                     + CONVERT(
                                                  VARCHAR,
                                                  CAST(ISNULL(
                                                                 (CASE
                                                                      WHEN [MSL].[IdMembershipSubscriptionLog] IS NOT NULL THEN
                                                                          1
                                                                      ELSE
                                                                          0
                                                                  END
                                     ...
                                     + CONVERT(VARCHAR, IIF(CSBT.CatTypeSubscriptionId = 2, 0, 1)) + ','
                                     + '"CategoryProductId": '
                                     + CONVERT(
                                                  VARCHAR,
                                                  IIF([MSL].[MembershipId] IS NOT NULL,
                                                      CMSL.CatProductCategoryId,
                                                      IIF(CSBT.CatProductCategoryId IS NOT NULL,
                                                       CSBT.CatProductCategoryId,
                                                       0))
                                              ) + ',' + '"ProductId": '
                                     + CONVERT(
                                                  VARCHAR,
                                                  IIF([MSL].[MembershipId] IS NOT NULL,
                                                      [MSL].[MembershipId],
                                                      IIF(MSL.SubscriptionId IS NOT NULL, MSL.SubscriptionId, 0))
                                     ...
                                     + COALESCE(FORMAT([dev].[DeliveryETA], 'ddMM'), '') + '",'
                                     + '"WayToPayDescription": "'
                                     + COALESCE(   (CASE
                              ...
                              FROM DeliveryBackOffice.dbo.DeliveryOrder dev WITH (NOLOCK)
                                  ...
                                  LEFT JOIN DeliveryBackOffice.dbo.Cost c WITH (NOLOCK)
                                      ON dev.Guide_Serie = c.GuideSerie
                                         AND dev.Guide_Number = c.GuideNumber
                              ...
                              WHERE dev.Guide_Serie = @Serie_Number
                                    AND dev.Guide_Number = @Guide_Number
                              FOR XML PATH(''), TYPE
                          ).value('.', 'varchar(max)'),
                          1,
                          1,
