...
SET @Cantidad = (SELECT count(IdCustomer) FROM Customer WITH(NOLOCK) WHERE RegexEmail Like '%' + @Email + '%')

	IF (@Cantidad > 0)
		BEGIN
			SELECT IdCustomer, Name, RegexSubject, RegexEmail, RegexFilename, ISNULL(CountryID,'GT') AS CountryID, ISNULL(RowSatus,1) AS ActiveUser FROM Customer WITH(NOLOCK)
				 WHERE RegexEmail Like '%' + @Email + '%'
				 AND IdCustomerType = 1 --corporativo
		END
