   ...
   DECLARE @NewRateGeneralDiscount INT = (
    ...
    BEGIN
		...
		print(@jsonResult)
				...
				SELECT 
			ISNULL(tr.ServiceName, '') AS Title,
			CONVERT(VARCHAR(1), @CalculateMembership) AS UseMembership,
			IIF(@IdCustomerParams = 0 AND @IdCustomer = 6, ISNULL(tr.ServiceName, ''), ISNULL(tr.Segment, '')) AS Service,
			ISNULL(tr.ServiceDescription, '') AS ServiceDescription,
			ISNULL(tr.Service, '') AS ServiceShortName,
			CONVERT(VARCHAR(24), @FechaCompra, 120) AS DeliveryDate,
			CONVERT(VARCHAR(20), 
				...
				CONVERT(DECIMAL(12, 2), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT', tr.InsuranceRate, 'false')) +
				CONVERT(DECIMAL(12, 2), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT', tr.CreditCardRate, 'false')) +
				CONVERT(DECIMAL(12, 2), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT', tr.OverWeightRate, 'false')) +
				CONVERT(DECIMAL(12, 2), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT', (CONVERT(DECIMAL(12, 2), tr.BaseRate) - CONVERT(DECIMAL(12, 2), tr.Discount) + CONVERT(DECIMAL(12, 2), tr.FragilRate) + CONVERT(DECIMAL(12, 2), tr.CollectedRate) + CONVERT(DECIMAL(12, 2), tr.InsuranceRate) + CONVERT(DECIMAL(12, 2), tr.CreditCardRate) + CONVERT(DECIMAL(12, 2), tr.OverWeightRate) + CONVERT(DECIMAL(12, 2), tr.IrregularPieceRate)), 'true'))
			) AS Price,
			@Currency AS Currency,
			'Servicio' AS Integration_Description1,
			CONVERT(VARCHAR(20), CONVERT(DECIMAL(12, 2), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT', tr.BaseRate + tr.IrregularPieceRate, 'false'))) AS Integration_Price1,
			@Currency AS Integration_Currency1
		FROM 
			@TempRate tr
