...
BEGIN
    ...
    DECLARE @IsPhoneValid BIT = CASE
                                    WHEN @Receiver_Phone IS NOT NULL
                                         AND LTRIM(RTRIM(@Receiver_Phone)) =
                                         (
                                             SELECT Receiver_Phone FROM @GuideOrderTemp
                                         ) THEN
                                        1
                                    ELSE
                                        0
    ...
    (
        ...
        FROM @GuideOrderTemp                                 do
            INNER JOIN dbo.DeliveryOrder                     dor WITH (NOLOCK)
                ON do.Guide_Serie = dor.Guide_Serie
                   AND do.Guide_Number = dor.Guide_Number
        ...
        WHERE do.Guide_Serie = @Guide_Serie
              AND do.Guide_Number = @Guide_Number
        UNION
        SELECT RANK() OVER (PARTITION BY dod.Guide_Number
             ...
             (CASE
                  ...
                  (
                      ...
                      WHERE dttt.ID = dod.DeliveryAttemptId
                  ) IS NOT NULL THEN
                  (
                      SELECT TOP 1
        ...
        FROM dbo.DeliveryOrderDetail                      dod WITH (NOLOCK)
            ...
            INNER JOIN @GuideOrderTemp                    GOT
                ON GOT.Guide_Serie = dod.Guide_Serie
                   AND GOT.Guide_Number = dod.Guide_Number
