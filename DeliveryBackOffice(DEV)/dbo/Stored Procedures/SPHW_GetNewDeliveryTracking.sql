...
BEGIN
    BEGIN TRY
        ...
        BEGIN
            SELECT @GuideSerie = ISNULL(Guide_Serie, ''),
        ...
        BEGIN
            DECLARE @Receiver_Phone NVARCHAR(200) =
                    (
                        SELECT RIGHT(LTRIM(RTRIM(Receiver_Phone)), 8)
                        FROM DeliveryBackOffice.dbo.DeliveryOrder WITH (NOLOCK)
                        WHERE Guide_Serie = @GuideSerie
                              AND Guide_Number = @GuideNumber
                    );
                ...
                =
                    (
                        SELECT CASE
                                   ...
                                   WHEN LEFT(DO.Receiver_Phone, 1) = '+'
                                        AND LEN(DO.Receiver_Phone) >= 11 THEN
                                   ...
                                   WHEN LEN(DO.Receiver_Phone) >= 10
                                        AND ISNUMERIC(LEFT(DO.Receiver_Phone, 3)) = 1 THEN
                        ...
                        FROM DeliveryBackOffice.dbo.DeliveryOrder DO WITH (NOLOCK)
                            LEFT JOIN DeliveryBackOffice.dbo.ConfigParams C WITH (NOLOCK)
                                ON C.[Name] = 'AreaCode'
                                   AND ISNULL(DO.ReceiverCountryId, 'GT') = C.IdCountry
            ...
            IF (
                   ...
                   OR (@Phone = @CodeArea + @Receiver_Phone)
                   OR (@Phone = '+' + @CodeArea + @Receiver_Phone)
               )
            BEGIN
            ...
            SET @IdCustomerExist = ISNULL(
                                   (
                                       SELECT TOP 1 IdCustomer FROM @GuideOrderTemp
                                   ),
                                   0
                                         );
            ...
            SET @StatusForCustomerExist = ISNULL(
                                          (
                                              SELECT TOP 1 1 FROM @StatusOrderForCustomer
                                          ),
                                          0
                                                );
        ...
        BEGIN
            ...
            BEGIN
                ...
                WHERE DO.Ticket_Number = @TicketNumber
                      ...
                      (
                          @IdCustomer IS NULL
                          OR DO.IdCustomer = @IdCustomer
                      );
            END;
            ELSE
            BEGIN
                SELECT 404 AS 'IdResult',
