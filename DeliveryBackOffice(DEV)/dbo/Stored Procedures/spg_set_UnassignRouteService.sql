...
AS
BEGIN
	DECLARE @ServiceManagementId INT = ( SELECT
				IdServiceManagement
	...
	BEGIN TRY	
		...
		BEGIN
			...
			UPDATE [DeliveryBackOffice].[dbo].[ServiceManagement] SET IdPuCourrier = NULL, IdPuRouteAssigment = NULL, ServiceStatusId = 1, Amount = 0 WHERE IdSchedulePickup = @idSchedulePickup;
			UPDATE [DeliveryBackOffice].[dbo].[SchedulePickup] SET AssigmentStatus = 0 WHERE SchedulePickupId = @idSchedulePickup;

			INSERT INTO [DeliveryBackOffice].[dbo].[EventService] (ServiceManagementId,ServiceStatusId,RowStauts,TokenCreated,DateCreated)
			VALUES(@ServiceManagementId,1,1,@token,GETDATE())

			--Desactivar el servicio si es una ruta de Rabbit
			IF (SELECT CodeRoute FROM CatRoute WHERE IdRoute = @idRoute) LIKE '%RABBIT%'
			BEGIN
				UPDATE spsd
				...
				JOIN SettlementPickupStation sps WITH(NOLOCK)
			...
			FROM DeliveryBackOffice.dbo.SchedulePickup AS shp WITH (NOLOCK)
				...
				LEFT JOIN (
					SELECT TimePlaId,IdHeaderRecolection,drosub.Sender_Town,drosub.Sender_Department,drosub.TypeService,drosub.Sender_Zone FROM [DeliveryBackOffice].[dbo].[DeliveryOrderPaymentDetail] dopsub
					LEFT JOIN DBO.DeliveryOrder drosub ON drosub.Guide_Number = dopsub.GuideNumber
					WHERE IdHeaderRecolection = @idSchedulePickup
					group by TimePlaId,IdHeaderRecolection,drosub.Sender_Town,drosub.Sender_Department,drosub.TypeService,drosub.Sender_Zone
				)dop_group
				...
				LEFT JOIN dbo.ServiceManagement srv ON srv.IdSchedulePickup = shp.SchedulePickupId
				LEFT JOIN [DeliveryBackOffice].[dbo].[CatServiceStatus] as css WITH(NOLOCK) on css.IdServiceStatus = srv.ServiceStatusId
