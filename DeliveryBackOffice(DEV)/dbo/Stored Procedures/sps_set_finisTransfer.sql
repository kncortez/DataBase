CREATE PROCEDURE [dbo].[sps_set_finisTransfer]
    ...
    BEGIN TRY
          ...
          UPDATE DeliveryBackOffice.dbo.DeliveryOrder
			...
			JOIN #listGuidesEnabled lge ON dsd.Guide_Serie = lge.Guide_Serie AND lge.Guide_Number= dsd.Guide_Number
				 /*
		  ...
		  -- Actualizar registros del detalle de servicios de devolución
			...
			FROM dbo.SettlementByPickup stp WITH (NOLOCK)
					LEFT JOIN dbo.SettlementByPickupDetail std WITH (NOLOCK) ON std.SettlementByPickupId = stp.Id AND std.RowStatus = 1
					JOIN #listGuidesEnabled lge ON lge.Guide_Serie = std.GuideSerie AND lge.Guide_Number = std.GuideNumber
					...
					WHERE stp.IdCourier = @IdCourier and smG.SubTypeServiceManagmentId = 3
