...
BEGIN
    ...
    --	    'sps_proof_ondelivery_fd',     -- ErrorProcedure - varchar(100)
    ...
    CREATE NONCLUSTERED INDEX IDX_GuidesProcessCOD
    ON #GuidesProcessCOD (
                             GuideSerie
                           , GuideNumber
                         );
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            BEGIN
                ...
                SET NameOfReceiver = @ReceiverName
                  ...
                  , StatusOrderId = IIF(
                                        @IdDeliveryOptionGuide = @IdDeliveryOption
                                        AND ISNULL(@IsReturn, 0) = 0
                                      , @StatusEXC
                                      , IIF(@IsExpress = 'true' AND ISNULL(@IsReturn, 0) = 0
                                            , @StatusEXC
                                            , IIF(@IsReturn = 1, 14, 5)))
                ...
                AS (SELECT TOP 1
                         ...
                         , A.Guide_Number
                         , MAX(ID) AS LastID
                    FROM [dbo].[DeliverySettlementDetail] A WITH (NOLOCK)
                    WHERE Guide_Serie = @GuideSerie
                          AND Guide_Number = @GuideNumber
                    GROUP BY Guide_Number
                           , Guide_Serie)
                UPDATE ds
                SET ds.StatusOrderId = IIF(
                                           @IdDeliveryOptionGuide = @IdDeliveryOption
                                           AND ISNULL(@IsReturn, 0) = 0
                                         , @StatusEXC
                                         , IIF(@IsExpress = 'true' AND ISNULL(@IsReturn, 0) = 0
                                               , @StatusEXC
                                               , IIF(@IsReturn = 1, 14, 5)))
                        FROM [dbo].[DeliverySettlementDetail] ds WITH(NOLOCK)
                    INNER JOIN [LatestID]             li
                        ON ds.Guide_Number = li.Guide_Number
                           AND ds.ID = li.LastID
                ...
                BEGIN TRY
                    ...
                    FROM [DeliveryBackOffice].[dbo].[DeliveryOrder]       DO WITH (NOLOCK)
                        INNER JOIN [DeliveryBackOffice].[dbo].[Account]   Acc WITH (NOLOCK)
                            ON DO.IdCustomer = Acc.IdCustomer
                    ...
                    SELECT TOP 1
                           @IsGuideValidForPoints = 0
                    FROM [DeliveryBackOffice].[dbo].[MembershipSubscriptionLog] MSL WITH (NOLOCK)
                        INNER JOIN [DeliveryBackOffice].[dbo].[Subscription]    SBSCPN WITH (NOLOCK)
                            ON MSL.MembershipId = SBSCPN.MembershipId
                               AND MSL.SubscriptionId = SBSCPN.IdSubscription
                    WHERE MSL.LogGuideSerie = @GuideSerie
                          AND MSL.LogGuideNumber = @GuideNumber
                          AND MSL.RowStatus = 1
                          AND MSL.LogServiceNumber <= SBSCPN.SubscriptionMaxServiceFixedValue;
                    ...
                    FROM [DeliveryBackOffice].[dbo].[PointsByServiceLog] PBSL
                    WHERE PBSL.GuideNumber = @GuideNumber
                          AND PBSL.GuideSerie = @GuideSerie
                    ...
                    IF (
                           @MembershipId > 0
                           AND @GuideIsCollect = 1
                           AND ISNULL(@IsGuideValidForPoints, 0) = 1
                           AND ISNULL(@GuideAlreadyInPointLog, 0) = 0
                       )
                    BEGIN

                        -- Agregar Log de puntos 
                        INSERT INTO [DeliveryBackOffice].[dbo].[PointsByServiceLog]
                        (
                            [MembershipId]
                          , [GuideSerie]
                          , [GuideNumber]
                        ...
                        VALUES
                        (   @MembershipId, @GuideSerie, @GuideNumber, @GuidePrice
                          , (CASE
                                 ...
                                 WHEN @ForzaPointsGenerationType = 'MONTO' THEN
                                     CAST((@GuidePrice / @ForzaPointsGenerationValue) AS INT)
                                 ELSE
                                     0
                             END
                            ) -- POINTS RECEIVED
                          , 0 -- POINTS CONSUMED
                          , @ForzaPointsGenerationType, NULL, 1, SYSDATETIME(), @Token);
                ...
                BEGIN
                    ;WITH DataToUpdate
                     AS (SELECT ord.Guide_Serie  AS GuideSerie
                              , ord.Guide_Number AS GuideNumber
                              , (
                                    SELECT TOP 1
                                           IdCourierman
        ...
        (
            SELECT TOP 1
                   1
            FROM [dbo].[DeliveryOrder] dlo WITH (NOLOCK)
            WHERE dlo.Guide_Serie = @GuideSerie
                  AND dlo.Guide_Number = @GuideNumber
                  AND dlo.IsLastMileReturn = 1
        )
           ) -- guía marcada para devolución
        BEGIN
            ...
            BEGIN
                ;WITH DataToUpdate
                 AS (SELECT @GuideSerie    AS GuideSerie
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        -- Si existe una ubicación para registrar
                        UPDATE [DeliveryBackOffice].[dbo].[VisitPointClient]
                        SET Latitude = ISNULL(@FixedLatitude, Latitude)
