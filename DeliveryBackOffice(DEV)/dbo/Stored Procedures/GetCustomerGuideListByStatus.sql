...
BEGIN
    ...
    BEGIN TRY --Verificación si el cliente cuenta con guias 
        IF NOT EXISTS
        (
            SELECT TOP 1
                   1
        ...
        BEGIN
            SELECT 204 AS 'responseCode',
                   'Cliente no encontrado' AS 'responseMessage';
        ...
        (
            ...
            FROM [DeliveryBackOffice].[dbo].[DeliveryOrder] AS DO WITH (NOLOCK)
                OUTER APPLY [DeliveryBackOffice].[dbo].[DelimitedSplit8K](@ListStatus, ',') AS ds
            WHERE DO.IdCustomer = @IdCustomer
        ...
        IF (@Method IS NULL)
            IF (@ListStatus = '0')
                SELECT CAST(serv.Sender_ID AS VARCHAR) + ' - ' + ISNULL(UPPER(serv.Sender_FirstName), '') + ' '
                       + ISNULL(UPPER(serv.Sender_LastName), '') [Sender],
                ...
                FROM [DeliveryBackOffice].[dbo].[DeliveryOrder] AS serv WITH (NOLOCK)
                    LEFT JOIN [DeliveryBackOffice].[dbo].[StatusOrder] AS so WITH (NOLOCK)
                        ON serv.StatusOrderId = so.StatusOrderId
                WHERE serv.DateCreated
                      BETWEEN @InitDate AND @EndDate
                ...
                (
                    SELECT TOP 1
                           1
                    FROM [dbo].[DelimitedSplit8K](@BlackListId, ',') AS dsb
            ...
            ELSE
                ...
                (
                    SELECT TOP 1
                           1
                    FROM [dbo].[DelimitedSplit8K](@ListStatus, ',') AS dsl
        ...
        ELSE IF (@Method = 'LST01')
            ...
            SELECT A1.Guide_Serie + CAST(A1.Guide_Number AS VARCHAR) [Numero de guia],
                   ...
                   ISNULL(A1.Receiver_Town, '') [Municipio], --Nuevo campo solicitado por Jennifer Luna
                   ...
                   CASE
                       WHEN CHARINDEX('-', A3.DescriptionOfClient) > 0 THEN
                           CAST(A1.Sender_ID AS VARCHAR) + ' - '
                           + ISNULL(
                                       UPPER(TRIM(LEFT(A3.DescriptionOfClient, CHARINDEX('-', A3.DescriptionOfClient)
                                                                               - 1)
                                                 )
                                            ),
                                       ''
                                   ) + ' ' + ISNULL(UPPER(A1.Sender_FirstName), '') + ' '
                           + ISNULL(UPPER(A1.Sender_LastName), '')
                       ELSE
                           CAST(A1.Sender_ID AS VARCHAR) + ' - ' + ISNULL(UPPER(A1.Sender_FirstName), '') + ' '
                           + ISNULL(UPPER(A1.Sender_LastName), '')
                   END AS [Sede/tienda origen],
                   A1.Order_Number [Numero de orden]         --Nuevo campo solicitado por Jennifer Luna
            FROM [DeliveryBackOffice].[dbo].[DeliveryOrder] AS A1 WITH (NOLOCK)
                ...
                LEFT JOIN [DeliveryBackOffice].[dbo].[VisitPointClient] AS A3 WITH (NOLOCK)
