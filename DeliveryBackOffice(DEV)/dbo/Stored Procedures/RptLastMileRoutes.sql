...
BEGIN
		...
		SET @STATUS_DELIVERED_ID = (	SELECT	[SO].[StatusOrderId]
										FROM	[dbo].[StatusOrder] SO
										WHERE	[SO].[OrderDescription] = 'Entregado');

		SET @STATUS_RETURNED_ID = (	SELECT	[SO].[StatusOrderId]
									FROM	[dbo].[StatusOrder] SO
									WHERE	[SO].[OrderDescription] = 'Devuelto');

		SET @STATUS_TRANSFERED_EX_ID = (SELECT	[SO].[StatusOrderId]
										FROM	[dbo].[StatusOrder] SO
										WHERE	[SO].[OrderDescription] = 'Traslado a Express Center');

		SET @STATUS_COD_PAID = (SELECT	[SO].[StatusOrderId]
								FROM	[dbo].[StatusOrder] SO
        ...
        SELECT		ROW_NUMBER() OVER (ORDER BY [DOBS].[Date_Dispatched]) Renglon,
					...
					SUM(IIF(DODdelivery.DeliveryExists IS NULL AND DODreturn.ReturnExists IS NULL AND DODtransfer.TransferExists IS NULL, 1, 0)) Other_Guides_Checkpoint,
					SUM(IIF(DODdelivery.DeliveryExists IS NULL AND DODreturn.ReturnExists IS NULL AND DODtransfer.TransferExists IS NULL, (DO.Pieces_Dry + DO.Pieces_Cold), 0)) Other_Pieces_Checkpoint,
		...
		OUTER APPLY
			(
				...
				WHERE
					...
					AND
					[DOD].[StatusOrderId] IN (@STATUS_DELIVERED_ID ,@STATUS_COD_PAID)
					AND 
					CAST([DOD].DateCreated AS DATE) = CAST([DOBS].[Date_Dispatched] AS DATE)
					AND
					[DOD].[RowStatus] = 1
			) DODdelivery
		OUTER APPLY
			(
				...
				WHERE
					...
					[DO].[Guide_Number] = [DOD].[Guide_Number]
					AND
					[DOD].[StatusOrderId] = @STATUS_RETURNED_ID
					AND 
					CAST([DOD].DateCreated AS DATE) = CAST([DOBS].[Date_Dispatched] AS DATE)
					AND
					[DOD].[RowStatus] = 1
		...
		OUTER APPLY
			(
				...
				WHERE
					...
					[DO].[Guide_Number] = [DOD].[Guide_Number]
					AND
					[DOD].[StatusOrderId] = @STATUS_TRANSFERED_EX_ID
					AND 
					CAST([DOD].DateCreated AS DATE) = CAST([DOBS].[Date_Dispatched] AS DATE)
					AND
					[DOD].[RowStatus] = 1
			) DODtransfer
		WHERE CAST(	[DOBS].[Date_Dispatched] AS DATE) BETWEEN @StartDate AND @EndDate
		GROUP BY	[DOBS].[ID],
