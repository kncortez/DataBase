...
BEGIN
    BEGIN TRY
        DECLARE @InitDate AS DATETIME,                       --Fecha de inicio del periodo a considerar / Opcional - valor defecto fecha inicial del mes
                @EndDate AS DATETIME,                        --Fecha final del periodo a considerar / Opcional - valor por defecto fecha final del mes
                @user AS NVARCHAR(70) = N'SYS-MANUALINVJOB'; --Usuario a reportar en los registros de guias / Opcional - valor por defecto SYS-MANUALINV + <fecha fin del mes>
        DECLARE @factor DECIMAL(10, 6) = CAST((2.00 / 24.00) AS DECIMAL(10, 6));

        SELECT @user = CASE
                           WHEN @user IS NULL
                                OR @user = '' THEN
                               'SYS-MANUALINVJOB' + FORMAT(EOMONTH(GETDATE()), 'yyyyMMdd')
                           ELSE
                               @user
                       END,
               @InitDate = CASE
                               WHEN @InitDate IS NULL
                                    OR @InitDate = '' THEN
        ...
        SELECT 'FD' AS Guide_Serie,
               ...
               @user AS UserCreated,
               CASE
                   WHEN DATEPART(MILLISECOND, dor.DateCreated) >= 500 THEN
                       DATEADD(SECOND, 1, dor.DateCreated)
                   ELSE
                       DATEADD(MILLISECOND, -DATEPART(MILLISECOND, dor.DateCreated), dor.DateCreated)
               END DateCreated,
        ...
        (
            SELECT TOP 1
                   dod.DateCreatedInSystem AS fechaCreacion
        ...
        (
            ...
            WHERE dod.Guide_Serie = dor.Guide_Serie
              ...
              AND CAST(dor.DateCreated AS DATE)
              BETWEEN CAST(@InitDate AS DATE) AND CAST(@EndDate AS DATE)
        ...
        SELECT Guide_Serie,
               ...
               UserCreated,
               CASE
                   WHEN CAST(DATEDIFF(DAY, dor.DateCreated, (dor.DateCreatedInSystem)) AS INT) = 1 THEN
                       CONVERT(NVARCHAR, dor.DateCreated + @factor, 20)
                   ELSE
                       CONVERT(
                                  NVARCHAR,
                                  (dor.DateCreated + DATEDIFF(DAY, dor.DateCreated, (dor.DateCreatedInSystem))) - 1,
                                  20
                              )
               END DateCreated,
               CASE
