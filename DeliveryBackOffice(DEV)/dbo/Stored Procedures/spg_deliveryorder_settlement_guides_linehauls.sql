...
BEGIN
    ...
    FROM 
        ...
        INNER JOIN 
			...
			ON 
				sm.IdServiceManagement = pbs.ServiceManagmentId
        INNER JOIN 
			DeliveryBackOffice.dbo.SettlementByPickup sbp WITH(NOLOCK)
			ON 
				SBP.SequenceCode = @IdManifest
				AND 
				SBP.ServiceManagmentId = SM.IdServiceManagement
				AND 
				SBP.SubTypeServiceManagmentId = 4
        INNER JOIN 
			DeliveryBackOffice.dbo.DeliveryOrderPiece PC WITH(NOLOCK)
			ON 
				PBS.GuidePieceId = PC.GuidePiece
        INNER JOIN 
			DeliveryBackOffice.dbo.DeliveryOrder DO WITH(NOLOCK)
			ON 
				DO.Guide_Serie = PC.GuideSerie 
				AND 
				DO.Guide_Number = PC.GuideNumber
    WHERE 
