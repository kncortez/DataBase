...
BEGIN
	...
	BEGIN TRY
		...
		SET @CustomerCodeApp = (SELECT IdCustomer FROM Ecommerce WHERE UserKey = @CodeApp AND EcommerceStatus = 1)

		--- Validar que el OriginCode de los servicios pertenezcan al Customer del CodeApp
		IF ( SELECT
				COUNT(1)
			FROM VisitPointClient vpc
			INNER JOIN @ServiceRoutes sr
				ON sr.OriginCode = vpc.CodeOfReference
			WHERE vpc.CustomerID = @CustomerCodeApp 
		...
		BEGIN
			...
			IF ( SELECT TOP 1
					COUNT(1)
				FROM Township tw WITH (NOLOCK)
				INNER JOIN @ServiceRoutes sr
					ON sr.HeaderCodeTownship = tw.HeaderCode
			...
			BEGIN
				...
				BEGIN
					...
					SELECT 
						...
						tw.IdTownship, --ReceiverIdTownship
						@system,
						@module,
						'FDD',
						sr.Username
					...
					INNER JOIN Township tw WITH (NOLOCK)
						ON tw.HeaderCode = sr.HeaderCodeTownship
					INNER JOIN Province pv WITH (NOLOCK)
						ON pv.IdProvince = tw.IdProvince
					...
					SELECT 
						...
						1,
						TRY_CONVERT(DECIMAL(18,15), sr.Latitude),
						TRY_CONVERT(DECIMAL(18,15), sr.Longitude),
						sr.Accuracy,
						1,
						'SYS-HERMESROUTES',
						GETDATE()
			...
			BEGIN
				SELECT
					0 AS 'StatusCode'
				   ,'HeaderCodeTownship no válido.' AS 'Description'
				   ,@@TRANCOUNT AS 'NumTransferID'
		...
		BEGIN
			SELECT
				0 AS 'StatusCode'
			   ,'El OriginCode no pertenece al cliente de CodeApp.' AS 'Description'
			   ,@@TRANCOUNT AS 'NumTransferID'
