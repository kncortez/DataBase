	...
	BEGIN TRY
		...
		BEGIN
			...
			INSERT INTO
				...
				(ResponseOrder, CoDProcessName, DeliveryBankId, ExecutionDate, ExecutionTime, ExecutionIsPending, ExecutionHasStarted, ExecutionHasCompleted)
			SELECT
				CDE.IdCoDDailyExecution, CDE.CoDProcessName, CDE.DeliveryBankId, CDE.ExecutionDate, CDE.ExecutionTime, CDE.ProcessPending, CDE.ProcessStarted, CDE.ProcessFinished
			FROM
				[DeliveryBackOffice].[dbo].[CoDDailyExecution] CDE WITH(NOLOCK)
			WHERE
				CDE.ExecutionDate = CAST(GETDATE() AS DATE)
		...
		BEGIN
			...
			BEGIN
				...
				OUTPUT
					inserted.IdCoDDailyExecution, inserted.CoDProcessName, inserted.DeliveryBankId, inserted.ExecutionDate, inserted.ExecutionTime, inserted.ProcessPending, inserted.ProcessStarted, inserted.ProcessFinished
					INTO @ResponseExecutionQueue(ResponseOrder, CoDProcessName, DeliveryBankId, ExecutionDate, ExecutionTime, ExecutionIsPending, ExecutionHasStarted, ExecutionHasCompleted)
				SELECT
