...
SELECT --TOP 100  
 ...
 (  
        ...
        WHERE dod10.Guide_Number = do.Guide_Number  
    ...
    (  
        ...
        WHERE dod1.Guide_Number = do.Guide_Number  
    ...
    (  
        ...
        WHERE dod2.Guide_Number = do.Guide_Number  
    ...
    (  
        ...
        WHERE dod3.Guide_Number = do.Guide_Number  
    ...
    (  
        ...
        WHERE dod4.Guide_Number = do.Guide_Number  
    ...
    (  
        ...
        WHERE dod5.Guide_Number = do.Guide_Number  
 ...
 (SELECT (CASE  
      ...
      SELECT COUNT(Guide_Number)  
        ...
        WHERE dod51.Guide_Number = do.Guide_Number  
 ...
 --,[Rack_Position] as [Ubicacion_Bodega]  
  ...
  ORDER BY da.Date_Created DESC  
    ...
    (  
        ...
        WHERE dod6.Guide_Number = do.Guide_Number  
    ...
    (ISNULL(  
     (  
         ...
         WHERE do.StatusOrderId NOT IN ( 5, 14, 22, 23 )  
               AND dod11.Guide_Number = do.Guide_Number /*and dod11.Guide_Serie = do.Guide_Serie*/  
  ...
  (  
   ...
   WHERE do.Guide_Serie = guiapagada.Guide_Serie   
    ...
    (  
        ...
        WHERE da1.Guide_Number = do.Guide_Number /*and da1.Guide_Serie = do.Guide_Serie*/  
    ) AS Manifiesto_Despacho,  
    (  
        SELECT TOP 1  
               DateCreated  
        FROM DeliveryBackOffice.dbo.DeliveryOrderDetail dod12 WITH (NOLOCK)  
        WHERE dod12.Guide_Number = do.Guide_Number  
...
SELECT Guide_Number FROM dbo.DeliveryOrder DOR  WITH (NOLOCK)  
 WHERE Guide_Number = do.Guide_Number  
 AND EXISTS  
 (  
  SELECT 1 FROM dbo.invoiceDetail IND  WITH(NOLOCK)  
  Inner JOIN DeliveryBackOffice.dbo.invoiceHeader INH WITH(NOLOCK) ON IND.dti_fk_header = INH.inv_pk_id  
  ...
  WHERE DOR.Guide_Number = IND.dti_fk_orderNumber  
  AND DOR.Guide_Serie = IND.dti_fk_orderSerie  
 )  
 ) > 0)  
 THEN 1  
    ELSE 0  
...
(  
 ...
 ISNULL((  
           ...
           FROM dbo.DeliveryOrder do98 WITH (NOLOCK)  
               LEFT JOIN dbo.Cost cst98 WITH (NOLOCK)  
                   ON cst98.GuideSerie = do.Guide_Serie AND cst98.GuideNumber = do.Guide_Number  
 ...
 ISNULL((  
           ...
           FROM dbo.DeliveryOrder do99 WITH (NOLOCK)  
               LEFT JOIN dbo.Cost cst99 WITH (NOLOCK)  
                   ON cst99.GuideSerie = do.Guide_Serie AND cst99.GuideNumber = do.Guide_Number  
 ...
 ISNULL((  
           ...
           FROM dbo.DeliveryOrder do97 WITH (NOLOCK)  
               LEFT JOIN dbo.Cost cst97 WITH (NOLOCK)  
                   ON cst97.GuideSerie = do.Guide_Serie AND cst97.GuideNumber = do.Guide_Number  
 ...
 (  
  ...
  WHERE dod15.Guide_Number = do.Guide_Number  
 ...
 (  
        ...
        WHERE dod34.Guide_Number = do.Guide_Number  
...
FROM [DeliveryBackOffice].[dbo].[DeliveryOrder] do WITH (NOLOCK) -- 1,197,011  
 ...
 LEFT JOIN(  
 SELECT MIN(PRF.Email) EMAIL, MIN(PRF.IdVisitPointByClientPortfolio) IdVisitPointByClientPortfolio , MIN(COALESCE(PRF.FirstName,'') + IIF(LEN(PRF.FirstName) = 0,'','') + COALESCE(PRF.SecondName,'') + IIF(LEN(PRF.SecondName)=0,'',' ') + COALESCE(PRF.LastNa
me,'') + IIF(LEN(PRF.LastName) = 0,'',' ') + COALESCE(PRF.SecondLastName,'')) FIRSTNAME , PRF.Phone    
 FROM   
...
--LEFT JOIN DeliveryBackOffice.dbo.DumpServiceCoverage dscd WITH(NOLOCK) ON dscd.HeaderCode = townd.HeaderCode  
 ...
 LEFT JOIN dbo.Cost cst WITH (NOLOCK)  
  ON cst.GuideSerie = do.Guide_Serie AND cst.GuideNumber = do.Guide_Number  
           AND cst.RowStatus = 1  
...
WHERE do.Guide_Number > 2301  
   ...
   AND do.DateCreated  >= DATEADD(DAY, -61, DATEDIFF(DAY, 0, GETDATE()))  
