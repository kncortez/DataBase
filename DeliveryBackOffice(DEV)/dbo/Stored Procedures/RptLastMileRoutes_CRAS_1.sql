...
-- =============================================
CREATE PROCEDURE [dbo].[RptLastMileRoutes_CRAS]
    @StartDate DATE
  , @EndDate DATE
AS
BEGIN
    ...
    SELECT ROW_NUMBER() OVER (ORDER BY [DOBS].[Date_Dispatched])                                               Renglon
         ...
         , ABS(SUM(   IIF((
                        ...
                        , 1
                        , 0)
                  ) - SUM(IIF(DSD.Guide_Returned = 1 AND DSD.RowStatus = 1, 1, 0))
              )                                                                                                Other_Guides_Checkpoint
         , ABS(SUM(   IIF((
                              DODdelivery.DeliveryExists IS NULL
                           AND DODreturn.ReturnExists IS NULL
                           AND DODtransfer.TransferExists IS NULL
                        ...
                        , 0)
                  ) - SUM(IIF(DSD.Guide_Returned = 1 AND DSD.RowStatus = 1, (DO.Pieces_Dry + DO.Pieces_Cold), 0))
              )                                                                                                Other_Pieces_Checkpoint
    ...
    GROUP BY [DOBS].[ID]
           ...
           , [CRT].[CodeRoute]
           , [CVH].[UnitNumber]
           , [DOBS].[StartingKilometers]) A1
	LEFT JOIN dbo.DeliveryAttempt AD ON ad.ID_Courier = A1.Courierman_Id AND ad.Date_Created  BETWEEN  @StartDate AND @EndDate
	LEFT JOIN dbo.ConfirmationOfIncidence cof ON cof.IdConfirmationOfIncidence = AD.ConfirmationOfIncidenceId  AND cof.CourierContempt = 1
