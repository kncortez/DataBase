...
-- =============================================
CREATE PROCEDURE [dbo].[GetBillingGuideDetail_BNHL]
...
BEGIN
	...
	PRINT 'Primera validación monto'
	PRINT @Amount
    ...
    BEGIN
		...
		BEGIN	
		PRINT 'rodondeo'
		PRINT ROUND(((@Amount * @PromoValue) / 100), 1)
			SET @Amount = (
				SELECT
					(
						CASE
							WHEN @ValueType = 'Porcentaje' COLLATE Latin1_General_CI_AI THEN 
								CASE
									WHEN @DiscountType = 'TOT' THEN
										@Amount - ROUND(((@Amount * @PromoValue) / 100), 1) --BNHL
									ELSE 
										@Amount
								END
							ELSE @Amount
