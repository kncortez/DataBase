...
BEGIN
    ...
    Declare  @IdWebhookEndpoint AS BIGINT =	(SELECT WebhookEndpointId FROM [dbo].[WebhookTrackingQueue] WHERE IdWebhookTrackingQueue = @IdWebhookTrackingQueue)
	...
	SELECT 
			...
			whEph.DateCreated,
			whEph.TokenCreated		
	FROM [dbo].[WebhookEndpointHeader] whEph
