...
BEGIN
	...
	INNER JOIN DeliveryBackOffice.dbo.SchedulePickup sp WITH(NOLOCK) ON sp.SchedulePickupId = fph.SchedulePickupId
	INNER JOIN DeliveryBackOffice.dbo.ServiceManagement sm WITH(NOLOCK) ON sm.IdSchedulePickup = sp.SchedulePickupId
	INNER JOIN DeliveryBackOffice.dbo.RouteAssigment ra WITH(NOLOCK) ON ra.IdRouteAssigment = sm.IdPuRouteAssigment
	INNER JOIN DeliveryBackOffice.dbo.CatRoute cr WITH(NOLOCK) ON cr.IdRoute = ra.IdRoute AND cr.RowStatus = 1
