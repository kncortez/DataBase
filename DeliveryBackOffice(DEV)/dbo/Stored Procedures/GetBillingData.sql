...
BEGIN
    ...
    SET @IdType =
    (
        SELECT IdCatInvoiceType FROM CatInvoiceType WHERE Name = 'Envío'
    );

    -- Table 0
    SELECT @IdCustomer  = cu.IdCustomer
         , @Email       = cu.RegexEmail
         , @Amount      = do.PriceShippment
         , @TypeService = do.TypeService
         , @IsCOD       = (CASE
                               WHEN do.Collect_OnDelivery > 0 THEN
                                   1
                               ELSE
                                   0
                           END
    ...
    (
        SELECT 1
        FROM dbo.invoiceHeader INH2 WITH (NOLOCK)
