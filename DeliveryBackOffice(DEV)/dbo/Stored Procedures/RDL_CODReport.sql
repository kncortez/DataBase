...
AS
BEGIN
--/****** Object:  StoredProcedure [dbo].[RDL_CODReport]    Script Date: 12/15/2023 2:44:52 PM ******/
...
SELECT DISTINCT -- TOP 10       
       ...
       (CASE
            WHEN ISNULL(CTV.IdCustomerType, CTM.IdCustomerType) = 2
                 AND KVP.IdKindOfVPClient <> 3 THEN
       ...
       IIF(INH.inv_cli_nit IS NULL, 'NO', 'SI') 'Facturado',
           ...
           (
               ...
               WHERE (
                         ...
                         (
                             ...
                             AND rac.RbcCodeOfReference IS NULL
                         )
                     )
       ...
       (
           ...
           FROM
           (
               SELECT rah.AdditionalWeightRate,
                      RbcCodeOfReference,
                      RbcIdCustomer
               FROM DeliveryBackOffice.dbo.RatebyCustomer rac WITH (NOLOCK)
                   LEFT JOIN DeliveryBackOffice.dbo.RateHeader rah WITH (NOLOCK)
       ...
       IIF(RTRIM(ISNULL(CTM.TaxIdentificationNumber, '')) <> '',
           CTM.TaxIdentificationNumber,
           IIF(RTRIM(ISNULL(CTV.TaxIdentificationNumber, '')) <> '', CTV.TaxIdentificationNumber, NULL)) 'NIT Cliente',
...
FROM DeliveryBackOffice.dbo.DeliveryOrder DOR WITH (NOLOCK)
	...
	(
        ...
        WHERE IdProduct = 1 --Pendiente cuando el pago se hizo en recolecciones 
           --AND ProductNumber = DOR.Guide_Serie + CAST(DOR.Guide_Number AS CHAR)
		   AND CST.GuideSerie = DOR.Guide_Serie
		   AND CST.GuideNumber = DOR.Guide_Number
    ...
    LEFT JOIN dbo.Township TONWO WITH (NOLOCK)
        ON TONWO.IdTownship = DOR.SenderIdTownship
           AND TONWO.TownshipStatus = 1
    LEFT JOIN dbo.Township TWNO WITH (NOLOCK)
        ON TWNO.TownshipName = DOR.Sender_Town
           AND TWNO.TownshipStatus = 1
    LEFT JOIN
    (
        SELECT CV.HeaderCode,
               MAX(CV.Hub) HUB
    ...
    LEFT JOIN DeliveryBackOffice.dbo.DeliveryBank DBA WITH (NOLOCK)
        ON DBA.Id_bank = DCBA.DCBA_Bank_Id
           ...
           AND DBA.Id_status = 1
    LEFT JOIN DeliveryBackOffice.dbo.StatusOrder STO
