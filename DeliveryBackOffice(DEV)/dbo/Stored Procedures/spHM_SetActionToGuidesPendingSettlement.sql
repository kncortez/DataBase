			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							INSERT INTO UnifiedRouteSettlementDetail (UnifiedRouteSettlementId, ServiceManagementId, GuideSerie, GuideNumber, ServiceSettlementAmount, ServiceCODSettlementAmount, IsArrival, IsReturn, IsDelivered, IsTransfered, RowStatus, TokenCreated, DateCreated, IsLost)
								VALUES (@IdUnifiedRouteSettlement, @ServiceManagementId, @GuideSerie, @GuideNumber, CASE WHEN @Action IN (1, 2) THEN IIF(@IsCollect = 1, @ServiceAmount, 0) ELSE 0 END, CASE WHEN @Action IN (1, 2) AND @IsLastMileReturn = 0 THEN @CODAmount ELSE 0 END, 0, 0, CASE WHEN @Action IN (1, 2) THEN 1 ELSE 0 END, CASE WHEN @Action IN (3) THEN 1 ELSE 0 END, 1, @Token, GETDATE(), CASE WHEN @Action IN (5) THEN 1 ELSE 0 END);
