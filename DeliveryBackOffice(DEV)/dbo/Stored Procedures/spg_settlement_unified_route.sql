...
-- Description:	<Obtiene información de la liquidación de rutas unificadas>
-- =============================================
CREATE PROCEDURE [dbo].[spg_settlement_unified_route]
...
BEGIN
	...
	WHERE urs.RowStatus = 1 AND urs.IdUnifiedRouteSettlement IN (SELECT DISTINCT
			Id
	...
	SELECT
	   ...
	   ,@TotalPiecesAct = SUM(urs.TotalPiecesMissing)
	FROM UnifiedRouteSettlement urs WITH (NOLOCK)
	WHERE urs.RowStatus = 1
	AND urs.IdUnifiedRouteSettlement IN (SELECT DISTINCT
			Id
		FROM @UnifiedRouteSettlementId)

	SELECT
		@TotalPiecesMissing = SUM(ursd.PiecesSettled)
	FROM UnifiedRouteSettlement urs WITH (NOLOCK)
	INNER JOIN UnifiedRouteSettlementDetail ursd WITH (NOLOCK)
	ON urs.IdUnifiedRouteSettlement = ursd.UnifiedRouteSettlementId
	AND ursd.RowStatus = 1 AND ursd.IsLost = 1
	WHERE urs.RowStatus = 1
	AND urs.IdUnifiedRouteSettlement IN (SELECT DISTINCT
			Id
		FROM @UnifiedRouteSettlementId)
	...
	SELECT TOP 1
	   ...
	   ,ISNULL(@TotalPiecesAct, 0) + ISNULL(@TotalPiecesMissing, 0) TotalPiecesMissing
	FROM UnifiedRouteSettlement urs WITH (NOLOCK)
