...
GO
/****** Object:  StoredProcedure [dbo].[spg_dsms_PhoneBook]    Script Date: 08/09/2021 13:55:54 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[spg_dsms_PhoneBook] 
...
BEGIN
	...
	declare @ToUpdate as table (_Series nvarchar(50), _Number int)
	  ...
	  and   CAST(do.DateCreatedInSystem as date)>CAST('2021-05-03' as date)
	  and (
		(do.StatusOrderId=11 and @ElementId = 1001 and ISNULL(ss.Sent,0) = 0 and ISNULL(ss.SentTypeStatus,0) <> 1) 
		or 
		(do.StatusOrderId=2 and @ElementId = 1002 and ISNULL(ss.Sent,0) = 0 and ISNULL(ss.SentTypeStatus,0) = 0) 
	  )
	  and not exists 
	  (select 1 from DeliveryBackOffice.dbo.DeliveryOrderDetail DOR2
	  where do.Guide_Serie = DOR2.Guide_Serie 
	  and do.Guide_Number = DOR2.Guide_Number
	  and StatusOrderId in (5,4)
	  )
	...
	SELECT
		...
		do.Guide_Serie,
		do.Guide_Number
		,ISNULL(IIF(do.Sender_ID <> 0,VPC.DescriptionOfClient,do.Sender_FirstName +' ' + do.Sender_LastName ),'') 
		,' https://forzadelivery.com/rastreo/' + do.Guide_Serie 
		  + convert(varchar,do.Guide_Number)
		, IIF(SDFG.GuideToken IS NOT NULL, CONCAT( ' https://forzadelivery.com/portal/landing-delivery/' , SDFG.GuideToken , '/' , do.Guide_Serie 
		  , convert(varchar,do.Guide_Number)),'')
	FROM DeliveryBackOffice.dbo.DeliveryOrder do WITH(NOLOCK)
	left join @ToUpdate tu on do.Guide_Serie=tu._Series and do.Guide_Number=tu._Number
	left join DeliveryBackOffice.dbo.VisitPointClient VPC ON VPC.CodeOfReference = do.Sender_ID
	left join DeliveryBackOffice.dbo.ServiceDataForGuide SDFG ON do.Guide_Serie = SDFG.GuideSerie and do.Guide_Number = SDFG.GuideNumber and SDFG.IsDelivery = 1
