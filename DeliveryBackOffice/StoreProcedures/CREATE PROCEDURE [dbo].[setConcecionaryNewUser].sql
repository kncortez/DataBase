...
GO
/****** Object:  StoredProcedure [dbo].[spws_SetHwNewCorporateUser]    Script Date: 15/03/2022 08:47:23 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	<Setup Usuario concesionario Portal Web>
-- =============================================


CREATE PROCEDURE [dbo].[setConcecionaryNewUser]
@UserCode BIGINT,
@UserName VARCHAR(50),
...
DECLARE @AccountType BIGINT = (SELECT TacIdTypeAccount FROM DeliveryBackOffice.dbo.CatTypeAccount WHERE TacName='Concesionario');

SELECT  @email = ISNULL(cu.ContactEmail,''), @nationality = ISNULL(cu.CountryID,'GT'), @firstName = vpc.DescriptionOfClient, @idCustomer = vpc.CustomerID 
...
IF EXISTS (SELECT TOP 1 * FROM DeliveryBackOffice.dbo.InternalUser WHERE IdUser = @UserCode )
BEGIN
...
IF(SELECT TOP 1 ISNULL(IdKindOfVPClient, -1) FROM DeliveryBackOffice.dbo.VisitPointClient WHERE IdVisitPointClient=@IdVisitPointClient)!=3

BEGIN

SELECT 'Código de punto de visita no es de tipo concesionario.'
...
INSERT INTO DeliveryBackOffice.dbo.Account
						...
						VALUES(CAST(concat('',ISNULL(@firstName,'')) AS VARCHAR(100)),@AccountType,1,'SYS-ADMIN',GETDATE(),@idCustomer,'C')
