...
GO
/****** Object:  StoredProcedure [dbo].[GetGuidesToPayCOD]    Script Date: 1/10/2021 12:05:09 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	bt.IdBatchCOD,
		   ...
		   (SELECT COUNT(1) FROM DeliveryOrderPiece WHERE GuideSerie = btd.GuideSerie AND GuideNumber = btd.GuideNumber) AS Pieces,
           btd.[Excluded]
		   ,CatConceptCODId,
		   btd.Comments
    FROM [dbo].[BatchDetailCOD] btd
        ...
        LEFT JOIN [dbo].[Township] twn
            ON CASE WHEN do.[ReceiverIdTownship] IS NULL THEN (
				...
				ELSE do.[ReceiverIdTownship] END = twn.[IdTownship]
	...
	ORDER BY bt.IdBatchCOD, bt.Date

	SELECT DISTINCT CommissionId, CommissionDate
