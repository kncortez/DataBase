...
GO
/****** Object:  StoredProcedure [dbo].[GetGuidesToPayCOD]    Script Date: 15/12/2021 08:51:30 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    SELECT bt.IdBatchCOD,
		   ...
		   ,(CASE WHEN vp.IdKindOfVPClient IN (2,6,8) THEN ''
			...
			COALESCE(dcba.DCBA_TaxId, cu.TaxIdentificationNumber) NIT
    FROM [dbo].[BatchDetailCOD] btd
		...
		LEFT JOIN [dbo].DeliveryCustomerBankAccount dcba
