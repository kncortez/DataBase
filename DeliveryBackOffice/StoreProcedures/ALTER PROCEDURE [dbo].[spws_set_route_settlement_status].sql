...
GO
/****** Object:  StoredProcedure [dbo].[spws_set_route_settlement_status]    Script Date: 11/02/2022 11:55:47 ******/
SET ANSI_NULLS ON
GO
        ...
        BEGIN
			...
			BEGIN
				...
				WHERE do.GuideSerie = @GuideSerie
					...
					AND do.StatusOrderId IN (7,11,5)

				IF @Status IS NOT NULL
					SET @Description = CONCAT('La pieza se encuentra en estado: ',@Status,'.')
				ELSE 
					IF EXISTS (SELECT 1 
						FROM DeliveryOrderPiece 
						WHERE GuideSerie = @GuideSerie 
						...
						SET @Description = 'Error: INSERT INTO TransactionalBackbone.'
					ELSE
			...
			ELSE
			IF @RModified = 0
				SET @Description = 'Error: INSERT INTO DeliveryOrderPiece.'
			ELSE 
			IF @RModified2 = 0
				SET @Description = 'Error: INSERT INTO DeliveryOrderDetail.'
