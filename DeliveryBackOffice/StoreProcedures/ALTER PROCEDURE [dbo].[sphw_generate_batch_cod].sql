...
GO
/****** Object:  StoredProcedure [dbo].[sphw_generate_batch_cod]    Script Date: 25/10/2021 10:18:15 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--EXEC  [dbo].[sphw_generate_batch_cod] 33,'8'
ALTER PROCEDURE [dbo].[sphw_generate_batch_cod]
	@IdBankParam INT,
	...
	BEGIN TRY
		...
		BEGIN
			SELECT @ProductNumber = (SELECT STUFF((SELECT /*TOP 50*/ ',' + CONCAT(pg.GuideSerie, pg.GuideNumber) 
													...
													AND ISNULL(do.IdCustomer, vpc.CustomerID) IN ( 370, 826, 57, 5688,7937,1038,6900,3267,527,7025,4851)
													--AND ISNULL(do.IdCustomer, vpc.CustomerID) IN (2662,505)
													--AND cus.IdCustomer IN (7937)
													AND pg.Date >'2021-09-29 00:00:00.000'
													--AND pg.Date >'2021-10-24 17:00:00.000'
		...
		BEGIN
			SELECT @ProductNumber = (SELECT STUFF((SELECT /*TOP 50*/ ',' + CONCAT(pg.GuideSerie, pg.GuideNumber) 
													...
													AND pg.BatchCODIdCommission IS NULL
													AND pg.RowStatus = 1
													AND (ISNULL(dcba.DCBA_Bank_Id,cus.CODAccountBankID) NOT IN (SELECT PayingBank
																					FROM DeliveryBackOffice.dbo.DeliveryBank
																					WHERE Id_country = @IdCountry
																					AND Id_status = 1
																					AND PayingBank <> @BankBAC
													    ...
													    AND cus.IdCustomer IN ( 370, 826, 57, 5688,7937,1038,6900,3267,527,7025,4851)
														--AND cus.IdCustomer IN (2662,505)
		...
		IF OBJECT_ID('tempdb.dbo.#TableAmountCOD', 'U') IS NOT NULL DROP TABLE #TableAmountCOD;
		...
		ORDER BY cus.IdCustomer,
			...
			CREATE NONCLUSTERED INDEX IX_TACT_ISINCTPCP ON #TableAmountCODTemp 
			...
			CREATE NONCLUSTERED INDEX IX_TFPT_GSGNCABI ON #TableForzaPaymentTemp 
...
WHERE tact.CODtoPay > 0;
			...
			CREATE NONCLUSTERED INDEX IX_TCPT_GSGNCABI ON #TableCustomerPaymentTemp 
				([GuideSerie],[GuideNumber],[CreditAccountId],[BankId]);
			...
			IF ((SELECT COUNT(1)
				 FROM #TableCustomerPaymentTemp) > 0)
			BEGIN
