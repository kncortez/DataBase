...
GO
/****** Object:  StoredProcedure [dbo].[sphw_generate_batch_cod]    Script Date: 1/10/2021 10:46:07 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN TRY
		...
		IF (@IdBankParam IN (SELECT PayingBank
							 ...
							 GROUP BY PayingBank))
		BEGIN
			SELECT @ProductNumber = (SELECT STUFF((SELECT top 3/*TOP 50*/ ',' + CONCAT(pg.GuideSerie, pg.GuideNumber) 
		...
		ORDER BY cus.IdCustomer,
			...
			WHERE (tact.Commision + IIF(do.IsCollect = 1, tact.Price, 0)) > 0
			AND tact.CODtoPay > 0
				...
				BEGIN
					...
					INSERT INTO DeliveryBackOffice.dbo.BatchDetailCOD 
							(BatchCODId, GuideSerie, GuideNumber, CatDebitAccountCODId, 
							 ...
							 Excluded, 
							 Comments,
							 BankName, TypeAccountName, AccountNumber, AccountName,CODCommissionPercentage)
					SELECT @NewIdBatchCODCustomer, tcpt.GuideSerie, tcpt.GuideNumber, tcpt.CatDebitAccountCODId, 
						   ...
						   tcpt.BankName, tcpt.TypeAccountName, tcpt.AccountNumber, tcpt.AccountName, CODRate
