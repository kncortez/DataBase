...
/****** Object:  StoredProcedure [dbo].[sps_proof_ondelivery]    Script Date: 17/01/2021 14:17:29 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[sps_proof_ondelivery]
	...
	@Accuracy NVARCHAR(20),
	 @CollectOnDelivery decimal(18,2),
	 @Collect decimal(18,2),
	 @PayCash decimal(18,2),
	 @PayTarjet decimal(18,2),
 	 @TicketPOS nvarchar(50),
	 @Autorizations nvarchar(50),
...
BEGIN
	...
	BEGIN TRANSACTION
		...
		BEGIN TRY
			...
			INSERT INTO DeliveryBackOffice.dbo.DeliveryProof (Guide_Serie, Guide_Number, Date_Photo, Proof_Dry, Proof_Cold, CollectOnDelivery,Collect,PayCash,PayTarjet,TicketPOS,Autoritations,TotalPay,IsDelivery) VALUES (@GuideSerie, @GuideNumber, GETDATE(), @PhotoDryVB, @PhotoColdVB,@CollectOnDelivery,  @Collect,@PayCash,@PayTarjet,@TicketPOS,  @Autorizations,  @TotalPay,@IsDelivery)
			SET @ID_Photo = SCOPE_IDENTITY()																																																												 
																																																																							  
			IF (@ID_Photo > 0)																																																																 
			BEGIN																																																																			 
				-- actualizar tabla de entregas																																																									
				UPDATE DeliveryBackOffice.dbo.DeliveryAttempt SET Delivered = 1, ID_Proof = @ID_Photo, Latitude = @Latitude, Longitude = @Longitude, Accuracy = @Accuracy, StatusPay = @StatusPay WHERE ID IN (SELECT ID FROM @Table)																				
