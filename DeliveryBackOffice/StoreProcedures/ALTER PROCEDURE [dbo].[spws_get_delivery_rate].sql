...
GO
/****** Object:  StoredProcedure [dbo].[spws_get_delivery_rate]    Script Date: 15/07/2021 10:58:50 ******/
SET ANSI_NULLS ON
GO
...
---------------------Determinar si existe exceso de libras ---------------------------------------------------------------------
	...
	IF OBJECT_ID('tempdb.dbo.#ParceWeigth', 'U') IS NOT NULL DROP TABLE #ParceWeigth;
	 ...
	 SELECT Item,
	   ...
	   into #ParceCode
	from DenariusDesktop_Dev.dbo.SplitUnlimited(@ParcelCode,',')
		...
		declare @jsonResult as nvarchar(max)
		 ...
		 set @jsonResult =( SELECT STUFF(( 
					...
					'"ServiceShortName":"' + isnull(tr.Service,'') + '",' +
						...
						'"Price":"' + convert(varchar(20), convert(decimal(12,1), CONVERT(decimal(12,1), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' ,tr.BaseRate + tr.IrregularPieceRate, 'false') ) +  CAST((dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' ,(tr.Discount * -1),'false')  ) AS decimal(18,1))  + convert(decimal(12,1),  dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' ,tr.FragilRate,'false')) + convert(decimal(12,1), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' ,tr.CollectedRate, 'false')) + convert(varchar(20), convert(decimal(12,1), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' ,tr.InsuranceRate , 'false')))  + convert(varchar(20), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' ,tr.CreditCardRate, 'false') ) + convert(varchar(20), convert(decimal(12,1), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' ,tr.OverWeightRate, 'false'))) + convert(decimal(12,1), dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' , (tr.BaseRate -  (tr.Discount)  + (tr.FragilRate) + tr.CollectedRate + tr.InsuranceRate  + tr.CreditCardRate + tr.OverWeightRate  + tr.IrregularPieceRate ), 'true')) )) + '",' +
						'"Currency":"' + @Currency + '",' +
						...
						iif((isnull(tr.Discount,0))>0, 
							...
							'"Price":"' + CONVERT(varchar, CAST((dbo.fnt_Iva_Calculator(@CalculateTaxes, 'GT' ,(tr.Discount * -1),'false')  ) AS decimal(18,1)) ) + '",'  +
							'"Currency":"' + COALESCE(@Currency,'') + '"' +
							'}' 
						,' '  ) +
