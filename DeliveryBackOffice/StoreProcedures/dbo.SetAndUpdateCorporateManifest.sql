...
GO
/****** Object:  StoredProcedure [dbo].[SetAndUpdateCorporateManifest]    Script Date: 25/04/2022 12:06:43 ******/
SET ANSI_NULLS ON
GO
...
ON
sr.GuideNumber = cmd.GuideNumber
AND sr.GuideSerie = cmd.GuideSerie
...
NULL,
1
FROM @servicesToAdd sta WHERE 
NOT EXISTS 
(SELECT cmd.GuideNumber FROM 
DeliveryBackOffice.dbo.CorporateManifestDetail cmd 
WHERE cmd.GuideNumber = sta.GuideNumber
AND cmd.RowStatus=1
)
...
WHERE cm.IdManifest= @IdManifest AND cm.ManifestSerie = @ManifestSerie
...
SELECT 500 'ErrorID',ERROR_MESSAGE() AS 'Error'
...
JOIN DeliveryBackOffice.dbo.CorporateManifestDetail cmd ON cmd.GuideNumber = do.Guide_Number AND cmd.GuideSerie= do.Guide_Serie AND cmd.RowStatus = 1 AND cmd.ManifestId = @IdManifest
WHERE do.Sender_ID = @CodeOfReference

SELECT
CONCAT(dop.GuideSerie,dop.GuideNumber,'-',dop.NoPiece) AS 'Piece'
FROM DeliveryBackOffice.dbo.DeliveryOrderPiece dop WITH (NOLOCK)
JOIN DeliveryBackOffice.dbo.CorporateManifestDetail cmd ON cmd.GuideSerie = dop.GuideSerie AND cmd.GuideNumber = dop.GuideNumber AND cmd.RowStatus = 1
WHERE cmd.ManifestId=@IdManifest
