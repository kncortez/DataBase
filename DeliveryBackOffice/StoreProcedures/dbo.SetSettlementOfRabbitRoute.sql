...
/****** Object:  StoredProcedure [dbo].[SetSettlementOfRabbitRoute]    Script Date: 3/17/2022 11:55:39 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SetSettlementOfRabbitRoute]
...
BEGIN
	...
	DECLARE @NewStatus INT = (SELECT TOP 1 SO.StatusOrderId FROM [DeliveryBackOffice].[dbo].[StatusOrder] SO WHERE SO.OrderDescription = 'Traslado a express center' COLLATE Latin1_General_CI_AI); 
	...
	BEGIN TRY
		...
		,@DPICOURIER = SR.CUI
		FROM DBO.SettlementPickupStation  SPS
		LEFT JOIN
		...
		BEGIN 
			...
			BEGIN
				...
				/*
				IF EXISTS(SELECT 1 FROM DBO.SettlementPickupStationDetail WHERE SettlementStationId=@IdSettlementPickupStation AND ServiceManagementId=@idservicemanagment)
				BEGIN

					UPDATE SPS
						SET SPS.Price=SPS.Price+BP.AmountToPay,
						SPS.TokenUpdated=@Token,
						SPS.DateUpdated=GETDATE()
					FROM DBO.SettlementPickupStationDetail SPS 
					JOIN @BrainProcessedGuides BP ON BP.GuideSerie=@GuideSerie AND BP.GuideNumber=@GuideNumber
					WHERE SettlementStationId=@IdSettlementPickupStation AND ServiceManagementId=@idservicemanagment
		...
		SET
			...
			,SPS.DateUpdated = GETDATE()
		FROM
			[DeliveryBackOffice].[dbo].[SettlementPickupStation] SPS
		WHERE
		...
		FROM
			[DeliveryBackOffice].[dbo].[SettlementPickupStation] SPS
			JOIN
				[DeliveryBackOffice].[dbo].[SettlementPickupStationDetail] SPSD
				ON
		...
		UPDATE
			SPSD
		SET
			SPSD.SettlementStationId = @IdExpressCenter
