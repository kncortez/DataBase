...
GO
/****** Object:  StoredProcedure [dbo].[spg_settlement_not_arrived_guides_PickUp]    Script Date: 14/02/2022 15:38:26 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	GROUP BY sbpd.GuideSerie
			,sbpd.GuideNumber),0) Pieces_Cold
	,ISNULL((SELECT
		COUNT(1)
	FROM SettlementByPickup sbp
	JOIN SettlementByPickupDetail sbpd
	...
	AND sbpd.GuideNumber = do.Guide_Number
	AND sbpd.IsPieceLiquidaded = 0
	AND (dop.IsDry IS NULL OR dop.IsDry = 1)
	GROUP BY sbpd.GuideSerie
			,sbpd.GuideNumber),0) Pieces_Dry
