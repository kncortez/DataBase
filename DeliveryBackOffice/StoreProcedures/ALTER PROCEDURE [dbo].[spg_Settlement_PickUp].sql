...
GO
/****** Object:  StoredProcedure [dbo].[spg_Settlement_PickUp]    Script Date: 19/08/2021 23:40:58 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	where ctr.CodeRoute = @Route and cast(rta.DateCreated as date) = @tiempo
	...
	*/
	...
	from RouteAssigment ra
		...
		--inner join  EventService es on (es.ServiceManagementId = sm.IdServiceManagement )
	where cr.CodeRoute = @Route and cast(ra.DateCreated as date) = @tiempo ---and es.ServiceStatusId = 3 and ord.StatusOrderId not in (11,10,7,5)  --
