...
GO
/****** Object:  StoredProcedure [dbo].[GetVisitPointByClientPortfolio]    Script Date: 24/11/2021 12:33:51 ******/
SET ANSI_NULLS ON
GO
                  ...
                  '"ContactName":"' + dbo.fnt_String_Escape(REPLACE(ISNULL(vcp.ContactName, ' '),'"',''),'json') + '",' +
								...
								ISNULL(	STUFF((    SELECT ',{ "IdBilling":"'  +  isnull( convert(varchar, SUB.BlpIdBilling) , ' ')  + '",' +
									...
									isnull(STUFF((    SELECT ',{ "Id":"'  +  isnull( convert(varchar, SUB.IdDeliveryFavCOD) , ' ')  + '",' +
														...
														'"NameBank":"'  + dbo.fnt_String_Escape( isnull( convert(varchar, [dbo].[fn_replace_special_characters](ISNULL(DB.Name,''))),'json') , ' ') + '",' +
