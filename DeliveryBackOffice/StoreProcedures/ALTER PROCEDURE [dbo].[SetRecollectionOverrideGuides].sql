...
GO
/****** Object:  StoredProcedure [dbo].[SetRecollectionOverrideGuides]    Script Date: 9/6/2021 3:21:52 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SetRecollectionOverrideGuides]
 @System						as int					= 1 
,@IdCustomer					as int					= 1  	  	
				...
				BEGIN
				   ...
				   --SELECT @ESTADO						
					...
					BEGIN--SI ES PARTE DE UN LOTTE DE PAGADO CON TARJETA
						...
						JOIN @TBGUIDES T
						ON T.GuideNumber = O.Guide_Number AND RTRIM(LTRIM(O.Guide_Serie)) = RTRIM(LTRIM(T.SerieGuide)) AND O.StatusOrderId  = 15
						WHERE T.ITERATOR = @IDENTYGUIDES
