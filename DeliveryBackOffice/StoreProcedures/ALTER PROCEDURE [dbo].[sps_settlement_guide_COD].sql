...
GO
/****** Object:  StoredProcedure [dbo].[sps_settlement_guide_COD]    Script Date: 26/01/2022 12:17:49 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[sps_settlement_guide_COD] @GuideSerie NVARCHAR(2),
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			INSERT INTO [dbo].[ProcessedGuideCOD] ([GuideSerie],
			[GuideNumber],
			[CourierManId],
			[Date],
			[BatchCODId],
		...
		BEGIN
			...
			UserCreated,
			DateCreated)
				VALUES (@GuideSerie, @GuideNumber, 24, @Token, GETDATE());
		...
		BEGIN
			...
			INSERT INTO [dbo].[Contingency] ([DeliveryOrderBySettlementId]
			, [Type]
			, [Value]
			, [Description]
			, [TokenCreated]
			, [DateCreated])
				VALUES (@IdDeliveryOrderBySettlement, @Type, @Value, @Description, @Token, GETDATE())
