...
GO
/****** Object:  StoredProcedure [dbo].[sps_get_payment_amount]    Script Date: 11/24/2020 12:59:50 AM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE  [dbo].[sps_get_payment_amount]
...
BEGIN
 ...
 SELECT STUFF((
  ...
  '"totalCollectedAmount":' + convert(varchar,sum(isnull(Det.Settlement_Collect_OnDelivery,0))) + ',' +
  '"Currency":"' + 'GTQ' + '"}' 
  ...
  '"Guide_Nuber":' + convert(varchar,o.Guide_Number) + ',' +
  ...
  '"CollectedAmount":' +  convert(varchar,isnull(Det.Settlement_Collect_OnDelivery,0)) + ',' +
  '"Comment":"' + ( 
	case
	...
	'"Account_Number":"' + ISNULL(c.DCBA_Num_account,'-1')  + '",' +
	'"Account_Name":"' + isnull(c.DCBA_Nom_account,'NO DISPONIBLE') + '",' +
	'"BankName":"' + ISNULL(b.Name,'NO DISPONIBLE') + '"}'
		FROM #listGuides_cod as guides
		inner join [DeliveryBackOffice].[dbo].[DeliveryOrder] o on o.Guide_Serie = guides.ItemSerie and o.Guide_Number = guides.ItemNumber
