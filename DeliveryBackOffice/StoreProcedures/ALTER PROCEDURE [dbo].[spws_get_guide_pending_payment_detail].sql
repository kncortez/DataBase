...
GO
/****** Object:  StoredProcedure [dbo].[spws_get_guide_pending_payment_detail]    Script Date: 2/09/2021 17:50:17 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	WHERE (UPPER(@ServiceType) = 'PICKUP'
			AND so.StatusOrderId NOT IN (1, 15))
	OR (UPPER(@ServiceType) = 'DELIVERY'
		AND so.StatusOrderId NOT IN (2, 3, 10, 11, 21))
	OR (UPPER(@ServiceType) = 'RETURN'
		AND so.StatusOrderId NOT IN (2, 3, 8, 10, 11, 12, 17, 21));
	...
	INNER JOIN DeliveryBackOffice.dbo.StatusOrder so
		ON do.StatusOrderId = so.StatusOrderId
	WHERE (UPPER(@ServiceType) = 'PICKUP'
			AND so.StatusOrderId IN (1, 15))
	OR (UPPER(@ServiceType) = 'DELIVERY'
		AND so.StatusOrderId IN (2, 3, 10, 11, 21))
	OR (UPPER(@ServiceType) = 'RETURN'
		AND so.StatusOrderId IN (2, 3, 8, 10, 11, 12, 17, 21));
