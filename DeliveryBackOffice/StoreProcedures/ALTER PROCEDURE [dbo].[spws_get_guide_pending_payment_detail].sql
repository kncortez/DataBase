...
BEGIN
	...
	WHERE (UPPER(@ServiceType) = 'PICKUP'
			AND so.StatusOrderId NOT IN (15))
	OR (UPPER(@ServiceType) = 'DELIVERY'
		AND so.StatusOrderId NOT IN (2, 3, 10, 11))
	OR (UPPER(@ServiceType) = 'RETURN'
		AND so.StatusOrderId NOT IN (2, 3, 8, 10, 11, 12, 17));
	...
	INNER JOIN DeliveryBackOffice.dbo.StatusOrder so
		ON do.StatusOrderId = so.StatusOrderId
	WHERE (UPPER(@ServiceType) = 'PICKUP'
			AND so.StatusOrderId IN (15))
	OR (UPPER(@ServiceType) = 'DELIVERY'
		AND so.StatusOrderId IN (2, 3, 10, 11))
	OR (UPPER(@ServiceType) = 'RETURN'
		AND so.StatusOrderId IN (2, 3, 8, 10, 11, 12, 17));*/
