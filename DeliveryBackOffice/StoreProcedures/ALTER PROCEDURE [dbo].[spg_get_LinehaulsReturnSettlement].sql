...
GO
/****** Object:  StoredProcedure [dbo].[spg_get_LinehaulsReturnSettlement]    Script Date: 2/08/2021 02:40:17 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[spg_get_LinehaulsReturnSettlement]
    @IdManifest AS INT,
    @subservice as INT
AS
BEGIN
    ...
    BEGIN
        SET @contador =
        (
            SELECT COUNT(DISTINCT pc.GuideNumber)
        ...
        from SettlementByPickup dbs
            join DeliveryBackOffice.dbo.SenderReceiver sr
                ON sr.ID = dbs.IdCourier
        WHERE dbs.SequenceCode = @IdManifest
              and dbs.SubTypeServiceManagmentId = @subservice


        INSERT INTO @GuidesDetail
        SELECT pc.GuideSerie + CONVERT(varchar, pc.GuideNumber) + '-' + CONVERT(varchar, pc.NoPiece) AS Guide,
               0 as Delivered,
               0.00 as COD
        FROM DeliveryOrder do
            INNER JOIN DeliveryOrderPiece pc
        ...
        SELECT DISTINCT
            ...
            pc.GuideSerie + CONVERT(varchar, pc.GuideNumber) + '-' + CONVERT(varchar, pc.NoPiece) AS Guide,
            0.00 as COD,
            0 as Delivered
        FROM DeliveryOrder do
            INNER JOIN DeliveryOrderPiece pc
    ...
    BEGIN
        SET @contador =
        (
            SELECT COUNT(DISTINCT sbpd.GuideNumber)
            ...
            where sbp.SequenceCode = @IdManifest
                  and sbp.SubTypeServiceManagmentId = @subservice
                  and (
                          sbpd.IsReturn is null
                          or sbpd.IsReturn = 0
                      )
                  and (
                          sbpd.IsPieceLiquidaded is null
                          or sbpd.IsPieceLiquidaded = 0
                      )
        )
