...
-- Description:	<Recupera información de las piezas ya escaneadas para despacho de la ruta seleccionada >
-- ============================================================================================

IF EXISTS (SELECT
			*
		FROM sysobjects
		WHERE ID = OBJECT_ID(N'[dbo].[spg_guides_per_route_linehauls]')
		AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
BEGIN
	DROP PROCEDURE [dbo].spg_guides_per_route_linehauls
END
...
BEGIN
	...
	SET @VALIDA = (	SELECT count(ISNULL(sbpud.IdSettlementByPickupDetail,0)) 
	...
	INNER join SettlementByPickup sbpu on sm.IdServiceManagement = sbpu.ServiceManagmentId
	INNER join SettlementByPickupDetail sbpud on sbpu.Id = sbpud.SettlementByPickupId 
