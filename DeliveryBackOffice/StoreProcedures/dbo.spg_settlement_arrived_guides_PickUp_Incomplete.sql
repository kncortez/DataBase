...
GO
/****** Object:  StoredProcedure [dbo].[spg_settlement_arrived_guides_PickUp_Incomplete]    Script Date: 28/04/2022 10:16:55 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	<Copia de spg_settlement_arrived_guides_PickUp cambiando validación para mostrar guías incompletas>
-- =============================================
CREATE PROCEDURE [dbo].[spg_settlement_arrived_guides_PickUp_Incomplete]
		@IdManifest INT
AS
BEGIN
	...
	INSERT INTO @temp
	SELECT
		CONCAT(do.Guide_Serie, do.Guide_Number) Guide_Code
	   ,ISNULL((SELECT
				COUNT(1)
	   ...
	   ,(SELECT
				DeliveryBackOffice.dbo.fn_get_rackposition(do.Guide_Serie, do.Guide_Number))
		Rack_Position
	   ,Collect_OnDelivery
	   ,X.Total Pieces_Total
	FROM (SELECT
			spd.GuideSerie
		   ,spd.GuideNumber
		   ,COUNT(1) Pieces
		...
		JOIN SettlementByPickupDetail spd
	...
	WHERE X.Pieces <> X.Total --Piezas incompletas
