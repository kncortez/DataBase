...
GO
/****** Object:  StoredProcedure [dbo].[sps_set_status_order_by_guide_linehauls]    Script Date: 21/04/2021 14:05:51 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	<Realiza las validaciones para evitar asignar erróneamente guías a rutas 
--               Linehauls y obtener la información correspondiente de las guías correctas>
-- ============================================================================================

IF EXISTS (SELECT
			*
		FROM sysobjects
		WHERE ID = OBJECT_ID(N'[dbo].[sps_set_status_order_by_guide_linehauls]')
		AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
BEGIN
	DROP PROCEDURE [dbo].[sps_set_status_order_by_guide_linehauls]
END

GO

CREATE PROCEDURE [dbo].[sps_set_status_order_by_guide_linehauls] @Guide_Serie AS VARCHAR(2), -- same guide for all numbers provided
		...
		BEGIN
			...
			BEGIN
				...
				SET @ExisteServicio = (SELECT
						...
						WHERE sm.IdPuRouteAssigment = @IdRouteASG AND sm.IdHubDestination = @HUB_Destino)
