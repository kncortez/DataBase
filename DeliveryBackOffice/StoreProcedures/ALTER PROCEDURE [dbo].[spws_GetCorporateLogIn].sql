...
GO
/****** Object:  StoredProcedure [dbo].[spws_GetCorporateLogIn]    Script Date: 31/12/2021 12:34:38 ******/
SET ANSI_NULLS ON
GO
    ...
    BEGIN
          ...
          --                                        AND us.UsrRowStatus = 1 order by cmo.ModOrder*/
                                        ...
                                        (
                                            ...
                                            INNER JOIN DeliveryBackOffice.dbo.InternalUser iu ON iu.RegisterUserID = us.UsrIdUser
                                            WHERE iu.UserName = @UserName order by cmo.ModOrder FOR XML PATH(''), TYPE
		   	...
		   	SELECT DISTINCT
					...
					'"SettlementName":"' + REPLACE(ISNULL(STL.Settlement,''),'"','') + '",' +
          ...
          '"HasRate":"'+ CONVERT(NVARCHAR,ISNULL(rc.[RbcRowStatus],'')) + '",'+
              ...
              '"ListCod":'+
                '[{'
											   ...
											   +'"Nit":"'+ISNULL(cu.[TaxIdentificationNumber],'')+'"',
								               +'}]'
              +
		            '}' 
			        FROM  
			        ...
			        JOIN DeliveryBackOffice.dbo.Settlement STL on VPC.IdSettlement = STL.IdSettlement
					JOIN DeliveryBackOffice.dbo.DumpServiceCoverage dsc ON dsc.IdSettlement = STL.IdSettlement AND dsc.RowStatus=1
			        JOIN DeliveryBackOffice.dbo.Township TWS on TWS.IdTownship = STL.IdTownship
			        JOIN DeliveryBackOffice.dbo.Province PRV ON PRV.IdProvince = TWS.IdProvince
              JOIN DeliveryBackOffice.dbo.Customer cu ON cu.IdCustomer = ac.IdCustomer AND cu.RowSatus = 1
			         ...
			         WHERE iu.UserName = @UserName AND (vpu.IdVisitPointClient = vpc.IdVisitPointClient OR vpu.IdVisitPointClient = vpc.CodeOfReference)
...
END
         ...
         SET @jsonResult =
                                        (
                                            SELECT STUFF(
                                        (
                                            SELECT '{"IdResult":200' + ',' + '"Token":"' + @Token + '",' + '"Modules":[' + @JsonModules + '],' + '"Accounts":[' + @JsonAccounts + '],' + '"Profile":[' + CASE WHEN @VERIFYUSER > 0 THEN @JsonProfile + ',' + @JsonProfileEXP ELSE @JsonProfile END + ']' + '}' FOR XML PATH(''), TYPE
                                        ).value('.', 'varchar(max)'), 1, 1, '')
                                        );
                                    END;
                                    ELSE
