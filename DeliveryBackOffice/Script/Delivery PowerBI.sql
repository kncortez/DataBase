...
SELECT --TOP 1000
	...
	(
        SELECT TOP 1
               CONVERT(VARCHAR, DateCreatedInSystem, 103)
    ...
    (
		ISNULL(CONVERT(NUMERIC(10,2), DeliveryBackOffice.dbo.fn_get_diff_minutes_without_sundays(
			(
                 SELECT TOP 1
                        DateCreatedInSystem
    ...
    --END [EstadoCOD],
	 (
		 ...
		 FROM DeliveryBackOffice.dbo.DeliveryOrderPaid guiapagada
...
SELECT CASE 
    WHEN ((
SELECT Guide_Number FROM dbo.DeliveryOrder DOR
...
FROM [DeliveryBackOffice].[dbo].[DeliveryOrder] do WITH (NOLOCK) -- 140,885
 ...
 --       ON KVPC.IdKindOfVPClient = vpc.IdKindOfVPClient
	...
	FROM 
	DBO.VisitPointByClientPortfolio PRF
...
WHERE do.Guide_Number > 2301
	 --do.Guide_Number = 403656
     -- AND DO.Guide_Number = 723895
	 -- AND (HB2.HUB IS NULL OR HB.HUB IS NULL)
	  ...
	  AND do.DateCreated >= '2021-01-01 00:00:00' --SOLICITADO POR VAN FILTRAR DESDE ESTE AÑO
	  --AND do.Guide_Number = 900000
	  ;
--and do.guide_number in (235203)--,176365,175961)
--and do.DateCreated >= '2021-01-01'
