...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN 
					SET @IsCanceled = 1
					SET @MessageToReturn = CONCAT('Ya existe una recolección identificada con el número ', @ServiceManagementFinded, ' asignado a ruta',@CodeRoute,', pero se encuentra cancelado. ¿Desea habilitarlo?')
				END
				ELSE IF @CatServiceStatusName = 'Incidencia'
				BEGIN 
					SET @IsIncidence = 1
					SET @MessageToReturn = CONCAT('Ya existe una recolección identificada con el número ', @ServiceManagementFinded, ' asignado a ruta',@CodeRoute,', pero con incidencia. ¿Desea crear otra solicitud? ')
				END
				ELSE IF @CatServiceStatusName = 'Recolectado'
				BEGIN
					SET @IsIncidence = 1
					SET @MessageToReturn = CONCAT('Ya existe una recolección identificada con el número ', @ServiceManagementFinded, ' recolectada. ¿Desea crear otra otra solicitud?')
				END
				ELSE IF @CatServiceStatusName = 'Reprogramado'
				BEGIN 
					SET @MessageToReturn = CONCAT('Ya existe una recolección identificada con el número ', @ServiceManagementFinded, ' en estado ', @CatServiceStatusName, '. No se puede crear la solicitud. ')
				END
				ELSE
				BEGIN 
					...
					BEGIN 
						...
						BEGIN
							SET @IsCanceled = 1
							SET @MessageToReturn = CONCAT('Ya existe una recolección identificada con el número ', @ServiceManagementFinded, ', pero se encuentra cancelada. ¿Desea habilitarla?')
						END
						ELSE
							SET @MessageToReturn = CONCAT('Ya existe una recolección identificada con el número ', @ServiceManagementFinded, '. No se puede crear la solicitud.')
					END
					ELSE
					BEGIN
						SET @MessageToReturn = CONCAT('Ya existe una recolección identificada con el número ', @ServiceManagementFinded, ' asignada a ruta',@CodeRoute,' en estado "', @CatServiceStatusName, '". No se puede crear la solicitud.')
					END
				END
			END
			ELSE
			BEGIN 
				...
				BEGIN
					SET @IsCanceled = 1
					SET @MessageToReturn = 'Ya existe una recolección , pero se encuentra cancelada. ¿Desea habilitarla?'
				END
				ELSE
					SET @MessageToReturn = CONCAT('Ya existe una recolección ',IIF(@ServiceManagementFinded IS NOT NULL, CONCAT('identificada con el número ',@ServiceManagementFinded),''),'. No se puede crear la solicitud.')
			END
	...
	IF (@IsDuplicated = 0 AND @SchedulePickupId = -1)
	OR (@SchedulePickupId <> -1 AND @IsCanceledPar = 0)
	BEGIN
		BEGIN TRY
			BEGIN TRANSACTION
	...
	BEGIN
		...
		END
	END
	ELSE IF @SchedulePickupId <> -1 AND @IsCanceledPar = 1
	BEGIN
