...
DECLARE @FrecuencyCOD INT = (SELECT
...
INSERT INTO #listGuides (Guide_Serie,
Guide_Number)
	SELECT
...
DECLARE @IdRateDefault INT = (SELECT TOP 1
		rh.RheId
	FROM DeliveryBackOffice.dbo.RateHeader rh WITH (NOLOCK)
	WHERE rh.RheRowStatus = 1
	AND rh.RheDefault = 1);
DECLARE @IdRate INT;
DECLARE @CODRateDefault DECIMAL(12, 2) = (SELECT
		CONVERT(DECIMAL(12, 2), ISNULL(cf.Value, '0')) val
	FROM DeliveryBackOffice.dbo.ConfigParams cf WITH (NOLOCK)
	WHERE cf.Name = 'CODRateDef'
	AND Status = 1);
DECLARE @CODExemptDefault DECIMAL(12, 2) = (SELECT
		CONVERT(DECIMAL(12, 2), ISNULL(cf.Value, '0')) val
	FROM DeliveryBackOffice.dbo.ConfigParams cf WITH (NOLOCK)
...
INSERT INTO #RevalueGuides (fila,
Guide_Serie,
Guide_Number)
	SELECT
		ROW_NUMBER() OVER (ORDER BY ord.Guide_Number ASC) AS fila
	   ,ord.Guide_Serie
	   ,ord.Guide_Number
	FROM #listGuides lst
...
SET @count = @count + 1;
            ...
            END;
-----------------------------------------------------------------------------------------------------
DECLARE @IdSegmentDefault INT = (SELECT TOP 1
		CrsId
...
SELECT
   ...
   ,MAX(Price) Price INTO #TableAmountCODTemp
FROM #TableAmountCOD
...
SELECT
	tact.Guide_Serie GuideSerie
   ,tact.Guide_Number GuideNumber
   ,(SELECT
			IdCatDebitAccountCOD
   ...
   ,CODRate
   ,tact.Price DiscountPrice INTO #TableForzaPaymentTemp
...
SET @Reference = @Reference + (SELECT
		COUNT(1)
...
DECLARE @MaxBatchNumber INT = 1 + (SELECT
		ISNULL(MAX(IdBatchCOD), 0)
...
INSERT INTO DeliveryBackOffice.dbo.BatchCOD (BankId,
BatchNumber,
BatchTimeRange)
...
-- OBTENCION DEL ID QUE CORRESPONDE AL LOTE CREADO
SELECT
	@NewIdBatchCODForza = SCOPE_IDENTITY();
...
IF ((@NewIdBatchCODForza IS NOT NULL)
	AND (@NewIdBatchCODForza > 0))
BEGIN
-- SE CREA EL DETALLE DEL LOTE PARA EL PAGO A FORZA DE LAS COMISIONES Y ENVIOS
INSERT INTO DeliveryBackOffice.dbo.BatchDetailCOD (BatchCODId,
GuideSerie,
GuideNumber,
CatDebitAccountCODId,
CreditAccountId,
...
DiscountPrice)
	...
	INNER JOIN ProcessedGuideCOD PGD WITH (NOLOCK)
...
BEGIN CATCH
SELECT
	'RollBackTransaction' AS message
   ,'FALSE' blnResult
