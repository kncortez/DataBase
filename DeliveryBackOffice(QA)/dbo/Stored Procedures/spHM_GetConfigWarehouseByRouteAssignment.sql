			...
			BEGIN
				...
				FROM 
					...
					(
						...
						FROM
							...
							OUTER APPLY (
								SELECT
									COUNT(DISTINCT URSDP.PieceNumber) PiecesMissingExtra
								FROM
									[DeliveryBackOffice].[dbo].[UnifiedRouteSettlementDetailPiece] URSDP WITH(NOLOCK)
								WHERE
									URSDP.ActCode IS NOT NULL
									AND
				...
				SELECT
				   ...
				   ,ISNULL(SUM(IIF(((URS.UserSettlement IS NULL AND (stsm.IdSubTypeServiceManagment IS NULL OR stsm.[Name] = 'Recolección') AND
					...
					, do.Pieces_Dry + do.Pieces_Cold, 0)), 0) - MAX(ISNULL(URS.TotalPiecesMissing,0)) TotalPhysicalPieces
				   ,ISNULL(SUM(IIF(URS.UserSettlement IS NULL AND (stsm.IdSubTypeServiceManagment IS NULL OR stsm.[Name] = 'Recolección') AND
					...
					so.OrderDescription NOT IN ('Paquete Extraviado', 'Entregado', 'Entregado En Express Center', 'COD liquidado', 'COD pagado', 'Devuelto', 'Devuelto en Express Center', 'Intento de entrega fallida', 'Traslado a Express Center')
