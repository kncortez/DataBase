...
BEGIN
    ...
    DECLARE @Counter INT =
            (
                ...
                WHERE cm.CodeOfReferenceId = @CodeOfReference
                      AND cm.RowStatus = 1
                      AND (CONVERT(DATE, cm.DateCreated)
    ...
    BEGIN
        ...
        (
            SELECT STUFF(
                            (
                                SELECT ',{' + '"DateCreated":"' + CONVERT(NVARCHAR(100), DateCreated, 23) + '",'
                                       + '"IdManifest":' + CONVERT(NVARCHAR(MAX), cm.IdManifest) + ',' + '"Manifest":"'
                                       + CONCAT(cm.ManifestSerie, CONVERT(NVARCHAR(MAX), cm.IdManifest)) + '",'
                                       + '"GuideCounter":'
                                           ...
                                           SELECT STUFF(
                                                           (
                                                               SELECT ',{' + '"StatusDescription":"'
                                                                      + so.OrderDescription + '",' + '"Quantity":'
                                                                      + CONVERT(NVARCHAR(MAX), COUNT(do.Guide_Number))
                                                                      + '}'
                                                               FROM DeliveryBackOffice.dbo.CorporateManifestDetail cmd WITH (NOLOCK)
    ...
    BEGIN
        ...
        (
            SELECT STUFF(
                            (
                                SELECT ',{									
								"Message":"No se encontraron manifiestos asociados.",' + '"Status": 400' + '}'
                                FOR XML PATH(''), TYPE
                            ).value('.', 'varchar(max)'),
                            1,
                            1,
