...
BEGIN
	...
	DECLARE @StatusOrderDestroyed INT = (SELECT so.StatusOrderId FROM StatusOrder so WHERE so.OrderDescription = 'Paquete destruido')
					...
					WHEN do.IsLastMileReturn = 1 THEN CASE
							WHEN (SELECT
								...
								>= (SELECT TOP 1
									...
									ORDER BY rc.RbcCodeOfReference DESC) THEN 3 --'IsBazar'
							ELSE 4 --'IsReturn'
						END
					ELSE CASE
							WHEN (SELECT
