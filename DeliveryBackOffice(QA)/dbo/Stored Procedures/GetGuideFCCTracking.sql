...
BEGIN
	...
	DECLARE @HubName NVARCHAR(50) = (
		SELECT ISNULL(vpc.DescriptionOfClient, (SELECT TOP (1) hub.HubAbbreviation 
...
FROM DeliveryBackOffice.dbo.DeliveryOrderDetail DOD WITH(NOLOCK)
		 JOIN DeliveryBackOffice.dbo.DeliveryOrder DOR WITH(NOLOCK) ON DOR.Guide_Number = DOD.Guide_Number
		 AND DOD.StatusOrderId = DOR.StatusOrderId
	LEFT JOIN DenariusUser_Dev.dbo.LGN_LogByToken token  WITH (NOLOCK) ON DOD.UserCreated = token.SSN_IdToken -- OrdChkPnt.Token = token.SSN_IdToken
	...
	LEFT JOIN DeliveryBackOffice.dbo.TokenLog tl WITH(NOLOCK) ON token.SSN_IdToken = tl.TknIdToken
