...
BEGIN
    ...
    FROM DeliveryBackOffice.dbo.DeliveryOrderBySettlement DOBS
        LEFT JOIN DeliveryBackOffice.dbo.DeliverySettlementDetail DSD
        ...
        LEFT JOIN DeliveryBackOffice.dbo.DeliveryOrder DO
            ON DO.Guide_Serie = DSD.Guide_Serie
               ...
               AND DSD.RowStatus = 1
        LEFT JOIN DeliveryBackOffice.dbo.SenderReceiver SRE
            ON SRE.ID = DOBS.ID_Courier
        LEFT JOIN DeliveryBackOffice.dbo.CatTypeSenderReceiver TYSRE
        ...
        LEFT JOIN DeliveryBackOffice.dbo.HubLogistics HBL
            ON HBL.IdHubLogistic = SRE.HubLogisticId
               AND HBL.HubStatus = 1
        LEFT JOIN DeliveryBackOffice.dbo.HubByRegion HBR
            ON SRE.HubLogisticId = HBR.HubLogisticId
               AND HBR.RowStatus = 1
        LEFT JOIN DeliveryBackOffice.dbo.CatRegion CR
            ON HBR.RegionId = CR.IdCatRegion
               AND CR.RowStatus = 1
        LEFT JOIN DeliveryBackOffice.dbo.CatRoute CRT
            ON DOBS.CatRouteId = CRT.IdRoute
               AND CRT.RowStatus = 1
        LEFT JOIN DeliveryBackOffice.dbo.CatVehicle CVH
