	...
	BEGIN
		BEGIN TRY
			...
			INSERT INTO @GuideStatusResponseTable
				(GuideSerie, GuideNumber, GuideStatusId, GuideStatus, GuideStatusChange)
			SELECT
				WTQ.GuideSerie
				,WTQ.GuideNumber
				,SO.StatusOrderId
			...
			BEGIN
				...
				BEGIN
					...
					FROM
							...
							FROM
								...
								LEFT JOIN
									...
									ON 
										DA.ID_Proof = DP.ID
							WHERE
								GSRT.GuideSerie = DA.Guide_Serie
								AND 
						...
						LEFT JOIN 
							...
							ON 
								GSRT.GuideSerie = DO.Guide_Serie 
								AND 
								GSRT.GuideNumber = DO.Guide_Number
				...
				END
				ELSE IF(@StatusId IN(12, 45)) /* Intento de entrega fallida */
				BEGIN
