...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @CreditAccountId = DCBA_Id, @CreditAccountName = DCBA_Nom_account
		...
		DECLARE @FrecuencyCOD INT = (SELECT CatBatchFrequencyCODId FROM CatBatchFrequencyCOD WHERE Name = 'Diaria')
	
		IF (@IdBankParam IN (SELECT PayingBank
							 ...
							 GROUP BY PayingBank))
		BEGIN
			SELECT @ProductNumber = (SELECT STUFF((SELECT /*TOP 50*/ ',' + CONCAT(pg.GuideSerie, pg.GuideNumber) 
													...
													LEFT JOIN dbo.VisitPointClient vpc ON vpc.CodeOfReference = do.Sender_ID
													LEFT JOIN dbo.Customer cus ON cus.IdCustomer = ISNULL(do.IdCustomer, vpc.CustomerID)
		...
		BEGIN
			SELECT @ProductNumber = (SELECT STUFF((SELECT /*TOP 50*/ ',' + CONCAT(pg.GuideSerie, pg.GuideNumber) 
													...
													AND pg.BatchCODIdCommission IS NULL
													AND pg.RowStatus = 1
													AND ((COALESCE(VPO.CODAccountBankID , cus.CODAccountBankID,dcba.DCBA_Bank_Id )) NOT IN (SELECT PayingBank
																					FROM DeliveryBackOffice.dbo.DeliveryBank
																					WHERE Id_country = @IdCountry
																					AND Id_status = 1
																					AND PayingBank <> @BankBAC
		...
		IF OBJECT_ID('tempdb.dbo.#listGuidesDaily', 'U') IS NOT NULL DROP TABLE #listGuidesDaily;
		IF OBJECT_ID('tempdb.dbo.#TempDataDaily', 'U') IS NOT NULL DROP TABLE #TempDataDaily;
		IF OBJECT_ID('tempdb.dbo.#CODDataDaily', 'U') IS NOT NULL DROP TABLE #CODDataDaily;
		IF OBJECT_ID('tempdb.dbo.#RevalueGuidesDaily', 'U') IS NOT NULL DROP TABLE #RevalueGuidesDaily;
		IF OBJECT_ID('tempdb.dbo.#PendingPaymentTempDaily', 'U') IS NOT NULL DROP TABLE #PendingPaymentTempDaily;
		IF OBJECT_ID('tempdb.dbo.#TableAmountCODDaily', 'U') IS NOT NULL DROP TABLE #TableAmountCODDaily;
		IF OBJECT_ID('tempdb.dbo.#TableAmountCODDailyTemp', 'U') IS NOT NULL DROP TABLE #TableAmountCODDailyTemp;
		IF OBJECT_ID('tempdb.dbo.#TableCustomerPaymentTempDaily', 'U') IS NOT NULL DROP TABLE #TableCustomerPaymentTempDaily;
		IF OBJECT_ID('tempdb.dbo.#TableForzaPaymentTempDaily', 'U') IS NOT NULL DROP TABLE #TableForzaPaymentTempDaily;
		...
		BEGIN
			...
			LEFT JOIN	 dbo.VisitPointConfiguration VPO ON VPO.VisitPointID = VPC.CodeOfReference 
		...
		ORDER BY cus.IdCustomer,
			...
			SELECT Guide_Serie, Guide_Number, Collect_OnDelivery, IDCUSTOMER, CODRate,
				   CODExempt, Commision, SUM(DeliveryPrice) DeliveryPrice, SUM(ISNULL(CODPaid, 0)) CODPaid, ReturnRates,
			...
			GROUP BY Guide_Serie, Guide_Number, Collect_OnDelivery, IDCUSTOMER, CODRate,
				   CODExempt, Commision, ReturnRates, Id_bank, [Name], 
				   DCBA_Id, DCBA_Num_account, DCBA_Nom_account, DCBA_BankAccountType
			ORDER BY Guide_Number;

			CREATE NONCLUSTERED INDEX IX_TACT_ISINCTPCP ON #TableAmountCODDailyTemp 
				([Guide_Serie],[Guide_Number],[CODtoPay],Commision,[Price]);
			...
			CREATE NONCLUSTERED INDEX IX_TFPT_GSGNCABI ON #TableForzaPaymentTempDaily 
				([GuideSerie],[GuideNumber],[CreditAccountId],[BankId]);

			SET @Reference = @Reference + (SELECT COUNT(1)
										   FROM #TableForzaPaymentTempDaily);
       ...
       ((ROW_NUMBER() OVER (ORDER BY tact.Guide_Number)) + IIF(@IdBankParam = @BankBAC, ISNULL(@Reference, 0), 0)) Reference,
       tact.Name BankName,
...
WHERE tact.CODtoPay > 0;
			...
			CREATE NONCLUSTERED INDEX IX_TCPT_GSGNCABI ON #TableCustomerPaymentTempDaily 
				([GuideSerie],[GuideNumber],[CreditAccountId],[BankId]);

			IF (@IdBankParam = @BankBAC)
			BEGIN
				SET @Reference = @Reference + (SELECT COUNT(1)
											   FROM #TableCustomerPaymentTempDaily);
			END;
			...
			DECLARE @MaxBatchNumber INT = 1 + (SELECT ISNULL(MAX(IdBatchCOD), 0)
											   FROM DeliveryBackOffice.dbo.BatchCOD);
			DECLARE @NewIdBatchCODCustomer INT;

			IF ((SELECT COUNT(1)
				 FROM #TableCustomerPaymentTempDaily) > 0)
			BEGIN
				-- SE CREA EL LOTE PARA EL PAGO A CLIENTES
				INSERT INTO DeliveryBackOffice.dbo.BatchCOD (BankId, BatchNumber, BatchTimeRange)
					VALUES (@IdBankParam, @MaxBatchNumber, @BatchTimeRange);

				-- OBTENCION DEL ID QUE CORRESPONDE AL LOTE CREADO
				SELECT @NewIdBatchCODCustomer = SCOPE_IDENTITY();

				IF ((@NewIdBatchCODCustomer IS NOT NULL) AND 
					(@NewIdBatchCODCustomer > 0) AND 
					(@NewIdBatchCODCustomer <> @MaxBatchNumber))
				BEGIN
				...
				BEGIN
					...
					INSERT INTO DeliveryBackOffice.dbo.BatchDetailCOD 
							(BatchCODId, GuideSerie, GuideNumber, CatDebitAccountCODId, 
							 CreditAccountId, Amount, Commission, CatTransactionTypeCODId, 
							 BankId, CatAccountTypeCODId, CatConceptCODId, Reference, 
							 Excluded, 
							 Comments,
							 BankName, TypeAccountName, AccountNumber, AccountName,CODCommissionPercentage,DiscountPrice)
					SELECT @NewIdBatchCODCustomer, tcpt.GuideSerie, tcpt.GuideNumber, tcpt.CatDebitAccountCODId, 
						   tcpt.CreditAccountId, tcpt.Amount, tcpt.Commision, tcpt.CatTransactionTypeCODId, 
						   tcpt.BankId, tcpt.CatAccountTypeCODId, tcpt.CatConceptCODId, tcpt.Reference,
						   IIF(DeliveryBackOffice.dbo.fn_validate_customer_bank_account(tcpt.AccountNumber,
																						tcpt.BankId,
																						tcpt.CatAccountTypeCODId,
																						tcpt.AccountName) IS NULL, 
							   'FALSE', 'TRUE') Excluded,
						   DeliveryBackOffice.dbo.fn_validate_customer_bank_account(tcpt.AccountNumber,
																					tcpt.BankId,
																					tcpt.CatAccountTypeCODId,
																					tcpt.AccountName) Comments,
						   tcpt.BankName, tcpt.TypeAccountName, tcpt.AccountNumber, tcpt.AccountName, CODRate,DiscountPrice
			...
			IF ((SELECT COUNT(1)
				 FROM #TableForzaPaymentTempDaily) > 0)
			BEGIN
				-- SE CREA EL LOTE PARA EL PAGO A FORZA DE LAS COMISIONES Y ENVIOS
				INSERT INTO DeliveryBackOffice.dbo.BatchCOD (BankId, BatchNumber,BatchTimeRange)
					VALUES (@BankBAC, @MaxBatchNumber, @BatchTimeRange);

				-- OBTENCION DEL ID QUE CORRESPONDE AL LOTE CREADO
				SELECT @NewIdBatchCODForza = SCOPE_IDENTITY();

				IF ((@NewIdBatchCODForza IS NOT NULL) AND 
					(@NewIdBatchCODForza > 0) AND 
					(@NewIdBatchCODForza <> @MaxBatchNumber))
				BEGIN
					UPDATE DeliveryBackOffice.dbo.BatchCOD
					SET BatchNumber = IdBatchCOD
				...
				BEGIN
					...
					INSERT INTO DeliveryBackOffice.dbo.BatchDetailCOD 
							(BatchCODId, GuideSerie, GuideNumber, CatDebitAccountCODId, 
							 ...
							 Comments,
							 BankName, TypeAccountName, AccountNumber, AccountName,CODCommissionPercentage, DiscountPrice)
					SELECT @NewIdBatchCODForza, tfpt.GuideSerie, tfpt.GuideNumber, tfpt.CatDebitAccountCODId, 
						   tfpt.CreditAccountId, tfpt.Amount, tfpt.Commision, tfpt.CatTransactionTypeCODId, 
						   tfpt.BankId, tfpt.CatAccountTypeCODId, tfpt.CatConceptCODId, tfpt.Reference,
						   IIF(DeliveryBackOffice.dbo.fn_validate_customer_bank_account(tfpt.AccountNumber,
																						tfpt.BankId,
																						tfpt.CatAccountTypeCODId,
																						tfpt.AccountName) IS NULL, 
							   'FALSE', 'TRUE') AS Excluded,
						   DeliveryBackOffice.dbo.fn_validate_customer_bank_account(tfpt.AccountNumber,
																						tfpt.BankId,
																						tfpt.CatAccountTypeCODId,
																						tfpt.AccountName) Comments,
						   tfpt.BankName, tfpt.TypeAccountName, tfpt.AccountNumber, tfpt.AccountName,CODRate,DiscountPrice
			...
			IF ((@NewIdBatchCODCustomer IS NOT NULL) AND 
				(@NewIdBatchCODForza IS NOT NULL) AND
				(@NewIdBatchCODCustomer > 0) AND 
				(@NewIdBatchCODForza > 0))
			BEGIN
		...
		ELSE
		BEGIN
			SELECT 0 PayingBank, 0 IdBatchCOD, 0 as BATCHNUMBER, 'TRUE' AS blnResult, 'NO HAY GUIAS PARA PROCESAR' Comment;
	...
	BEGIN
		...
		BEGIN
			SELECT 0 PayingBank, 0 IdBatchCOD, 0 as BATCHNUMBER, 'TRUE' AS blnResult, 'NO SE GENERO NINGUN LOTE' Comment;
		END
		ELSE IF (@NewIdBatchCODCustomer IS NULL)
		BEGIN
			SELECT @BankBAC PayingBank, @NewIdBatchCODForza IdBatchCOD, @NewIdBatchCODForza as BATCHNUMBER, 'TRUE' AS blnResult, 'SOLO SE GENERO LOTE DE COMISIONES Y ENVIOS' Comment;
		END
		ELSE IF (@NewIdBatchCODForza IS NULL)
		BEGIN
			SELECT @IdBankParam PayingBank, @NewIdBatchCODCustomer IdBatchCOD, @NewIdBatchCODCustomer as BATCHNUMBER, 'TRUE' AS blnResult, 'SOLO SE GENERO LOTE DE PAGO A CLIENTES' Comment;
		END
		ELSE
		BEGIN
			SELECT @IdBankParam PayingBank, @NewIdBatchCODCustomer IdBatchCOD, @NewIdBatchCODCustomer as BATCHNUMBER, 'TRUE' AS blnResult, 'LOTE DE PAGO A CLIENTES Y LOTE DE COMISIONES Y ENVIOS - PAGO A CLIENTES' Comment
			UNION
			SELECT @BankBAC PayingBank, @NewIdBatchCODForza IdBatchCOD, @NewIdBatchCODForza as BATCHNUMBER, 'TRUE' AS blnResult, 'LOTE DE PAGO A CLIENTES Y LOTE DE COMISIONES Y ENVIOS - COMISIONES Y ENVIOS' Comment;
		END
