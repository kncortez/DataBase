...
BEGIN
	...
	END
	else IF (@Type = 2) --VISA O MASTERCARD inento sin completar 
	BEGIN 
	SELECT 
		@IdTransaction 	= isnull([IdTransaction],0)			
		FROM DeliveryBackOffice.dbo.CreditCardTransactionByCustomer WITH(NOLOCK)
	...
	END 
	else IF (@Type = 3) --VISA O MASTERCARD Completo 
	BEGIN 
	SELECT 
		@IdTransaction 	= isnull([IdTransaction],0)			
		FROM DeliveryBackOffice.dbo.CreditCardTransactionByCustomer WITH(NOLOCK)
		WHERE OrderNumber = @OrderNumber
	    select @IdTransaction as A,@StatusSend StatusSend, @ReasonCode nRS into TMP_TEST
		--if (@IdTransaction > 0 ) 
		--begin 
			update DeliveryBackOffice.dbo.CreditCardTransactionByCustomer
			 SET 
