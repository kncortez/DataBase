...
BEGIN
    ...
    (
        SELECT STUFF(
                        (
                            SELECT ',{' + '"Guide":"' + CONCAT(cmd.GuideSerie, cmd.GuideNumber) + '",'
                                   + '"ServiceType":"' + (CASE
                                                              WHEN do.TypeService = 'NDD' THEN
                                                                  'NEXT DAY'
                                                              WHEN do.TypeService = 'TDA' THEN
                                                                  'NEXT DAY'
                                                              ELSE
                                                                  'SAME DAY'
                                                          END
                                   ...
                                   + '"TotalPieces":'
                                   + CAST(ISNULL(do.Pieces_Dry, 0) + ISNULL(do.Pieces_Cold, 0) AS NVARCHAR) + ','
                                   + '"PiecesCold":' + CAST(ISNULL(do.Pieces_Cold, 0) AS NVARCHAR) + ','
                                   + '"PiecesDry":' + CAST(ISNULL(do.Pieces_Dry, 0) AS NVARCHAR) + ','
                                   + '"StatusDescription":"' + so.OrderDescription + '",' + '"StatusOrderId":'
                                   + CAST(do.StatusOrderId AS NVARCHAR) + '}'
                            FROM DeliveryBackOffice.dbo.CorporateManifest cm WITH (NOLOCK)
                                JOIN DeliveryBackOffice.dbo.CorporateManifestDetail cmd WITH (NOLOCK)
                                    ON cmd.ManifestId = cm.IdManifest
                                       AND cmd.RowStatus = 1
                                JOIN DeliveryBackOffice.dbo.DeliveryOrder do WITH (NOLOCK)
                                    ON do.Guide_Serie = cmd.GuideSerie
                                       AND do.Guide_Number = cmd.GuideNumber
                                JOIN DeliveryBackOffice.dbo.StatusOrder so
                                    ON so.StatusOrderId = do.StatusOrderId
                            WHERE cm.ManifestSerie = @ManifestSerie
                                  AND cm.IdManifest = @ManifestNumber
                            FOR XML PATH(''), TYPE
                        ).value('.', 'varchar(max)'),
                        1,
                        1,
    ...
    BEGIN
        ...
        (
            SELECT STUFF(
                            (
                                SELECT ',{									
								"Message":"No se encontraron guias asociadas a este manifiesto.",' + '"Status": 400'
                                       + '}'
                                FOR XML PATH(''), TYPE
                            ).value('.', 'varchar(max)'),
                            1,
