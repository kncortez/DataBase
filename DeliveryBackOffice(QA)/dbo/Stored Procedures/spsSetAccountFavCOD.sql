...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			INSERT INTO DeliveryBackOffice.dbo.DeliveryFavCOD (AliasFavCOD, NameAccountFavCOD, TypeAccountFavCOD, DocumentIdFavCOD, StatusFavCOD, IdAccountFavCOD, TokenCreated, DateCreated, TokenUpdate, DateUpdate, IdBank, NumberAccFavCOD, IsDefault,PhoneNumber,NIT,FlagInstantDeposits,FlagApprovalofCODPaymentsWithTC)
				VALUES (@Alias, @NameAccount, @TypeAccount, @DocID, 1, @IdAcount, @Token, GETDATE(), NULL, NULL, @IdBank, @NumberAcc, @IsDefault,@PhoneNumber,@NIT,@FlagInstantDeposits,@FlagApprovalofCODPaymentsWithTC)
			...
			if (@Status =1) -- estado activo
				begin
					...
					SET  AliasFavCOD = @Alias , NameAccountFavCOD = @NameAccount, TypeAccountFavCOD = @TypeAccount, DocumentIdFavCOD = @DocID, StatusFavCOD = 1, IdAccountFavCOD = @IdAcount, TokenUpdate = @Token, DateUpdate = GETDATE(), IdBank = @IDBank, NumberAccFavCOD = @NumberAcc, IsDefault = @IsDefault,
					     PhoneNumber=@PhoneNumber, NIT=@NIT, FlagInstantDeposits=@FlagInstantDeposits, FlagApprovalofCODPaymentsWithTC=@FlagApprovalofCODPaymentsWithTC
