...
-- =============================================
CREATE PROCEDURE [dbo].[spws_get_rate_estimate_byheadercode]
...
-- declare @TimeLimitDate as time = '29:59:59'
	...
	--se identifica el Ecommerce en base al CodeApp enviado 
		...
		crs.CrsId as IdService,
			crs.CrsName  as ServiceName,
			crs.CrsShortName as ServiceShortName,
			rdt.RdtRate as Rate,
			iif(@IsFragile = 'true',
			(CASE
			WHEN 	rdt.RdtFragileRate > 1 THEN rdt.RdtFragileRate
			ELSE  round((rdt.RdtFragileRate * rdt.RdtRate),2)
			END),0) AS FragileRate,

			iif(@IsInsurance = 'true',
			(CASE
			WHEN 	rdt.RdtInsuranceRate > 1 THEN rdt.RdtInsuranceRate
			ELSE  round((rdt.RdtInsuranceRate * @AmmountValue),2)
			END),0) AS InsuranceRate,

			iif(@IsCollected = 'true',
			(CASE
			WHEN 	rdt.RdtCollectedRate > 1 THEN rdt.RdtCollectedRate
			ELSE  round((rdt.RdtCollectedRate * rdt.RdtRate),2)
			END),0) AS CollectedRate,

			iif(@WeightValue> rdt.RdtWeightLimit,(@WeightValue - rdt.RdtWeightLimit) *  rdt.RdtWeightAdditionalRate ,0) as WeightAdditionalRate,
			iif(@WeightValue> rdt.RdtWeightLimit,(@WeightValue - rdt.RdtWeightLimit)  ,0) as WeightAdditional,

			(select (rdt.RdtRate * sur.PercentValue /100 ) from dbo.Surcharge sur where sur.IdSurcharge = 6) as Taxes 
		 ...
		 into #GeneralRate
			from dbo.RatebyCustomer rbc
			 join dbo.RateDetail rdt on rdt.RdtIdRate = rbc.RbcIdRate
			join dbo.CatTypeService cts on cts.CtsId = rdt.RdtIdCatService
			join dbo.CatRateSegment crs on crs.CrsId = rdt.RdtIdRateSegment
			left join dbo.ScheduleByHubLogistics shu on shu.SbhIdCatService =  rdt.RdtIdCatService and shu. SbhIdRateSegment =  rdt.RdtIdRateSegment and shu.SbhIdIdHubLogistics = @HubDestiny and shu.SbhRowStatus = 1
		where  rbc.RbcIdCustomer = @IdCustomer and rdt.RdtIdRateSegment = @IdRateSegment and rdt.RdtRowStatus = 1  and  convert(datetime, @Time, 108)<=isnull(convert(datetime, shu.SbhCollectionTimeLimit, 108) ,convert(datetime, '23:59:59', 108))
		...
		BEGIN TRANSACTION;
		 ...
		 set @jsonResult =( SELECT STUFF(( 
					...
					'"Service":"' + grate.ServiceName + '",' +
						...
						'"Price":"' + convert(varchar(20),(grate.Rate +grate.FragileRate + grate.InsuranceRate + grate.CollectedRate + grate.WeightAdditionalRate + grate.Taxes)) + '",' +
						'"Currency":"' + @Currency + '",' +
						...
						'},'+
						'{"Description":"' + 'IVA' + '",' +
						'"Price":"' +convert(varchar(20),grate.Taxes) + '",' +
						'"Currency":"' + @Currency + '"' +
						'}'+
