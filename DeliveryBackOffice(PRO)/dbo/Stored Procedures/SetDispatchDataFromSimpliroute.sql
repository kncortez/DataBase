    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            WHERE RP.CatRouteId = @IdRoute
                  AND CAST(RP.DateCreated AS DATE) = CAST(@DateOfEvent AS DATE)
                  AND RP.RowStatus = 1;
            ...
            BEGIN
                ...
                VALUES
                (CAST(@DateOfEvent AS DATE), @GuideQuantity, @PiecesDryDispatched, @PiecesColdDispatched, 1, @Token,
                 GETDATE(), NULL, NULL, @IdRoute, 1);
                ...
                BEGIN
                    ...
                    SELECT RPDs.IdRoutePreparationDetail,
                           DOP.NoPiece,
                           IIF(DOP.IsDry = 1, 1, 0),
                           1,
                           GETDATE(),
                    ...
                    FROM [DeliveryBackOffice].[dbo].[DeliveryOrderPiece] DOP
                        JOIN @RoutePreparationDetails RPDs
                            ON DOP.GuideSerie = RPDs.GuideSerie
                               AND DOP.GuideNumber = RPDs.GuideNumber
                        LEFT JOIN [DeliveryBackOffice].[dbo].[RoutePreparationDetailPiece] RPDP
                            ON RPDs.IdRoutePreparationDetail = RPDP.RoutePreparationDetailId
                               AND DOP.NoPiece = RPDP.PieceNumber
                               AND RPDP.RowStatus = 1
                    WHERE RPDP.IdRoutePreparationDetailPiece IS NULL;
					...
					*/
                    ...
                    UPDATE DOP
                    SET DOP.StatusOrderId = NULL --- Programado para entrega
                    FROM @GuidesTable lg
                        JOIN [DeliveryBackOffice].[dbo].[DeliveryOrderPiece] DOP
                            ON DOP.GuideNumber = lg.GuideNumber
                               AND DOP.GuideSerie = lg.GuideSerie;
                    ...
                    FROM RoutePreparationDetail rpd
                        ...
                        JOIN @GuidesTable lg
                            ON rpd.Guide_Serie = lg.GuideSerie
                               AND rpd.Guide_Number = lg.GuideNumber
                    WHERE rp.DateRoutePreparation = CAST(@DateOfEvent AS DATE)
                          AND rp.IdRoutePreparation <> @IdRoutePreparation;

                    -- UPDATE DE TABLA DE SERVICIOS DE PLATAFORMA EXTERNA
                    UPDATE EPS
                    SET EPS.[Plan] = GT.[Plan],
                        EPS.[Route] = GT.[Route],
                        EPS.[Order] = GT.[Order],
                ...
                BEGIN
                    ...
                    (
                        ...
                        WHERE RoutePreparationId = @IdRoutePreparation
                              AND Guide_Serie = lg.GuideSerie
                              AND Guide_Number = lg.GuideNumber
                              AND RowStatus = 1
                    );
                    ...
                    SELECT NA.IdRoutePreparationDetail,
                           DOP.NoPiece,
                           IIF(DOP.IsDry = 1, 1, 0),
                           1,
                           GETDATE(),
                    ...
                    FROM [DeliveryBackOffice].[dbo].[DeliveryOrderPiece] DOP
                        JOIN @NewAdded NA
                            ON DOP.GuideSerie = NA.GuideSerie
                               AND DOP.GuideNumber = NA.GuideNumber
                        LEFT JOIN [DeliveryBackOffice].[dbo].[RoutePreparationDetailPiece] RPDP
                            ON RPDP.RoutePreparationDetailId = NA.IdRoutePreparationDetail
                               AND DOP.NoPiece = RPDP.PieceNumber
                               AND RPDP.RowStatus = 1
                    WHERE RPDP.IdRoutePreparationDetailPiece IS NULL;
                    ...
                    UPDATE RPD
                    SET RPD.GuideOrder = GT.[Order]
                    FROM [DeliveryBackOffice].[dbo].[RoutePreparationDetail] RPD
                        JOIN @GuidesTable GT
                            ON RPD.Guide_Serie = GT.GuideSerie
                               AND RPD.Guide_Number = GT.GuideNumber
                               AND RPD.RowStatus = 1
                    ...
                    BEGIN
                        ...
                        DECLARE @NewGuideQuantity INT =
                                (
                                    SELECT COUNT(*)FROM @NewAdded
                                );
                        DECLARE @NewPiecesDryDispatched INT =
                                (
                                    SELECT SUM(GuideDry)FROM @NewAdded
                                );
                        DECLARE @NewPiecesColdDispatched INT =
                                (
                                    SELECT SUM(GuideCold)FROM @NewAdded
                                );
        ...
        BEGIN
            ...
            BEGIN
                SELECT @IdRoutePreparation AS 'StatusCode',
