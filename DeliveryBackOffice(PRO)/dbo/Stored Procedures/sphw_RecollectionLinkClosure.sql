...
BEGIN
    ...
    SELECT @CodeOfReference = VPDL.VisitPointId
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            VALUES
            (@CodeOfReference, @IdCustomer, @IdTownship, @IdCountry, @NameVP, @ContactName, @Address1, @Phone,
             @TownShipName, @ProvinceName, @RecollectionLatitude, @RecollectionLongitude, 6, @IdKindOfVPBusiness, 1,
             @Token, GETDATE(), 1);
            ...
            VALUES
            (@IdTownship, @IdAccount, @IdCountry, @NameVP, @Address1, @NirPhone, @Phone, @AdditionalInstructions, 1,
        ...
        BEGIN
            ...
            VALUES
            (@IdTownship, @IdAccount, @IdCountry, @NameVP, @Address1, '', @NirPhone, @Phone, @AdditionalInstructions,
             1  , @Token, GETDATE(), NULL, NULL, @CodeOfReference, @IdCityPlace);
        ...
        BEGIN
            ...
            EXECUTE [dbo].[sphw_RegisterRecollectionRequest] @TAC1,
                                                             @TAC2,
                                                             @TAC3,
                                                             @Scheduled,
                                                             @CodeOfReference,
        ...
        IF (
           (
               SELECT StatusCode FROM @RESULTREGISTERRECOLECTION
           ) = 1
           OR  ISNULL(@IsOnlyVisitPoint, 1) = 1
           )
        BEGIN
            --CERRANDO LINK
            UPDATE dbo.VisitPointDataLink
            SET DataLinkStatusId =
                (
                    SELECT IdCatDataLinkStatus
                    FROM dbo.CatDataLinkStatus
                    WHERE DataLinkStatusName = 'Completado'
                )
            ...
            BEGIN
                SELECT 1 'StatusCode',
                       'Punto de visita generado exitosamente' 'Description',
                       0 'ServiceId';
            END;
            ELSE
            BEGIN
                SELECT StatusCode 'StatusCode',
                       ...
                       ServiceId 'ServiceId'
                FROM @RESULTREGISTERRECOLECTION;
            END;
