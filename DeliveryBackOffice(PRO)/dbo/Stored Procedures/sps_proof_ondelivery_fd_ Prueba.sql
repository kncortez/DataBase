...
CREATE PROCEDURE [dbo].[sps_proof_ondelivery_fd_ Prueba]
...
BEGIN
   ...
   -- DECLARE @PhotoColdVB VARBINARY(MAX);
	...
	DECLARE @StatusEXC AS INT = ( SELECT TOP 1 StatusOrderId FROM StatusOrder WHERE OrderDescription = 'Reenviado a Express Center') --FDAPI-337
	--Se obtiene el IdDeliveryOption configurado
	DECLARE @IdDeliveryOption AS INT  = (SELECT TOP 1 IdDeliveryOption FROM DeliveryBackOffice.dbo.CatDeliveryOptions WHERE Name = 'Express Center') --FDAPI-337
	--Se obtiene el IdDeliveryOption que tiene la guía
	DECLARE @IdDeliveryOptionGuide AS INT 
	DECLARE @IsExpress AS bit 
	
	SELECT TOP 1 @IdDeliveryOptionGuide=IdDeliveryOption , @IsExpress= IIF(ISNULL(kvp.KindOfVPName,'') = 'Express Center','true','false')  FROM DeliveryBackOffice.dbo.DeliveryOrder  LEFT JOIN dbo.VisitPointClient vpr ON vpr.CodeOfReference = DeliveryOrder.Receiver_ID  LEFT JOIN dbo.KindOfVPClient kvp ON kvp.IdKindOfVPClient = vpr.IdKindOfVPClient WHERE Guide_Serie = @GuideSerie AND Guide_Number = @GuideNumber  --FDAPI-337
        ...
        BEGIN
			...
			PRINT @ExcludeCODPyament
                ...
                SET NameOfReceiver = @ReceiverName,
					...
					LastCollectOnDelivery = IIF(@ExcludeCODPyament = 'false', 0, Collect_OnDelivery),
                ...
                VALUES
                (@GuideSerie, @GuideNumber, IIF(@IdDeliveryOptionGuide = @IdDeliveryOption, @StatusEXC,IIF(@IsExpress ='true',@StatusEXC,5)), @Token, GETDATE(), GETDATE(), NULL,
