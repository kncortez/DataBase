...
BEGIN
	...
	FROM [DeliveryBackOffice].[dbo].[Membership] WHERE AccountId = @IdAcount  AND RowStatus = 1

	--- validar si cliente posee credito​
	SELECT
		TOP 1 
			@CustomerType = ISNULL(Cu.IdCustomerType,0)
		   ,@HasCredit = ISNULL((CASE WHEN CCOP.ConditionOfPaymenAbbreviation LIKE '%CREDITO%' THEN 1 ELSE 0 END),0) ---custumerType es 1 para corporativos
  ...
  BEGIN TRY
	...
	​
			...
			SELECT
				...
				,(
					CASE
						WHEN @CustomerType = 1 AND @HasCredit = 1 AND @IdTarjeta = 0 AND @TypeOfInMoneyId = 8 THEN NULL
						WHEN @CustomerType = 2 THEN NULL
						ELSE @IdTarjeta
					END
			...
			WHERE
				...
				NOT EXISTS (
								...
								WHERE
									M.CustomerId = @Idcustumer
									AND
									(M.AccountId = @IdAcount)
									AND
									M.RowStatus = 1
							)
			...
			​  ----------Rango de descuento
				INSERT INTO
					[DeliveryBackOffice].[dbo].[MembershipDiscountRange]
					(MembershipId, ValueTypeId, DiscountValue, DiscountLowServiceRange, DiscountTopServiceRange, RowStatus, TokenCreated, DateCreated)
				SELECT
					ANM.IdNewMembership, CMDR.ValueTypeId, CMDR.DiscountValue, CMDR.DiscountLowServiceRange, CMDR.DiscountTopServiceRange, 1, @Token, GETDATE()
				FROM
					[DeliveryBackOffice].[dbo].[CatMembershipDiscountRange] CMDR WITH (NOLOCK)
					CROSS JOIN
						@AuxNewMembership ANM
				WHERE
					CMDR.CatMembershipId = @IdSalePackage

			​---- Log de pago de membresia
				INSERT INTO
					[DeliveryBackOffice].[dbo].[MembershipPaymentLog]
					(MembershipId, TypeOfInOutOfMoneyId, [Authorization], RowStatus, TokenCreated, DateCreated)
				SELECT
					ANM.IdNewMembership
					,@TypeOfInMoneyId
					,(
						CASE
							WHEN @CustomerType = 1 AND @HasCredit = 1 AND @IdTarjeta = 0 AND @TypeOfInMoneyId = 8 THEN 'CREDIT'
							WHEN @CustomerType = 2 AND @TypeOfInMoneyId = 1 THEN 'CASH'
							ELSE @Vaucher
						END
					)
					,1
				...
				(
					SELECT STUFF(
									(
										SELECT '{{"IdResult":200,' + '"Message":"Adquisición de :'+ @TypeSalePackage  +' Exitosa...!!", "ActivationCode": "'+ISNULL(@ActivationCode, '')+'" }'
										FOR XML PATH(''), TYPE
									).value('.', 'varchar(max)'),
									1,
									1,
	...
	​	
		...
		DECLARE @AuxNewSubscriptions AS TABLE (IdNewSubscriptions INT);
				...
				NOT EXISTS (
								SELECT
									TOP 1
										1
								FROM
									[DeliveryBackOffice].[dbo].[Subscription] S WITH (NOLOCK)
								WHERE
									S.CustomerId = @Idcustumer
									AND
									(S.AccountId = @IdAcount)
									AND
									S.CatSubscriptionId = @IdSalePackage
									AND
									S.RowStatus = 1
							)
			...
			​  ----------Rango de descuento
				INSERT INTO
					[DeliveryBackOffice].[dbo].[SubscriptionDiscountRange]
					(SubscriptionId, ValueTypeId, DiscountValue, DiscountLowServiceRange, DiscountTopServiceRange, RowStatus, TokenCreated, DateCreated)
				SELECT
					ANM2.IdNewSubscriptions, CMDR.ValueTypeId, CMDR.DiscountValue, CMDR.DiscountLowServiceRange, CMDR.DiscountTopServiceRange, 1, @Token, GETDATE()
				FROM
					[DeliveryBackOffice].[dbo].[CatSubscriptionDiscountRange] CMDR WITH (NOLOCK)
					CROSS JOIN
						@AuxNewSubscriptions ANM2
				WHERE
					CMDR.CatSubscriptionId = @IdSalePackage

			​---- Log de pago de membresia
				INSERT INTO
					[DeliveryBackOffice].[dbo].[SubscriptionPaymentLog]
					(SubscriptionId, TypeOfInOutOfMoneyId, [Authorization], RowStatus, TokenCreated, DateCreated)
				SELECT
					ANM2.IdNewSubscriptions
					,@TypeOfInMoneyId
					,(
						CASE
							WHEN @CustomerType = 1 AND @HasCredit = 1 AND @IdTarjeta = 0 AND @TypeOfInMoneyId = 8 THEN 'CREDIT'
			...
			BEGIN
				 ...
				 SET @JsonResponse  =
						(
							SELECT STUFF(
											(
												SELECT '{{"IdResult":500,' + '"Message":"Adquisición de :'+ @TypeSalePackage  +' no fue posible...!!" }'
												FOR XML PATH(''), TYPE
											).value('.', 'varchar(max)'),
											1,
											1,
		...
		ROLLBACK TRANSACTION;
		 ...
		 SET @JsonResponse  =
				(
					SELECT STUFF(
									(
										SELECT '{{"IdResult":500,' + '"Message":"Adquisición de :'+ @TypeSalePackage  +' no fue posible...!!"'+ISNULL(@StatusSubcription,'0')+' }'
										FOR XML PATH(''), TYPE
									).value('.', 'varchar(max)'),
									1,
									1,
