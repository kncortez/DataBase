...
BEGIN
    ...
    SET @EXISTING_LRP =
    (
        SELECT COUNT([LRP].[IdLinehaulRoutePreparation]) AS IdLinehaulRoutePreparation
    ...
    SET @EXISTING_TAG =
    (
        SELECT COUNT([LCM].[IdLinehaulRoutePreparationCustomsMark]) AS IdLinehaulRoutePreparationCustomsMark
    ...
    BEGIN
        SELECT 0 [spResult],
               'CUI de piloto ingresado NO válido' [spMessage];
        RETURN;
    END;
    ...
    BEGIN
        SELECT 0 [spResult],
               'No existe ningún registro de despacho de linehaul con los datos ingresados' [spMessage];
        RETURN;
    END;
    ...
    BEGIN
        SELECT 2 [spResult],
               'Marchamo ya ha sido registrado en otro proceso de linehaul.' [spMessage];
        RETURN;
    END;

    SET @EXISTING_EMPTY_CONTAINERS =
    (
    ...
    BEGIN TRY
        ...
        VALUES
        (@IdLinehaulRoutePreparation, @Tag, 1, @TknUser, SYSDATETIME());
